...
// id 回退链（栏 → 部件 → 页面）读同一份（D-20）。
//
// ★模式换的是时间轴的含义，不是三个工具（D-18）：
//   配置 configure — 没有时间轴：一个时刻，一个解（zerod · discharge ·
//                    pfwave · breakdown 四条栏）
//   设计 design    — 整条脉冲已经存在，播放头在其中来回读（pulse 栏）
//   仿真 simulate  — 只有过去，右缘是现在（sim 栏）
//
// ★一个场景一个控制器文件，按页面运行它们的顺序分节——本文件依次是：
//   §0 模式开关（只管可见性，不持有任何状态）
//   §1 配置模式的四条栏（原 scenario-design.js）
//   §2 设计模式（原 scenario-pulse.js）
//   §3 仿真模式（原 scenario-sim.js）
//
// ★一个部件，六条栏：部件是「谁拥有运行链与工具条」，而模式是「时间轴是什么
// 意思」——它们不是同一件事，所以模式不是部件。合并前每页各有一个部件，那只是
// 「一页一个部件」的副产品。
(function (root) {
  'use strict';
  //: 三节共用的这一个部件，先于它们建立
  root.__FY_PD_PART = root.FyScenario.part('pulse_design',
                                           { lockWhileBusy: ['run'] });
})(self);
...
//
//   配置 configure — there is NO axis.  One instant, one solve: the operating
//                    point, the coils that hold a shape, the breakdown null.
//   设计 design    — the whole pulse already exists and a play-head reads it.
//   仿真 simulate  — only the PAST exists; the right-hand edge is now.
//
// ★★WHAT THIS FILE MUST NOT DO is own any state.  The shared controls — this
// shot's geometry, current, phases, heating, limits — live in the page's two
// `page-shared` panels and are read by every bar through the framework's own
// id fallback (bar → part → page).  If the switch copied values between modes
// there would again be three answers to "what is I_p on this shot", which is
// exactly the thing the merge removed (D-20).  So: this file toggles
// visibility, writes one sentence, and stops.
//
// ★A mode block is HIDDEN, never rebuilt (D-19).  Rebuilding would drop the
// canvases, the folded state of every 功能栏 and every value typed into the
// mode's own controls — the failure three separate pages already had, one
// navigation at a time.
(function (root) {
  'use strict';

  var PAGE = 'pulse_design';
  var MODES = ['configure', 'design', 'simulate'];
  var DEFAULT = 'design';
  var current = null;

  function bar() { return document.getElementById(PAGE + '-modebar'); }
  function note() { return document.getElementById(PAGE + '-mode-note'); }
  function blocks() {
  ...
  function paintNote() {
    var e = note();
    if (!e || !root.FyI18n) return;
    //: the note says what the TIME AXIS means here — that is the mode's
    //: definition, and it is the one thing a reader cannot see by looking
    e.setAttribute('data-i18n', 'pd.mode.note.' + current);
    e.innerHTML = root.FyI18n.t('pd.mode.note.' + current);
  }

  /** Show one mode, hide the others.  Nothing else changes. */
  function set(mode, opts) {
    if (MODES.indexOf(mode) < 0) mode = DEFAULT;
    ...
    if (!opts || opts.hash !== false) {
      try { root.history.replaceState(null, '', '#' + mode); } catch (e) { /* file:// */ }
    }
    //: ★A canvas drawn while its block was hidden measured 0 × 0 and stayed
    //: that way (`plot.js` clamps to 1 px).  The framework already has the
    //: hook for "the box you draw into changed size" — the resize refreshers
    //: every bar registers — so showing a mode fires exactly that, rather
    //: than this file learning what each bar draws.
    root.dispatchEvent(new Event('resize'));
  }

  //: ★`#part-<栏>` 也要认：闸子与入口卡片按**栏**深链（`#part-zerod`），而栏
  //: 属于哪个模式是这一页自己的事，不是链接调用方该知道的。旧的三条入口就是
  //: 这样被引用的，合并不该让那些链接失效——它们指的仍是同一件事。
  var OF_BAR = { zerod: 'configure', discharge: 'configure',
                 pfwave: 'configure', breakdown: 'configure',
                 pulse: 'design', sim: 'simulate' };

  function fromHash(h) {
    var want = (h || '').replace('#', '');
    if (MODES.indexOf(want) >= 0) return want;
    if (want.indexOf('part-') === 0) {
      var b = want.slice(5);
      if (OF_BAR[b]) return OF_BAR[b];
      if (MODES.indexOf(b) >= 0) return b;
    }
    return DEFAULT;
  }
  ...
  function init() {
    ...
    current = null;
    set(fromHash(root.location.hash), { hash: false });
    root.addEventListener('hashchange', function () {
      set(fromHash(root.location.hash), { hash: false });
    });
    if (root.FyI18n && root.FyI18n.onChange) root.FyI18n.onChange(paintNote);
  }

  //: ★`forBar` 是给深链与闸子的：一条栏在哪个模式里是这一页自己的事，问它的人
  //: 不该自己维护一张表——那张表迟早与页面不一致。
  root.FyModes = { set: set, get: function () { return current; }, all: MODES,
                   forBar: function (b) { return OF_BAR[b] || DEFAULT; } };
...
// ========================================================================
// §1 配置模式 configure — 工况 · 位形反解 · PF 波形 · 击穿
// ========================================================================
...
var DESIGN = self.__FY_PD_PART;            //: §1 配置模式
...
FyScenario.whenDevices(function () {
  ...
  var S = DESIGN.bar('zerod', {
    ...
    sliders: { ip: 0, ne: 1, te: 1, tite: 2, pn: 1, pt: 1, zeff: 1, dtf: 2,
               kappa: 2, paux: 1, pfscale: 2, hfac: 2, meff: 2, w0: 1,
  ...
  function phases() {
    return { t_breakdown: 0.0,
             t_rampup_end: +$('t_ru').value,
             t_flattop_end: +$('t_ft').value,
             t_end: +$('t_end').value };
  }
  ...
  var CONTROLS = ['t_ru', 't_ft', 't_end', 'ip', 'ne', 'te',
                  'tite', 'pn', 'pt', 'zeff', 'dtf', 'r0', 'a', 'kappa',
                  'paux', 't_on', 't_off', 'tau_law', 'hfac', 'meff', 'w0',
                  'phiavail'];
  ...
  ['ip', 'ne', 'te', 'tite', 'pn', 'pt', 'zeff', 'dtf', 'kappa', 'paux']
    .forEach(function (id) {
      $(id).addEventListener('input', function () { syncLabels(); });
      $(id).addEventListener('change', run);
    });
  ['t_ru', 't_ft', 't_end', 't_on', 't_off', 'r0', 'a',
...
FyScenario.whenDevices(function () {
  ...
  function drawCtlRows() {
    ...
    if (!ctlRows.length) {
      ...
      host.innerHTML = ctlRows.map(function (row, i) {
        var id = 'pulse_design-discharge-ctl' + i + '-';
  ...
  function handOver() {
    var el = document.getElementById('pulse_design-handoff');
    if (!el) return;
...
FyScenario.whenDevices(function () {
  ...
  // reason.
  var PCONTROLS = ['t_bd', 't_ru', 't_ft', 't_end', 'npts', 'a0',
                   'nverify', 'vcap'];
  ...
  ['t_bd', 't_ru', 't_ft', 't_end'].forEach(function (id) {
...
// other arrangement the design document argues for: ONE pulse script — four
// phase times, an Ip trapezoid, a boundary trajectory, a heating window — and
// every panel is a VIEW on it.
//
// What runs, in order, and where each number comes from:
//
//   1. `zerod`  the whole discharge as 0-D traces (the kernel's, one call)
//   2. `pulse`  the boundary trajectory as per-channel current and VOLTAGE
//               waveforms, plus a free-boundary solve at the instants asked
//               for — those, and only those, are 已解片
//
// ★NOTHING HERE COMPUTES PHYSICS.  The trapezoid is the kernel's
// (`zerodWaveform`, D-3: the phase boundaries decide what a run IS, so they
// have one host), the boundary is the kernel's `millerBoundary`, the traces
// are `zerodEvaluate`'s and the waveforms are the feed-forward inverse of the
// circuit integrator this repo already ships.  This file decides WHICH
// instants to ask about and how to draw the answers.
//
// ★A SLICE THE PAGE DID NOT SOLVE SAYS SO.  The play-head moves over a grid
// of ~120 instants; the equilibrium is solved at a handful.  Everything else
// is the target boundary — what was ASKED for — and the panel says which of
// the two the reader is looking at (D-8).  A page that drew an interpolated
// boundary in the colour of a solved one would be reporting a wrong number as
// a right one, which is the failure `validate-design.mjs` exists to catch.

var PULSE = self.__FY_PD_PART;             //: §2 设计模式

FyScenario.whenDevices(function () {
  'use strict';

  //: ★THE PAGE'S ONE 功能栏, declared through the framework rather than left
  //: implicit: a bar owns its run key, its fold and — the reason this matters
  //: here — the unfolding of its result panels when a run finishes.  A part
  //: that reports through the part-level `setBusy` leaves five panels folded
  //: over the answer it just computed.
  var S = PULSE.bar('pulse', {
    title: 'nav.pulse',
    sliders: { ip: 0, a: 3, kappa: 2, du: 2, dl: 2, a0: 2, a1: 2,
               paux: 1, ne: 1, te: 1, npts: 0, nverify: 0 },
    on: { ready: onReady, error: onError, zerod: onZerod, pulse: onPulse,
          progress: onProgress },
  });
  var $ = S.$, T = FyI18n.t;
  var M = self.FYLITE_MACHINE, P = self.FyPhys;
  ...
  /** The four phase times, in the kernel's own order. */
  function phases() {
    return [+$('t_bd').value, +$('t_ru').value,
            +$('t_ft').value, +$('t_end').value];
  }
  ...
  /**
   * The trapezoid, from the KERNEL.
   *
   * ★Not four lines of JavaScript.  `fylite_rs_zerod_waveform`'s own comment
   * says why it exists: both hosts had a copy of this shape and of the phase
   * test, and the phase boundaries decide what a run IS — the ramp rates, the
   * flux budget, and the label every slice is reported under.
   */
  ...
  // own.  `a1/a` is that second variable, and the X point is handed back with
  // it: a diverted plasma is asked for on the flat top and NOT during the two
  // ramps, which is what `xWeight` per waypoint would say if the worker took
  // one — it takes a single flag, so the trajectory says it by SHAPE.

  function shapeAt(ph, t) {
    var flat = {
      r0: +$('r0').value, z0: 0, a: +$('a').value,
      kappa: +$('kappa').value,
      deltaU: +$('du').value, deltaL: +$('dl').value,
    };
    var a0 = +$('a0').value, a1 = +$('a1').value, s = 1;
    if (t <= ph[1]) {
      //: the plasma grows from a small circular column into the flat-top
      //: boundary; κ and δ arrive with the size, because a small early
      //: plasma is not an elongated one
      var g = ph[1] > ph[0] ? (t - ph[0]) / (ph[1] - ph[0]) : 1;
      s = a0 + (1 - a0) * Math.max(0, Math.min(1, g));
    } else if (t > ph[2]) {
      var g2 = ph[3] > ph[2] ? (t - ph[2]) / (ph[3] - ph[2]) : 1;
      s = 1 + (a1 - 1) * Math.max(0, Math.min(1, g2));
    }
    return { r0: flat.r0, z0: 0, a: flat.a * s,
             kappa: 1 + (flat.kappa - 1) * s,
             deltaU: flat.deltaU * s, deltaL: flat.deltaL * s, frac: s };
  }

  /** The boundary polyline of one instant, flat `[r,z,r,z,…]` for the plot. */
  function boundaryFlat(target, n) {
    var pts = P.millerBoundary({ r0: target.r0, z0: target.z0, a: target.a,
                                 kappa: target.kappa, deltaU: target.deltaU,
                                 deltaL: target.deltaL }, n || 121);
    var out = new Float64Array(pts.length * 2);
    for (var i = 0; i < pts.length; i++) {
      out[2 * i] = pts[i][0]; out[2 * i + 1] = pts[i][1];
    }
    return out;
  }
  ...
  var last = null;        // {in, out, limits}
  var pf = null;          // the pulse answer
  var grid = null;        // {t, phase}

  function inputs() {
    var ph = phases();
    var t = linspace(ph[0], ph[3], NT);
    var ipFlat = +$('ip').value * 1e3;
    var neFlat = +$('ne').value * 1e19;
    var teFlat = +$('te').value;
    var pAux = +$('paux').value * 1e6;
    var tOn = +$('t_on').value, tOff = +$('t_off').value;
    var ip = wave(ph, t, ipFlat, 0, 0);
    var ne0 = wave(ph, t, neFlat, 0.02 * neFlat, 0.02 * neFlat);
    var te0 = wave(ph, t, teFlat, 0.01 * teFlat, 0.01 * teFlat);
    var pInj = new Float64Array(NT);
    for (var i = 0; i < NT; i++)
      pInj[i] = (t[i] >= tOn && t[i] <= tOff) ? pAux : 0;
    return {
      t: t, ip: ip, ne0: ne0, te0: te0, pInj: pInj, phases: ph,
      rho: linspace(0, 1, NR),
      par: new Float64Array([0.9, 1.0, 1.5, 0.05, +$('r0').value,
                             +$('a').value, +$('kappa').value,
                             1.8, 0.9, 0.5]),
    };
  }
  ...
  function rateNote() {
    var ph = phases(), ipFlat = +$('ip').value * 1e-3;   // MA
    var up = ph[1] > ph[0] ? ipFlat / (ph[1] - ph[0]) : NaN;
    var dn = ph[3] > ph[2] ? ipFlat / (ph[3] - ph[2]) : NaN;
    $('rate-note').textContent =
      T('p.rate', { up: f(up, 2), down: f(dn, 2) });
  }
  ...
  function run() {
    if (!P.kernel()) {
      S.setBusy(false, T('p.no_kernel'), 'err', 'fail');
      return Promise.resolve();
    }
    var d = inputs();
    last = null; pf = null;
    S.setBusy(true, T('p.running'));
    S.progress(0.15);
    S.send({ cmd: 'zerod', t: Array.from(d.t), ip: Array.from(d.ip),
             ne0: Array.from(d.ne0), te0: Array.from(d.te0),
             pInj: Array.from(d.pInj), rho: Array.from(d.rho),
             par: Array.from(d.par),
             geom: { r0: +$('r0').value, a: +$('a').value,
                     kappa: +$('kappa').value },
             phases: d.phases, li: 0.9, phiAvail: 0 });
    pending = d;
    return S.settle('zerod').then(function () { return sendPulse(d); });
  }

  var pending = null;

  /** The waypoints: fewer instants than the corridor, each one a design. */
  function sendPulse(d) {
    var ph = d.phases, n = +$('npts').value;
    var ts = [], i;
    for (i = 0; i < n; i++) ts.push(ph[0] + (ph[3] - ph[0]) * i / (n - 1));
    var ipFlat = +$('ip').value * 1e3;
    var ipAt = wave(ph, new Float64Array(ts), ipFlat, 0, 0);
    var wps = ts.map(function (t, k) {
      var sh = shapeAt(ph, t);
      return { t: t, ip: ipAt[k],
               target: { r0: sh.r0, z0: sh.z0, a: sh.a, kappa: sh.kappa,
                         deltaU: sh.deltaU, deltaL: sh.deltaL } };
    });
    //: ★THE CHECKS ARE SPREAD OVER THE PHASES, not over the index range.
    //: Evenly spaced indices put all three inside the flat top on a default
    //: pulse — three checks of the same standing configuration, and nothing
    //: at all about the two ramps, which is where a trajectory is hard.
    var nv = +$('nverify').value, want = [];
    var mid = function (a, b, g) { return a + (b - a) * g; };
    if (nv >= 1) want.push(mid(ph[1], ph[2], 0.5));       // flat top
    if (nv >= 2) want.push(mid(ph[0], ph[1], 0.7));       // ramp-up
    if (nv >= 3) want.push(mid(ph[2], ph[3], 0.4));       // ramp-down
    for (i = 3; i < nv; i++)
      want.push(mid(ph[1], ph[2], (i - 2) / (nv - 2 + 1)));
    var verify = [];
    want.forEach(function (tw) {
      var best = -1, bd = Infinity;
      ts.forEach(function (tv, k) {
        var d = Math.abs(tv - tw);
        if (d < bd && wps[k].ip > 0 && verify.indexOf(k) < 0) { bd = d; best = k; }
      });
      if (best >= 0) verify.push(best);
    });
    verify.sort(function (a, b) { return a - b; });
    grid = { t: ts, ph: ph, verify: verify };
    S.setBusy(true, T('p.designing', { n: n }));
    S.progress(0.5);
    S.send({ cmd: 'pulse', waypoints: wps, nPoints: 24, xWeight: 0,
             verify: verify, prof: { beta0: 0.55, emp: 1.0, enp: 1.0,
                                     r0: +$('r0').value },
             solve: { maxIter: 600, relax: 0.3 },
             etaVessel: M && M.vessel_resistivity_uohm_m
               ? M.vessel_resistivity_uohm_m * 1e-6 : undefined });
    return S.settle('pulse');
  }
  ...
  function onReady() {
    S.setBusy(false, T('p.ready'), '', 'idle');
    syncAll();
  }

  function onProgress(m) {
    if (m.total) S.progress(0.5 + 0.4 * (m.pass / m.total));
  }

  function onError(m) {
    S.setBusy(false, (m.where ? m.where + ': ' : '') + m.message, 'err', 'fail');
  }
  ...
  function onPulse(m) {
    ...
    drawChecks();
    drawSlice();
    var over = channelsOverCap();
    S.setBusy(false,
              over.length ? T('p.done.over', { n: over.length })
                          : T('p.done', { n: grid.t.length }),
              over.length ? 'warn' : '', over.length ? 'miss' : 'done');
    S.progress(1);
  }
  ...
  function phaseBands() {
    if (!last) return [];
    var ph = last.in.phases, col = FyPlot.palette($('tr-ip'));
    return [{ x0: ph[0], x1: ph[1], color: col.accent, label: T('p.ph.up') },
            { x0: ph[1], x1: ph[2], color: col.alt, label: T('p.ph.flat') },
            { x0: ph[2], x1: ph[3], color: col.coil, label: T('p.ph.down') }];
  }

  function sliceIndex() { return Math.min(+$('slice').value, NT - 1); }

  function tNow() { return last ? last.in.t[sliceIndex()] : 0; }

  function trace(canvas, series, ylabel) {
    var o = { series: series, xlabel: T('p.axis.t'), ylabel: ylabel,
              bands: phaseBands() };
    if (last) o.marker = tNow();
    FyPlot.xy($(canvas), o);
  }

  function drawCorridor() {
    if (!last) return;
    var t = last.in.t, o = last.out;
    var col = FyPlot.palette($('tr-ip'));
    trace('tr-ip', [{ x: t, y: mul(last.in.ip, 1e-3), color: col.accent,
                      label: 'Ip [kA]' }], 'kA');
    trace('tr-vloop', [{ x: t, y: o.vLoop, color: col.accent,
                         label: 'V_loop [V]' }], 'V');
    trace('tr-heat', [
      { x: t, y: mul(last.in.pInj, 1e-6), color: col.lcfs, label: 'P_aux [MW]' },
      { x: t, y: mul(o.pFus, 1e-6), color: col.alt, label: 'P_fus [MW]' },
    ], 'MW');
    trace('tr-q', [{ x: t, y: o.q, color: col.accent, label: 'Q' }], 'Q');
    $('slice-label').textContent = T('p.slice.at', { t: f(tNow(), 2) });
  }

  function mul(v, k) {
    var out = new Float64Array(v.length);
    for (var i = 0; i < v.length; i++) out[i] = v[i] * k;
    return out;
  }

  /**
   * The verified check this slice IS, if any.
   *
   * ★★THE TWO GRIDS DO NOT SHARE INSTANTS, and an exact test therefore never
   * matched: the corridor is ~120 evenly spaced points and the waypoints are
   * a dozen or two, so a solved instant falls BETWEEN two play-head
   * positions.  The page could not reach the sentence 「已解片」 at all — it
   * reported every slice as interpolated, which is the safe direction to be
   * wrong in and still wrong.  A slice counts as the solved one when the
   * play-head is within half a corridor step of it, and the note prints the
   * instant the solve was actually done at rather than the one under the
   * cursor.
   */
  function checkAt(t) {
    if (!pf || !pf.checks || !last) return null;
    var dt = (last.in.t[1] - last.in.t[0]) || 0;
    var tol = Math.max(dt * 0.5, 1e-9), best = null, bd = Infinity;
  ...
  function nearestCheck(t) {
    ...
    return best ? { c: best, dt: bd } : null;
  }

  function drawSlice() {
    if (!last) return;
    var t = tNow(), k = sliceIndex(), ph = last.in.phases;
    ...
    if (solved && solved.shape) {
      ...
      //: polyline, so this is a reconstruction and the note says so
      o.lcfs = boundaryFlat({ r0: solved.shape.r0, z0: solved.shape.z0 || 0,
                              a: solved.shape.a, kappa: solved.shape.kappa,
                              deltaU: solved.shape.deltaU,
                              deltaL: solved.shape.deltaL });
    }
    S.cross(eq, o);
    $('cross-legend').innerHTML = FyPlot.legendHTML(
      solved ? [{ label: T('p.leg.target'), color: FyPlot.palette($('cross')).accent,
                  kind: 'line', dash: [4, 4] },
                { label: T('p.leg.got'), color: FyPlot.palette($('cross')).lcfs,
                  kind: 'line' }]
             : [{ label: T('p.leg.target'), color: FyPlot.palette($('cross')).accent,
                  kind: 'line', dash: [4, 4] }]);
    $('cross-cap').textContent = T('p.slice.cap', { t: f(t, 2) });
    //: ★已解 / 插值 — the sentence D-8 asks for, in the panel that shows the
    //: boundary rather than in a caption somewhere else
    if (solved) {
      $('slice-note').className = 'note';
      //: ★what a verified slice can actually say.  The verification solve is
      //: summarised WITHOUT a profile block, so q95 and the boundary class
      //: come back empty — reporting them as 「—」 reads as a broken panel.
      //: The number this slice does have is the one that matters here: how
      //: far the boundary it got is from the boundary it was asked for.
      var tg = solved.target || {}, sh = solved.shape || {};
      var derr = (isFinite(sh.a) && isFinite(tg.a))
        ? f(Math.abs(sh.a - tg.a) * 1e3, 1) + ' mm' : '—';
      //: ★the STATE as an attribute beside the sentence: a gate that greps
      //: prose breaks the next time the prose improves — and this page's two
      //: sentences both contain the word 「已解」.
      $('slice-note').setAttribute('data-slice', 'solved');
      $('slice-note').textContent = T('p.slice.solved', {
        t: f(solved.t, 2), err: derr,
        kind: solved.bndKind || T('p.kind.unreported') });
    } else {
      var nr = nearestCheck(t);
      $('slice-note').setAttribute('data-slice', 'interpolated');
      $('slice-note').className = 'note caveat';
      $('slice-note').textContent = nr
        ? T('p.slice.interp', { t: f(t, 2), near: f(nr.c.t, 2) })
        : T('p.slice.nosolve', { t: f(t, 2) });
    }
    drawProfiles(k);
    drawCorridor();
  }

  function drawProfiles(k) {
    if (!last || !last.out.ne) return;
    var rho = last.in.rho, nr = rho.length;
    var col = FyPlot.palette($('pr-ne'));
    var ne = last.out.ne.slice(k * nr, (k + 1) * nr);
    var te = last.out.te.slice(k * nr, (k + 1) * nr);
    var ti = last.out.ti.slice(k * nr, (k + 1) * nr);
    FyPlot.xy($('pr-ne'), { series: [{ x: rho, y: mul(ne, 1e-19),
                                       color: col.accent, label: 'n_e' }],
                            xlabel: 'ρ', ylabel: '10¹⁹ m⁻³' });
    FyPlot.xy($('pr-t'), { series: [
      { x: rho, y: te, color: col.accent, label: 'T_e' },
      { x: rho, y: ti, color: col.lcfs, label: 'T_i' },
    ], xlabel: 'ρ', ylabel: 'keV' });
  }

  function drawPF() {
    if (!pf) return;
    var col = FyPlot.palette($('icur')), nch = pf.nch;
    var si = [], sv = [];
    for (var c = 0; c < nch; c++) {
      var yi = new Float64Array(pf.t.length), yv = new Float64Array(pf.t.length);
      for (var k = 0; k < pf.t.length; k++) {
        yi[k] = pf.x[k * nch + c] * 1e-3;
        //: ★`nv` is the PASSIVE count (it sizes `y`); the voltages are one
        //: per CHANNEL per instant.  Indexed by `nv` every peak came back
        //: NaN — a table of NaN reads as "no limit data", which is a
        //: different and much quieter failure than an empty column.
        yv[k] = pf.v ? pf.v[k * pf.nch + c] : 0;
      }
      si.push({ x: pf.t, y: yi, color: FyPlot.seriesColor(col, c),
                label: 'ch ' + (c + 1) });
      sv.push({ x: pf.t, y: yv, color: FyPlot.seriesColor(col, c),
                label: 'ch ' + (c + 1) });
    }
    FyPlot.xy($('icur'), { series: si, xlabel: T('p.axis.t'),
                           ylabel: 'kA·turn', bands: phaseBands(),
                           marker: tNow() });
    FyPlot.xy($('volt'), { series: sv, xlabel: T('p.axis.t'),
                           ylabel: 'V/turn', bands: phaseBands(),
                           marker: tNow() });
    $('pf-note').textContent = T('p.pf.note');
  }
  ...
  function peaks() {
    var out = [];
    if (!pf) return out;
    for (var c = 0; c < pf.nch; c++) {
      var im = 0, vm = 0;
      for (var k = 0; k < pf.t.length; k++) {
        im = Math.max(im, Math.abs(pf.x[k * pf.nch + c]));
        if (pf.v) vm = Math.max(vm, Math.abs(pf.v[k * pf.nch + c]));
      }
      out.push({ i: im * 1e-3, v: vm });
    }
    return out;
  }

  function channelsOverCap() {
    var cap = capOf();
    if (cap === null) return [];
    return peaks().map(function (p, c) { return p.v > cap ? c : -1; })
                  .filter(function (c) { return c >= 0; });
  }

  function drawChannels() {
    var cap = capOf(), rows = peaks().map(function (p, c) {
      var over = cap !== null && p.v > cap;
      return '<tr><td>' + (c + 1) + '</td><td>' + f(p.i, 2) + '</td><td>'
        + f(p.v, 2) + '</td><td>' + (cap === null ? T('p.cap.none') : f(cap, 0))
        + '</td><td class="' + (over ? 'bad' : (cap === null ? 'unk' : ''))
        + '">' + (cap === null ? T('p.mark.undeclared')
                               : (over ? T('p.mark.over') : T('p.mark.ok')))
        + '</td></tr>';
    });
    $('channels').innerHTML = rows.join('');
    //: ★where the limit came from, or that there is none: the device
    //: descriptor carries no per-supply ceiling, so a number here is the
    //: reader's own — the page says that rather than implying a machine datum
    $('limits-src').textContent =
      cap === null ? T('p.limits.none') : T('p.limits.yours', { v: f(cap, 0) });
  }

  function drawChecks() {
    if (!pf || !pf.checks) { $('checks').innerHTML = ''; return; }
    var rows = pf.checks.map(function (c) {
      if (c.error)
        return '<tr><td>' + f(c.t, 2) + '</td><td colspan="4" class="bad">'
          + c.error + '</td></tr>';
      var tg = c.target || {}, sh = c.shape || {};
      var err = (sh.a !== undefined && tg.a)
        ? Math.abs(sh.a - tg.a) : NaN;
      return '<tr><td>' + f(c.t, 2) + '</td><td>'
        + f(tg.r0, 3) + ', ' + f(tg.a, 3) + ', ' + f(tg.kappa, 2) + '</td><td>'
        + f(sh.r0, 3) + ', ' + f(sh.a, 3) + ', ' + f(sh.kappa, 2) + '</td><td>'
        + (c.bndKind || '—') + '</td><td>'
        + (isFinite(err) ? f(err * 1e3, 1) + ' mm' : '—') + '</td></tr>';
    });
  ...
  function syncAll() { S.sync(); rateNote(); }

  ['ip', 'a', 'kappa', 'du', 'dl', 'a0', 'a1', 'paux', 'ne', 'te',
   'npts', 'nverify'].forEach(function (id) {
    var e = $(id);
    if (e) e.addEventListener('input', syncAll);
  });
  ['t_bd', 't_ru', 't_ft', 't_end', 'ip'].forEach(function (id) {
    var e = $(id);
    if (e) e.addEventListener('input', rateNote);
  });
  $('slice').addEventListener('input', function () {
    if (last) drawSlice();
  });
  ...
  S.onRefresh(function () {
    syncAll();
    if (last) { drawCorridor(); drawSlice(); }
    if (pf) drawPF();
  });

  /**
   * The trajectory this run designed — instants, targets, which of them were
   * verified, and what came back.  ★Published rather than left to be read off
   * the tables: a gate that parsed the rendered rows would be checking the
   * page's prose, not its answer.
   */
  S.publish(function () {
    if (!grid || !pf) return null;
    return { t: grid.t.slice(), phases: grid.ph.slice(),
             verify: grid.verify.slice(),
             checks: (pf.checks || []).map(function (c) {
               return { t: c.t, error: c.error || null,
                        target: c.target || null, shape: c.shape || null };
             }),
             nch: pf.nch, peaks: peaks() };
  });
...
// has not been computed, and what the reader does with the sliders decides it.
//
//   合开关     the discharge starts: Ip walks toward its target at the ramp
//              rate that was declared, and the phase becomes 上升 → 平顶
//   断开关     it enters a CONTROLLED ramp-down — not a stop, not a
//              disruption; the phase becomes 下降 and then 结束
//   不操作     inputs stay put, the march runs on, and the state settles:
//              dW/dt = P_heat − W/τ_E has a fixed point.  The page says
//              「定态」 when it is there — and says beside it how long the
//              MACHINE could hold it, which is a flux question and not a
//              transport one (D-17)
//
// ★The phases are RESULTS here, not inputs (D-16).  The pulse page reads the
// kernel's four-phase trapezoid; this page has no timetable to read — it
// records when each phase began, because that is the only place that answer
// exists.
//
// ★WHAT A SLIDER MEANS IS DIFFERENT FROM THE OTHER PAGE (D-12).  There it
// edits the whole waveform and everything is recomputed.  Here it edits the
// FUTURE: the history array already written is never touched, so what the
// corridor shows before the change is what actually happened under the old
// setting.  That is the whole content of "interactive" and it is one line of
// code — `H.paux.push(now())` rather than a rebuild — so it is worth being
// explicit that it is deliberate.
//
// ★NO CONTROLLER DYNAMICS.  「形状反馈」 re-designs the coil currents for the
// unchanged boundary at each equilibrium beat: it is the STEADY answer to
// "who holds this shape now", with no gain, no delay and no noise.  Those
// belong to the control scenario, and this page does not pretend to them.

var SIM = self.__FY_PD_PART;               //: §3 仿真模式

FyScenario.whenDevices(function () {
  'use strict';

  //: ★one 功能栏, declared: it owns the run key and unfolds the panels its
  //: run fills — see the same note on the pulse page.
  var S = SIM.bar('sim', {
    title: 'nav.sim',
    sliders: { dt: 2, rate: 0, eqevery: 0, ip: 0, a: 3, kappa: 2, du: 2, dl: 2,
               paux: 1, ne: 1, hfac: 2 },
    on: { ready: onReady, error: onError, zerod: onZerod, zerodb: onZerodB,
          zerodflux: onFlux, start: onStart, solve: onSolve },
  });
  var $ = S.$, T = FyI18n.t;
  var M = self.FYLITE_MACHINE, P = self.FyPhys;
  var NR = 41;
  //: the equilibrium's pressure parameter at the reference state.  ★It is
  //: scaled by the 0-D poloidal beta RATIO below, and that is an assumption,
  //: not a self-consistent coupling — the page says so where it shows it.
  var BETA0_REF = 0.55;
  ...
  var H = null;          // the history: only the past is in here
  var phase = 'idle';
  var began = {};        // when each phase began — the phases ARE this record
  var timer = null, ticks = 0, eqSolves = 0;
  var pendZ = false, pendEq = false;
  var ev = null;         // last `zerod` answer (traces + profiles + criteria)
  var pred = null;       // last `zerodb` answer (W_th, tau_E)
  var eq = null;         // last solved equilibrium
  var eqAt = null;       // the instant it was solved at
  var chan = null;       // the coil currents in force
  var fluxRow = null;
  var betaPRef = null;

  function reset() {
    stopClock();
    H = { t: [], ip: [], ipTarget: [], ne: [], paux: [] };
    phase = 'idle'; began = {}; ticks = 0; eqSolves = 0;
    pendZ = false; pendEq = false;
    ev = null; pred = null; eq = null; eqAt = null; chan = null;
    fluxRow = null; betaPRef = null;
    $('power').checked = false;
    paintPhase();
    paintRecord();
    drawAll();
    S.setBusy(false, T('s.idle'), '', 'idle');
  }

  // --- the clock ------------------------------------------------------------

  function dt() { return +$('dt').value; }
  function rate() { return +$('rate').value; }
  function eqEvery() { return +$('eqevery').value; }

  function clockNote() {
    $('clock-note').textContent = T('s.clock.note', {
      rt: f(dt() * rate(), 2), dt: f(dt(), 2), n: eqEvery(),
      eq: f(dt() * eqEvery(), 2) });
  }

  function startClock() {
    stopClock();
    timer = setInterval(tick, Math.max(50, 1000 / rate()));
  }
  ...
  // --- the phase machine ----------------------------------------------------
  //
  // ★Five states, and the switch is the only thing that moves between them
  // except the ramp rates.  A discharge that has ENDED cannot be resumed —
  // there is no plasma to resume — so the switch is refused and the reader is
  // told to reset, which is a different run and gets a different record.

  function targetIp() { return +$('ip').value * 1e3; }
  function rampUp() { return +$('rup').value * 1e6; }     // A/s
  function rampDown() { return +$('rdn').value * 1e6; }

  function onPower() {
    if ($('power').checked) {
      if (phase === 'end') {
        $('power').checked = false;
        S.report(T('s.ended'), 'warn');
        return;
      }
      if (phase === 'idle' || phase === 'down') phase = 'up';
      if (began.up === undefined) began.up = now();
      S.setBusy(true, T('s.running', { t: f(now(), 2) }));
      startClock();
    } else if (phase === 'up' || phase === 'flat') {
      phase = 'down';
      began.down = now();
    }
    paintPhase();
  ...
  function now() { return H.t.length ? H.t[H.t.length - 1] : 0; }
  function ipNow() { return H.ip.length ? H.ip[H.ip.length - 1] : 0; }

  function paintPhase() {
    var bar = $('phasebar');
    if (bar) Array.prototype.forEach.call(bar.children, function (e) {
      e.className = e.getAttribute('data-phase') === phase ? 'on' : '';
    });
    $('phase-note').textContent = T('s.phase.note.' + phase, {
      t: f(now(), 2),
      up: began.up === undefined ? '—' : f(began.up, 2),
      flat: began.flat === undefined ? '—' : f(began.flat, 2) });
  }

  /** One step of discharge time.  The past is appended to, never rewritten. */
  function tick() {
    var d = dt(), ip = ipNow(), tgt = targetIp();
    if (phase === 'up') {
      ip = Math.min(tgt, ip + rampUp() * d);
      if (ip >= tgt - 1e-9) { phase = 'flat'; began.flat = now() + d; }
    } else if (phase === 'flat') {
      //: the target can move under the flat top — the slider is live — and it
      //: is followed at the SAME declared rate, not jumped to
      var step = (tgt > ip ? rampUp() : rampDown()) * d;
      ip = tgt > ip ? Math.min(tgt, ip + step) : Math.max(tgt, ip - step);
    } else if (phase === 'down') {
      ip = Math.max(0, ip - rampDown() * d);
      if (ip <= 1e-9) {
        phase = 'end'; began.end = now() + d;
      }
    } else { return; }

    var frac = tgt > 0 ? clamp(ip / tgt, 0, 1) : 0;
    H.t.push(now() + d);
    H.ip.push(ip);
    H.ipTarget.push(tgt);
    //: density follows the current fraction — a plasma that is not there yet
    //: has no density, and the reader's slider is the FLAT-TOP value
    H.ne.push(+$('ne').value * 1e19 * Math.max(frac, 0.02));
    //: ★the drive is read AT THIS INSTANT: that is what makes a slider an
    //: intervention in the future rather than an edit of the past
    H.paux.push(ip > 0 ? +$('paux').value * 1e6 : 0);
    ticks++;

    if (phase === 'end') {
      stopClock();
      $('power').checked = false;
      S.setBusy(false, T('s.finished', { t: f(now(), 2), n: ticks }), '', 'done');
    } else {
      S.setBusy(true, T('s.running', { t: f(now(), 2) }));
    }
    paintPhase();
    askZerod();
    if (ticks % Math.max(1, eqEvery()) === 0) askEquilibrium();
    drawAll();
    paintRecord();
  }

  // --- what the kernel is asked, and how often ------------------------------
  //
  // ★A REQUEST IS NOT SENT WHILE ITS ANSWER IS OUTSTANDING.  The 0-D pass
  // costs milliseconds and the free-boundary solve costs about a second; a
  // clock that queued one of each per tick would build a backlog the reader
  // sees as a page that lags further behind the longer it runs.  So each has
  // a latch, and a beat that finds it closed simply skips — the next beat is
  // 200 ms away.

  function par() {
    return new Float64Array([0.9, 1.0, 1.5, 0.05, +$('r0').value,
                             +$('a').value, +$('kappa').value,
                             1.8, 0.9, 0.5]);
  }

  function askZerod() {
    if (pendZ || H.t.length < 3) return;
    pendZ = true;
    var rho = linspace(0, 1, NR);
    var te0 = new Float64Array(H.t.length);
    //: the prescribed tier needs a central temperature; it is taken from the
    //: predicted one when there is a prediction and from a cold guess before
    for (var i = 0; i < H.t.length; i++)
      te0[i] = pred && pred.te0 && pred.te0[i] ? Math.max(pred.te0[i], 0.05)
                                               : 0.3;
    S.send({ cmd: 'zerod', t: H.t, ip: H.ip, ne0: H.ne, te0: Array.from(te0),
             pInj: H.paux, rho: Array.from(rho), par: Array.from(par()),
             geom: { r0: +$('r0').value, a: +$('a').value,
                     kappa: +$('kappa').value },
             phases: phasesSoFar(), li: 0.9, phiAvail: 0 });
    S.send({ cmd: 'zerodb', t: H.t, ip: H.ip, ne0: H.ne, pAux: H.paux,
             rho: Array.from(rho), par: Array.from(par()),
             pred: [0, +$('hfac').value, 2.5,
                    Math.abs(self.FyDevice.tf(M).b0), 0] });
  }

  /**
   * The four phase times AS RECORDED.
   *
   * ★This is D-16 in one function: the flux account and every phase label
   * need four instants, and on this page they are not given — they are when
   * the switch was thrown and when the ramp got there.  A phase that has not
   * happened yet is reported as the present, which is what "so far" means.
   */
  function phasesSoFar() {
    var t0 = H.t.length ? H.t[0] : 0, tn = now();
    return [t0,
            began.flat === undefined ? tn : began.flat,
            began.down === undefined ? tn : began.down,
            tn];
  }

  function askEquilibrium() {
    //: ★A DYING CURRENT HAS NO SHAPE.  Asked at 20 kA out of 400 the solver
    //: still returns something — a collapsed column with q_cyl above 20 —
    //: and drawing it beside the word 「解出的 LCFS」 would report the tail of
    //: a ramp-down as a configuration.  Below a twentieth of the target the
    //: page keeps the last real boundary and says how old it is.
    if (!M || pendEq || ipNow() <= 0.05 * targetIp()) return;
    pendEq = true;
    if ($('fb').checked || !chan) {
      //: ★形状反馈：the currents are re-designed for the SAME boundary at the
  ...
  function readTarget() {
    return { r0: +$('r0').value, z0: 0, a: +$('a').value,
             kappa: +$('kappa').value,
             deltaU: +$('du').value, deltaL: +$('dl').value };
  }

  /** The pressure parameter this solve is given, and where it came from. */
  function beta0Now() {
    var bp = criterion('betaP');
    if (bp === null) return BETA0_REF;
    if (betaPRef === null || !(betaPRef > 0)) betaPRef = bp;
    return BETA0_REF * clamp(bp / betaPRef, 0.2, 5);
  }

  function sendSolve() {
    if (!chan) { pendEq = false; return; }
    S.send({ cmd: 'solve', chan: Array.from(chan), ip: ipNow(),
             prof: { beta0: beta0Now(), emp: 1.0, enp: 1.0,
                     r0: +$('r0').value } });
  }
  ...
  function onReady() {
    //: ★THE HANDSHAKE MUST NOT UNDO A RUN.  It arrives a moment after the
    //: page is already usable — and when the worker is rebuilt (a stop, or a
    //: device change) it arrives again.  Resetting here threw away a
    //: discharge the reader had already started, silently, and the switch
    //: came back up on its own.  The kernel is stateless between calls apart
    //: from `init`, which the framework replays, so a run in flight simply
    //: carries on; only the latches are cleared.
    pendZ = false; pendEq = false;
    if (phase === 'idle' && ticks === 0) { reset(); return; }
    S.setBusy(!!timer, timer ? T('s.running', { t: f(now(), 2) })
                             : T('s.paused', { t: f(now(), 2) }));
  }

  function onError(m) {
    pendZ = false; pendEq = false;
    S.report((m.where ? m.where + ': ' : '') + m.message, 'err');
  }

  function onZerod(m) {
    pendZ = false;
    ev = { out: m.result, limits: m.limits, nr: m.nr, nt: m.nt };
    askFlux();
    drawAll();
  }
  ...
  /**
   * The flux account, over the part of the run it is an account OF.
   *
   * ★★IT STOPS WHERE THE RAMP-DOWN BEGINS, and that is not tidiness.  The
   * budget integrates the loop voltage, and V_loop = Ip·Rp + L dIp/dt has no
   * useful value as the current goes to zero — carried through the ramp-down
   * it came back as 18 000 Wb, a number that is not wrong by a factor but
   * meaningless, and a page that prints it teaches the reader to skip the
   * row.  What is being asked here is 「平顶还能撑多久」, which is a question
   * about the flat top.
   */
  function askFlux() {
    if (!ev || H.t.length < 3) return;
    var stop = began.down === undefined ? now() : began.down;
    var t = [], v = [], ip = [];
    for (var i = 0; i < H.t.length; i++) {
      if (H.t[i] > stop + 1e-12 || !(H.ip[i] > 0)) continue;
      t.push(H.t[i]); v.push(ev.out.vLoop[i]); ip.push(H.ip[i]);
    }
    if (t.length < 3) { fluxRow = null; drawFlux(); return; }
    S.send({ cmd: 'zerodflux', t: t, vLoop: v, ip: ip,
             phases: [t[0],
                      began.flat === undefined ? t[t.length - 1] : began.flat,
                      t[t.length - 1], t[t.length - 1]],
             r0: +$('r0').value, a: +$('a').value, li: 0.9,
             phiAvail: +$('phiavail').value });
  }

  // --- reading the answers --------------------------------------------------

  function criterion(name) {
    if (!ev || !ev.limits || !ev.limits[name]) return null;
    var v = ev.limits[name];
    var x = v[v.length - 1];
    return isFinite(x) ? x : null;
  }

  /** Is the state settled?  ★A threshold, not an impression. */
  function steady() {
    if (!pred || !pred.wTh || pred.wTh.length < 12) return null;
    var w = pred.wTh, n = w.length;
    var a = w[n - 1], b = w[n - 11];
    if (!(a > 0)) return null;
    var rel = Math.abs(a - b) / a;
    return { rel: rel, on: rel < 2e-3 };
  }

  // --- drawing --------------------------------------------------------------

  function drawAll() {
    drawCorridor(); drawSnap(); drawProfiles(); drawFlux(); paintEqNote();
  }

  /**
   * How old the boundary on screen is.
   *
   * ★It is repainted every beat although the FIGURE is not: between two real
   * solves the picture is the same picture, and what changes is how far
   * behind it has fallen — which is exactly what the reader needs to know
   * (D-13).  A note frozen with the figure would keep saying 「上次真解
   * t = 3.80 s（现在 3.80 s）」 through a second of discharge time.
   */
  function paintEqNote() {
    $('eq-note').className = 'note' + (eq ? '' : ' caveat');
    $('eq-note').textContent = eq
      ? T('s.eq.solved', { at: f(eqAt, 2), t: f(now(), 2), n: eqSolves,
                           fb: $('fb').checked ? T('s.fb.on') : T('s.fb.off') })
      : T('s.eq.none');
  }

  function window0() {
    var tn = now();
    return { t0: Math.max(H.t.length ? H.t[0] : 0, tn - 4), t1: tn + 0.35 };
  }

  function trace(canvas, series, ylabel) {
    var w = window0();
    FyPlot.xy($(canvas), { series: series, xlabel: T('s.axis.t'),
                           ylabel: ylabel, xmin: w.t0, xmax: w.t1,
                           marker: now() });
  }

  function arr(a) { return a && a.length ? a : [0]; }
  ...
  function drawCorridor() {
    ...
    var t = arr(H.t);
    trace('tr-ip', [
      { x: t, y: H.ipTarget.map(function (v) { return v * 1e-3; }),
        color: col.muted, dash: [5, 3], label: T('s.leg.target') },
      { x: t, y: H.ip.map(function (v) { return v * 1e-3; }),
        color: col.accent, label: T('s.leg.got') },
    ], 'kA');
    trace('tr-w', pred && pred.wTh ? [
      { x: t, y: Array.prototype.map.call(pred.wTh, function (v) { return v * 1e-6; }),
        color: col.accent, label: 'W_th [MJ]' },
      { x: t, y: Array.prototype.map.call(pred.tauE || [], function (v) { return v; }),
        color: col.alt, label: 'τ_E [s]' },
    ] : [], 'MJ · s');
    trace('tr-p', [
      { x: t, y: H.paux.map(function (v) { return v * 1e-6; }),
        color: col.lcfs, label: 'P_aux [MW]' },
      { x: t, y: pred && pred.pOhm
          ? Array.prototype.map.call(pred.pOhm, function (v) { return v * 1e-6; })
          : t.map(function () { return 0; }),
        color: col.accent, label: 'P_ohm [MW]' },
    ], 'MW');
    drawCoilTrace(col);
  }

  //: the coil currents as they were designed, beat by beat.  ★They are a
  //: STAIRCASE and drawn as one: between two equilibrium beats nothing was
  //: solved, and a smooth line through them would claim a continuity the page
  //: does not have.
  var coilHist = { t: [], x: [] };

  function drawCoilTrace(col) {
    if (chan && (!coilHist.t.length ||
                 coilHist.t[coilHist.t.length - 1] !== eqAt)) {
      if (eqAt !== null) {
        coilHist.t.push(eqAt);
        coilHist.x.push(Float64Array.from(chan));
      }
    }
    if (!coilHist.t.length) { FyPlot.xy($('tr-coil'), { series: [] }); return; }
    var nch = coilHist.x[0].length, series = [];
    for (var c = 0; c < Math.min(nch, 8); c++) {
      var y = coilHist.x.map(function (v) { return v[c] * 1e-3; });
      //: ★DOTS, not a line.  Each point is one beat at which the currents
      //: were actually designed; joining them would draw a continuity
      //: between two beats that nothing computed.
      series.push({ x: coilHist.t, y: y, color: FyPlot.seriesColor(col, c),
                    kind: 'dots', radius: 2.5, label: 'ch ' + (c + 1) });
    }
    trace('tr-coil', series, 'kA·turn');
  }
  ...
  function drawCross() {
    ...
    var col = FyPlot.palette($('cross'));
    $('cross-legend').innerHTML = FyPlot.legendHTML([
      { label: T('s.leg.tshape'), color: col.accent, kind: 'line', dash: [4, 4] },
      { label: T('s.leg.gshape'), color: col.lcfs, kind: 'line' },
    ]);
    $('cross-cap').textContent = eq
      ? T('s.cross.cap', { at: f(eqAt, 2), beta: f(beta0Now(), 3) })
      : T('s.cross.none');
    //: ★上一次真解 —— the sentence D-13 asks for, on the panel itself
    paintEqNote();
  }
  ...
  function targetFlat() {
    ...
    var tg = readTarget();
    var pts = P.millerBoundary(tg, 121), out = new Float64Array(pts.length * 2);
    for (var i = 0; i < pts.length; i++) {
      out[2 * i] = pts[i][0]; out[2 * i + 1] = pts[i][1];
    }
    return out;
  ...
  function drawProfiles() {
    if (!ev || !ev.out.ne || !H.t.length) return;
    //: ★the index is the ANSWER's last slice, not the history's: the march
    //: has moved on since this 0-D pass was asked for, and indexing a shorter
    //: array by the longer one's length silently drew nothing at all.
    var nr = NR, k = Math.max(0, (ev.nt || 1) - 1), rho = linspace(0, 1, NR);
    var col = FyPlot.palette($('pr-ne'));
    var ne = ev.out.ne.slice(k * nr, (k + 1) * nr);
    var te = ev.out.te.slice(k * nr, (k + 1) * nr);
    var ti = ev.out.ti.slice(k * nr, (k + 1) * nr);
    if (!ne.length) return;
    FyPlot.xy($('pr-ne'), { series: [{ x: rho, y: Array.prototype.map.call(ne,
      function (v) { return v * 1e-19; }), color: col.accent, label: 'n_e' }],
      xlabel: 'ρ', ylabel: '10¹⁹ m⁻³' });
    FyPlot.xy($('pr-t'), { series: [
      { x: rho, y: te, color: col.accent, label: 'T_e' },
      { x: rho, y: ti, color: col.lcfs, label: 'T_i' },
  ...
  function row(k, v, cls) {
    return '<tr><td>' + k + '</td><td class="' + (cls || '') + '">' + v
      + '</td></tr>';
  }

  function drawSnap() {
    var st = steady(), rows = [];
    rows.push(row(T('s.snap.t'), f(now(), 2) + ' s'));
    rows.push(row(T('s.snap.ip'), f(ipNow() * 1e-3, 1) + ' kA'));
    if (pred && pred.wTh && pred.wTh.length)
      rows.push(row(T('s.snap.w'),
                    f(pred.wTh[pred.wTh.length - 1] * 1e-6, 3) + ' MJ'));
    if (pred && pred.tauE && pred.tauE.length)
      rows.push(row(T('s.snap.tau'),
                    f(pred.tauE[pred.tauE.length - 1], 3) + ' s'));
    var bp = criterion('betaP'), q = criterion('qCyl'), fg = criterion('fGw');
    if (bp !== null) rows.push(row('β_p', f(bp, 3)));
    if (q !== null) rows.push(row('q_cyl', f(q, 2)));
    if (fg !== null) rows.push(row('f_GW', f(fg, 3),
                                   fg > 1 ? 'warn' : ''));
    if (st) rows.push(row(T('s.snap.steady'),
                          st.on ? T('s.steady.yes', { rel: (st.rel * 100).toFixed(2) })
                                : T('s.steady.no', { rel: (st.rel * 100).toFixed(2) })));
    $('snap').innerHTML = rows.join('');
  }

  function drawFlux() {
    if (!fluxRow) { $('fluxtab').innerHTML = ''; return; }
    var b = fluxRow, rows = [], avail = +$('phiavail').value;
    var W = function (x) { return isFinite(x) ? f(x, 3) + ' Wb' : '—'; };
    rows.push(row(T('s.flux.ind'), W(b.phiInd)));
    rows.push(row(T('s.flux.res'), W(b.phiResRamp)));
    rows.push(row(T('s.flux.used'), W(b.phiConsumed)));
    rows.push(row(T('s.flux.vflat'), isFinite(b.vFlattop)
                  ? f(b.vFlattop, 3) + ' V' : '—'));
    //: ★UNDECLARED IS NOT ZERO, and it is not a duration either: with no
    //: swing given, how long the machine could hold this is 未知 — the
    //: kernel returns null for it and the page prints that word (D-17).
    if (avail > 0) {
      rows.push(row(T('s.flux.left'), W(avail - b.phiConsumed)));
      rows.push(row(T('s.flux.hold'), b.tSustain === null
                    ? T('s.flux.unknown') : f(b.tSustain, 1) + ' s', 'warn'));
    } else {
      rows.push(row(T('s.flux.hold'), T('s.flux.unknown'), 'warn'));
    }
    $('fluxtab').innerHTML = rows.join('');
  }

  function paintRecord() {
    $('record').innerHTML = [
      row(T('s.rec.steps'), String(ticks)),
      row(T('s.rec.dt'), f(dt(), 2) + ' s'),
      row(T('s.rec.eq'), String(eqSolves)),
      row(T('s.rec.phase'), T('s.ph.' + phase)),
      row(T('s.rec.kind'), T('s.rec.kind.run')),
    ].join('');
  }

  // --- wiring ---------------------------------------------------------------

  function syncAll() { S.sync(); clockNote(); }

  ['dt', 'rate', 'eqevery', 'ip', 'a', 'kappa', 'du', 'dl', 'paux', 'ne', 'hfac']
    .forEach(function (id) {
      var e = $(id);
      if (e) e.addEventListener('input', function () {
        syncAll();
        if (id === 'rate' && timer) startClock();
        if (id === 'a' || id === 'kappa' || id === 'du' || id === 'dl')
          drawCross();
      });
    });
  $('power').addEventListener('change', onPower);
  $('pause').addEventListener('click', function () {
    if (timer) { stopClock(); S.setBusy(false, T('s.paused', { t: f(now(), 2) }), 'warn', 'miss'); }
    else if (phase === 'up' || phase === 'flat' || phase === 'down') startClock();
  });
  $('step').addEventListener('click', function () {
    if (phase === 'idle' && $('power').checked === false) return;
    stopClock();
    tick();
  });
  ...
  $('phiavail').addEventListener('input', askFlux);

  S.onAbort(function () { stopClock(); });
  S.onRefresh(function () { syncAll(); drawAll(); drawCross(); });

  //: the page's run key starts the discharge — the same thing the switch
  //: does, because a page with a run button that does nothing is worse than
  //: one with two ways to do the same thing
  /**
   * What this run IS, for anything that has to read it without reading the
   * page's prose.
   *
   * ★A gate cannot check 「滑块改的是未来」 from a picture: the claim is about
   * two histories, the one before the change and the one after, and the only
   * honest way to compare them is to read the array itself.  Published for
   * the same reason the design page publishes its trajectory.
   */
  S.publish(function () {
    return { t: H.t.slice(), ip: H.ip.slice(), ipTarget: H.ipTarget.slice(),
             paux: H.paux.slice(), ne: H.ne.slice(),
             phase: phase, began: JSON.parse(JSON.stringify(began)),
             ticks: ticks, eqSolves: eqSolves, eqAt: eqAt,
             dt: dt(), running: !!timer };
  });

  S.onRun(function () {
    if (!$('power').checked) { $('power').checked = true; onPower(); }
    return Promise.resolve();
  });
