...
def reduce_est2(get, shot: int, time_s: float, *,
    ...
    #: ★2026-09-13: the main-tree Ip node is optional — a document built from the
    #: A-Box names none, because the A-Box binds Ip only as `pcs_east:\PCRL01`
    #: (the read below).  Without a main-tree node that binding is the only read.
    try:
        ip_node = _dev().MDS_IP
    except _dev().MachineDataMissing:
        ip_node = None
    plasma = (avg(ip_node, "east", 1000.0, required=False, drift=False)
    ...
    if plasma is None:
        raise error(f"no plasma current for shot {shot}: "
                    + (f"both {ip_node} and " if ip_node else
...
def read_east_mds(shot: int, time_s: float, *,
    ...
    * ``\\ipm`` (kA) -> ``PLASMA`` [A]; ``\\focs_it`` -> ``BTOR``.
    ...
    (``server=`` → ``$FYLITE_MDSIP_SERVER`` → the device document).  Reads through the engine's mdsip client.
    """
    import numpy as np
    from .. import kernel
