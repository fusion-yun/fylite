...
〔2026-09-12〕**本组空了**：F-16 · F-34 · H-10（会话文档那一半）· F-1（门）· H-15（1.4 与 1.2 的算法）当日做完并推送，各自的落地读数在 `PLAN.md` 同号行。余下的每一条都在**等一句裁定**或**缺仓外材料** —— 见下两组。★**裁定的杠杆最大的是 `H-19`**（p′/FF′ 的规范）：它一句话解开 F-1 余下的十条判据、`code/discharge` 的一维剖面、以及两步算例那条链。

## 二、待裁（15 行 · 16 个号，等用户一句口径）

| 号 | 归属 | 要裁的是什么 |
| :--- | :--- | :--- |
| **H-15**（1.2 余下一半） | fylite · fyo | **Dα 触发的输入端口**：本分发里没有任何 Dα 节点名（EAST 绑定表无 filterscope 通道）。相位选择的算法与闸已在 `io/est2.py`；要「进得了计划的一步」得先定这个槽由 fyo 铸、还是本仓带前缀自有（同 O-1 / O-2 那条缝）|
