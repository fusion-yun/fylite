...
| 号 | 归属 | 下一步做什么 |
| :--- | :--- | :--- |
| **H-15** | fylite | **1.4 已关**（`code/separatrix_align` + 八条闸，`FYL-DESIGN-21` 表 1.4 改 ◐）。**下一步**：1.2 ELM 相位条件平均落在 `python/fylite/io/est2.py`（窗口均值就在那里），「进得了计划的一步」那一半另需 Dα 输入端口；2.3 MSE 的 Er 修正**阻塞**于卷宗无 MSE 几何 |
