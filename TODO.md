...
| **I-2** | kernel | 本机原生量耗时：10 ms 外步 · `neutrals_mc` · 沉积重算；`code/discharge` 冷 / 热启一项等 I-5 的 CFEDR 盒 |
| **I-5** | fydoc · kernel · fylite | CFEDR 装置卡由 fydoc 重导并补解算盒与限制器；`FYDOC-REPORT-20` 爬升节点落成指令回放文件（盒的最终落点随 O-5 / K-2 第二批） |
...
| **I-3** | kernel | CFEDR 步进会话走**新门**（`CoreMarch` + 内核装配闭包，推荐），还是解开 `code/evolve` 对 species / 表格沉积的拒绝（会移动页面已录答案）—— I-4 · I-6..I-10 都等它 |
| **I-12** | fylite · kernel | CFEDR 无 NBI / LHW 硬件：非零指令按名拒绝（推荐），还是用通用缺省硬件并标注「非 CFEDR 设计」 |
