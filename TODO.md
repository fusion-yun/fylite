...
| 号 | 归属 | 要裁的是什么 |
| :--- | :--- | :--- |
| **K-2** | kernel · fyo · fydoc | **2026-09-13 已裁「进文档」**，余下是**逐条落点**待确认：十六条里哪几条有 DD 家（供电上下限 `pf_active/supply/voltage_limit_max` · `current_limit_max`〔kA → A〕· `turns_with_sign`），哪几条要 fyo 新槽（解算盒四边与网格数 · IC 线圈 · 逐回路电阻率） |
...
| **F-28** | kernel | 〔2026-09-13 查全文〕那两篇是 Poli 2018（TORBEAM 2.0）与 Prater 2008（EC 基准）——**全文在库，但只有接受稿**；期刊版缺：`10.1016/j.cpc.2017.12.018` · `10.1088/0029-5515/48/3/035006`（IOP 标免费阅读，脚本下载被反爬拦下）。本条余下的是 B 类可复算参照件，不再卡在全文 |
| **F-30** | kernel | 〔2026-09-13 查全文〕三篇**都在库**：Karney & Fisch 1979 与 Fisch 1987 的**期刊版**当日由作者 PPPL 主页取回、已入库挂 Zotero（原有 PPPL 报告版保留）；Ignat 1994 只有报告版（PPPL-2929），期刊版 `10.1088/0029-5515/34/6/i07` 无合法开放件。LSC 源码按裁定不读 |
| **F-31** | kernel | 〔2026-09-13 查全文〕Krivenski–Orefice 1983 与 Shkarofsky 1966 的**期刊版都在库**（2026-09-11 已收）——本行「缺全文」已不成立，弱相对论极化一项现在可以按原文动手 |
