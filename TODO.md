...
| **F-7** | 须裁定 | **`B-09`：要不要自跑 DINA 当参考**。本体在本机且为 LGPL 3.0，但牵动①许可与清净室（本仓对「读物理源码」有既有裁定）②可行性未探（有 MATLAB 依赖）。**在探之前改判是不诚实的** | SURVEY-2026-09-08 §一 |
| **F-8** | 可发信 | **`C-08` 向作者索取 GENE 那批次的 ν* 与逐半径 Miller 参数**（或那次 JINTRAC 模拟的对应量）。这是六条阻塞里**唯一一条只差一封信**的 | plan.jsonld `C-08` |

★F-1 / F-2 / F-4 / F-5 / F-6 是**本仓能自己做完**的；F-3 是排期问题；F-7 要你裁定；
F-8 要对外联系。
