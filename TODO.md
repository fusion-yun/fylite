...
| 号 | 归属 | 下一步做什么 |
| :--- | :--- | :--- |
| **H-15** | fylite | **1.4 已关**（`code/separatrix_align` + 八条闸）· **1.2 的算法已关**（`est2.py` 三个纯函数 + `reduce_est2` 的 `elm=` 开关 + 十条闸）。**余下**：1.2 要「进得了计划的一步」得先有 Dα 输入端口（槽治理，O 序，**待裁**）；2.3 MSE 的 Er 修正**阻塞**于卷宗无 MSE 几何 |
