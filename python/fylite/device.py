...
``rv6565.ddd``               vessel → loop/probe/grid response tables —
                             **cross-check only** (:func:`vessel_table_check`)
...
__all__ = ["MachineDataMissing", "DEVICE_ENV", "configured", "data_dir",
           ...
           # 3. what the conductors do (was `fylite.circuits`)
           "element_filaments", "mutual_filaments", "mutual_matrix",
           "resistance_vector", "grid_response", "point_response",
           "channel_response", "vessel_table_check", "ellipke",
           "probe_element_response", "element_flux", "filament_flux",
           "psi_from_channels", "channel_matrices",
           "step_circuits", "evolve_circuits_voltage",
           # 4. what the sight lines do (was `fylite.chords`)
           "ChordError", "Chord", "psin_map", "psin_at", "integrate",
           "line_integral"]
...
def coil_response_tables(path=None) -> dict:
    ...
    :func:`vessel_table_check` does for the vessel: asking whether a
...
#:   :func:`vessel_table_check`, run in ``tests/test_circuits.py``.
#:
...
# --------------------------------------------------------------------------- #
# complete elliptic integrals, pure numpy (AGM)                               #
# --------------------------------------------------------------------------- #
def ellipke(m):
    """K(m), E(m) — the kernel's (arithmetic-geometric mean).

    ★It used to take a ``tol`` it deleted on the first line, kept "so
    existing call sites keep working".  There were none, in this tree or the
    browser's — a parameter that no caller passes and the body ignores
    documents a choice the reader does not have.
    """
    return _kernel().ellipke(m)


def mutual_filaments(r1, z1, r2, z2):
    """Mutual inductance [H] between coaxial circular filaments (vectorized).

    M = MU0*sqrt(r1*r2) * ((2/k - k) K - (2/k) E),  k^2 = 4 r1 r2 / d^2,
    d^2 = (r1+r2)^2 + (z1-z2)^2.  Symmetric in (1<->2) by construction —
    the kernel's.

    ★Elementwise, on equal-length arrays.  A caller who wants every pair of
    two SETS wants :func:`fylite.kernel.mutual_outer`, which answers the
    ``(n_a, n_b)`` block without materialising either broadcast — this entry
    used to sniff for the ``r[:, None]`` / ``r[None, :]`` shape and forward
    to it, which meant the cheap path was reached by writing the expensive
    spelling and hoping the sniff fired.  It is asked for by name now.
    """
    return _kernel().mutual_filaments(*(np.asarray(x, float)
...
# conductor elements                                                          #
# --------------------------------------------------------------------------- #
def element_filaments(e, nu: int = 4, nv: int = 4):
    """Subdivide an element into nu x nv filaments -> (r[], z[]) arrays —
    the kernel's.  ``e`` is a :class:`fylite.device.Element`.

    ★The annotation used to say ``e: Element`` — a name that left this module
    with the deck readers.  ``from __future__ import annotations`` keeps an
    undefined annotation from ever being evaluated, so a dangling type name
    is invisible until something introspects the signature.
    """
    return _kernel().element_filaments(e.r, e.z, e.w, e.h, e.a, e.a2,
                                       nu=nu, nv=nv)
...
# matrices                                                                    #
# --------------------------------------------------------------------------- #
def mutual_matrix(elems_a, elems_b=None, *, turns_a=None, turns_b=None,
                  nu: int = 4, nv: int = 4):
    """Mutual-inductance matrix [H] between element lists — the kernel's.

    With ``elems_b is None`` the self-set matrix is returned (symmetric,
    diagonal by filament-averaged self inductance).  ``turns_*`` scale
    rows/columns into TURN space (``M_ij -> N_i N_j M_ij``) — ★that scaling
    is the kernel's too: a turn count enters an inductance once per SIDE,
    and getting it wrong yields a plausible L/R time that is wrong by
    exactly the winding.

    ★The DIAGONAL is the part worth naming: a filament's mutual with itself
    is singular, so the self term is the thin-filament inductance of one
    sub-filament of equivalent circular cross-section.  That substitution
    is physics and it lives in the kernel with the rest of it.
    """
    same = elems_b is None
    return _kernel().mutual_matrix(
        element_arrays(elems_a),
        None if same else element_arrays(elems_b), nu=nu, nv=nv,
        turns_a=turns_a, turns_b=turns_b)


def resistance_vector(elems, eta_uohm_m, turns=None):
    """R [Ohm] per element: eta * 2*pi*r / area, x N^2 when turns given.

    ``eta_uohm_m`` is scalar or per-element, in micro-Ohm-metres (the unit
    the deck quotes).  Verified: vessel element 1 -> 9.5189e-3 Ohm from the
    deck geometry.
    """
    #: the deck quotes micro-ohm-metres; the conversion belongs here, where
    #: the deck is read, and the formula belongs in the kernel
    eta = np.broadcast_to(np.asarray(eta_uohm_m, float) * 1e-6, (len(elems),))
    return _kernel().resistances([e.r for e in elems], [e.area for e in elems],
                              eta, turns)


# --------------------------------------------------------------------------- #
# grid response + the two-path acceptance                                     #
# --------------------------------------------------------------------------- #
def grid_response(elems, grid_r, grid_z, *, nu: int = 4, nv: int = 4):
    """psi response [Wb/A] of each element on grid nodes -> (nelem, nr, nz).

    The grid node is treated as a filament loop at (R, Z); the response is
    the filament-averaged mutual inductance — i.e. poloidal flux through
    the circle of radius R at height Z per ampere in the element.  The
    kernel's, and the same mutual `point_response` uses, so the gridded and
    the scattered answer cannot drift apart.
    """
    return _kernel().grid_response(element_arrays(elems), grid_r,
                                   grid_z, nu=nu, nv=nv)


def element_flux(elems, amps, grid_r, grid_z, *, nu: int = 4, nv: int = 4):
    """The psi [Wb] a conductor set PRODUCES on the grid, from its
    ampere-turns — the kernel's.

    ★Not :func:`grid_response` followed by a contraction.  That was how
    every caller here used to spell it, and it builds an
    ``(nelem, nr, nz)`` tensor in order to collapse it on the next line —
    a grid's worth of memory per conductor for a quantity that is one
    accumulator per node.  It also left the fold at the call site, which
    is where a per-radian/per-turn convention gets applied twice.
    """
    return _kernel().element_flux(element_arrays(elems), amps, grid_r,
                                  grid_z, nu=nu, nv=nv)


def filament_flux(fil_r, fil_z, amps, grid_r, grid_z):
    """The psi [Wb] a cloud of current FILAMENTS produces on the grid — the
    element-free twin of :func:`element_flux`, for a current distribution
    that is already a point cloud (a plasma discretised into filaments).
    """
    return _kernel().filament_flux(fil_r, fil_z, amps, grid_r, grid_z)


def psi_from_channels(conductors, grid_r, grid_z, aturns, vessel_amps=None,
                      *, nu: int = 4, nv: int = 4):
    """External full-flux field [Wb] on the grid from BRSP channel
    ampere-turns, plus optional per-segment vessel amperes.

    ``conductors`` is a :func:`fylite.device.conductor_set` mapping — the
    machine already resolved.  The deck-geometry Green construction that
    every line's forward solve starts from (it matches the Fortran vacuum to
    ~2% RMS once the plasma flux is accounted for).  ★It used to live in
    ``rusteq``, one import away from three scenario lines, next to three thin
    re-wrappings of kernel entries.  It is a question about the CONDUCTORS
    and the channel map, and both halves it is made of — the fold and
    :func:`element_flux` — meet here.
    """
    #: the channel-to-element fold is a RELABELLING (which deck element a
    #: supply drives, and in what split) and it belongs on this side; what
    #: the elements then PRODUCE on the grid is the kernel's
    el_at = _kernel().channel_fold(conductors["weights"], aturns)
    psi = element_flux(conductors["coils"], el_at, grid_r, grid_z,
                       nu=nu, nv=nv)
    if vessel_amps is not None:
        psi = psi + element_flux(conductors["vessel"],
                                 np.asarray(vessel_amps, float),
                                 grid_r, grid_z, nu=nu, nv=nv)
    return psi


def point_response(elems, pr, pz, *, nu: int = 3, nv: int = 3):
    """Per-element ``(psi, Br, Bz)`` at scattered points — the scattered-point
    twin of :func:`grid_response`.
...
def vessel_table_check(vessel, table, grid_r, grid_z, *, segments=None,
                       nu: int = 6, nv: int = 6) -> dict:
    """The P0 two-path acceptance: how the vessel->grid psi response
    recomputed from GEOMETRY compares with a device's own Green table.

    ``vessel`` is the element list, ``table`` the ``(n_seg, nr, nz)``
    response the machine ships (``rv6565.ddd``'s ``gridvs``).  Returns the
    per-segment median ratio (table/computed) and the overall spread.  Grid
    nodes inside or touching an element are excluded — the table's own
    filamentisation differs there by construction.

    ★Both inputs are passed in.  This check exists to ask whether two
    DESCRIPTIONS of one machine agree, so it must be given both of them
    rather than resolve either: reading the geometry itself would let it
    compare a document's conductors against a deck's table and report the
    disagreement as a numerical one.
    """
    segs = list(range(len(vessel))) if segments is None else list(segments)
    elems = [vessel[i] for i in segs]
    mine = grid_response(elems, grid_r, grid_z, nu=nu, nv=nv)
    #: ★the statistic is the kernel's, near-field exclusion included.  What
    #: it drops and what it averages are numerical choices with a stated
    #: reason (a node beside its own conductor measures filamentisation,
    #: not agreement; a mean is moved past tolerance by one such node), and
    #: this face is not where a second version of them belongs.
    out = _kernel().table_ratio_check(np.asarray(table)[segs], mine,
                                      grid_r, grid_z,
                                      element_arrays(elems))
    return {"segments": segs, **out}


def channel_matrices(conductors, *, eta_coil_uohm_m: float,
                     eta_vessel_uohm_m=None, passive=None, nu: int = 4,
                     nv: int = 4) -> dict:
    """Per-ampere-turn circuit matrices in the measured channel space.

    ``conductors`` is a :func:`fylite.device.conductor_set` mapping.

    State convention: coil state x_c is the BRSP value [A-turn]; vessel
    state I_v is plain amperes.  In ampere-turn space the loop equation
    carries only per-turn geometric quantities — U/N_tot = M1 dx/dt + R1 x
    — so the (uncertain) name-to-turn table never enters:
    M_ch = W M1 W^T, R_ch = sum_j w_j^2 R1_j with the conductor set's own
    weights W.  ★That assembly is the KERNEL's: the weight enters the
    inductance once and the resistance SQUARED, and getting that wrong
    yields a plausible L/R time that is wrong by the split.

    ``passive`` replaces the deck vessel with ``(elements, eta_uohm_m)`` —
    what :func:`fylite.device.passive_set` returns — for a configuration
    carrying more passive structure than the Green tables have response
    columns for.  Without it the conductor set's own vessel is used and
    ``eta_vessel_uohm_m`` is required.

    ★This entry used to be one of THREE spellings of the same block
    assembly (the vertical-stability plant and the pulse designer each
    rebuilt it around their own passive set, in numpy, at nu=3).  They now
    ask for the passive set they want instead of assembling it again.
    """
    coils, W = conductors["coils"], conductors["weights"]
    if passive is None:
        if eta_vessel_uohm_m is None:
            raise ValueError(
                "channel_matrices needs eta_vessel_uohm_m for the machine's "
                "own vessel, or a `passive=(elements, eta_uohm_m)` set")
        pas_el = conductors["vessel"]
        pas_eta = np.broadcast_to(np.asarray(eta_vessel_uohm_m, float),
                                  (len(pas_el),))
    else:
        pas_el, pas_eta = passive
        pas_eta = np.broadcast_to(np.asarray(pas_eta, float),
                                  (len(pas_el),))
    #: the deck quotes micro-ohm-metres; the conversion belongs on this side,
    #: with the rest of the unit and name work, and the formula in the kernel
    m, r = _kernel().channel_matrices(
        element_arrays(coils), element_arrays(pas_el), W,
        eta_coil=np.broadcast_to(np.asarray(eta_coil_uohm_m, float),
                                 (len(coils),)) * 1e-6,
        eta_vessel=pas_eta * 1e-6, nu=nu, nv=nv)
    return {"M": m, "R": r, "n_channels": W.shape[0],
            "n_vessel": len(pas_el), "weights": W,
            "channels": conductors["channels"]}


# --------------------------------------------------------------------------- #
# circuit evolution — adapted from fyeq circuits.py @ b0648816 (E-11)         #
# --------------------------------------------------------------------------- #
def step_circuits(inductance, resistance, currents_prev, voltages, dt,
                  dpsi_plasma=0.0):
    """One implicit-Euler step: (M/dt + diag(R)) I+ = M/dt I + V - dpsi_pl/dt.

    The kernel's — ★including the ``dpsi_plasma`` term, which used to be the
    reason a second implementation of this step lived here: the trajectory
    entry took no plasma flux, so a call that had one fell back to numpy.

    ★★And including the CHECKS.  ``dt > 0`` and the ``(n, n)`` shape were
    both restated here, and both were already refused below — ``dt`` by the
    ABI entry itself, the shape by :func:`fylite.kernel.step_circuits`.  A
    guard duplicated in a wrapper is not free: it is the guard a reader
    trusts, so the direct callers of the kernel entry (the browser among
    them) read as unguarded when they are not, and the day the two disagree
    the wrapper's version is the one that decides.
    """
    dp = np.broadcast_to(np.asarray(dpsi_plasma, float),
                         (len(np.asarray(currents_prev, float)),))
    return _kernel().step_circuits(inductance, resistance, currents_prev,
                                   voltages, dt,
                                   dpsi_plasma=None if not np.any(dp) else dp)


def evolve_circuits_voltage(inductance, resistance, currents0, time, voltages,
                            psi_plasma=None):
    """Voltage-waveform-driven trajectory (fyeq-compatible conventions).

    The implicit step over [t_k, t_k+1] consumes the interval-END sample
    ``voltages[k+1]``; ``time`` may be non-uniform but must increase.
    Returns (n_time, n_loops) including the initial state.

    ★★"must increase" is now the KERNEL's rule (ABI v93), which is where it
    belongs and where it was not.  It was checked here, in one of several
    callers, and a non-increasing grid does NOT fail on its own downstream:
    ``M/dt + diag(R)`` with ``dt < 0`` stays positive definite whenever R
    dominates, so the Cholesky succeeds and the answer is a plausible
    trajectory running BACKWARDS.  Every other host could ask for one — the
    browser's ``evolveCircuits`` reaches the same entry with no check at all.
    """
    #: ★★ONE kernel call either way.  The plasma back-reaction only changes
    #: the right-hand side, so it is a SWITCH on the trajectory entry, not a
    #: reason to reassemble the advance here — which is what it used to be: a
    #: caller with `psi_plasma` got a Python loop over `step_circuits`, the
    #: same advance a second time, and nothing compared the two.
    #:
    #: ★And the comment this replaced said the kernel "factorises the matrix
    #: once", which `evolution::evolve` does NOT do — it re-forms `M/dt +
    #: diag(R)` and re-factorises every step, as it must on a non-uniform
    #: time grid.  Reusing one factorisation across a uniform grid is a real
    #: optimisation and it is NOT taken; the claim is removed rather than
    #: left standing as a description of code that does the other thing.
    return _kernel().evolve_circuits(inductance, resistance, currents0, time,
                                     voltages, psi_plasma=psi_plasma)
...
# =========================================================================== #
# 4. What the SIGHT LINES do — chord geometry and line integrals (K-24)       #
# =========================================================================== #
#: Every chord-integrated diagnostic asks the same three questions — where
#: does the sight line go, what ψ_N does it see at each point, and what is
#: ``∫ f(ψ) ds`` — yet this port had answered them exactly once, inside the
#: beam model (the old ``beam_chord``, now the kernel's ``beam_deposit`` ray
#: march), in a form specialised to a tangential ray at a fixed height.  The
#: POINT/Faraday forward model answers them inside the Fortran (pre-folded
#: into the ``pol2.est`` response table), and the lower-hybrid model
#: sidesteps them entirely because the antenna's poloidal position is
#: unknown.  A soft-X-ray tomography or bolometry provider (K-7) would have
#: been the third implementation.  This is the one implementation.
#:
#: Two geometries cover the instruments that matter, and both are the same
#: object — a straight segment in 3-D Cartesian space whose ``(R, Z)`` trace
#: is sampled against the flux map:
#:
#: * **poloidal** (:meth:`Chord.poloidal`) — a sight line in a plane of
#:   constant toroidal angle: SXR arrays, bolometer fans, most interferometer
#:   chords.  ``R`` and ``Z`` both vary; the trace is straight in ``(R, Z)``.
#: * **tangential** (:meth:`Chord.tangential`) — a ray at fixed height with a
#:   tangency radius: neutral beams, tangential interferometer views.  ``Z``
#:   is fixed and the ``(R, Z)`` trace is *curved*, since ``R(s) = |P(s)|``.
#:
#: Both reduce to sampling ψ_N(R(s), Z(s)) bilinearly, which is why they
#: belong together — and why they belong HERE: a sight line is a piece of
#: instrument geometry, described by the same document that places the probes
#: and the flux loops.
#:
#: On quadrature: the tomography specimen this requirement came
#: from — integrates with a uniform-step midpoint sum and no convergence
#: check.  That is the cheapest possible rule and it is what makes a grazing
#: chord (few samples inside the plasma) unreliable.  Here the rule is
#: selectable (:func:`integrate`, Simpson by default), and the kernel's
#: ``line_integral_converged`` refines a profile integral until it stops
#: moving, so a chord that needs more samples says so instead of quietly
#: returning a worse number.

class ChordError(ValueError):
    """A chord that cannot be evaluated (degenerate geometry, or off the grid)."""


# --------------------------------------------------------------------------- #
# The flux map a chord is sampled against
# --------------------------------------------------------------------------- #
def psin_map(g) -> dict:
    """``{r_grid, z_grid, psin2d}`` — the normalized flux map a chord samples.

    ``g`` is a g-file path/dict, or an existing map/table dict already carrying
    those three keys (e.g. :func:`kefit_east.nbi._surface_table`'s result), which
    is passed through so callers need not rebuild it.
    """
    if isinstance(g, dict) and {"r_grid", "z_grid", "psin2d"} <= set(g):
        return g
    if not isinstance(g, dict):
        g = geqdsk.read_geqdsk(g)
    #: the normalisation is :func:`fylite.io.geqdsk.flux_map`'s — one host for
    #: "the ψ_N a g-file implies", which three modules used to spell out
    #: separately, each with its own flat-ψ guard
    try:
        r, z, psin, _ = geqdsk.flux_map(g)
    except ValueError as exc:
        raise ChordError(f"{exc} to sample") from exc
    out = {"r_grid": r, "z_grid": z, "psin2d": psin}
    # the LCFS polygon, when the g-file carries one: psin <= 1 is NOT a
    # containment test (see :func:`fylite.kernel.chord_mask`), so the polygon
    # is what a chord integral of a confined quantity must be restricted to.
    if int(g.get("nbbbs") or 0) >= 3:
        out["boundary"] = (np.asarray(g["rbbbs"], float),
                           np.asarray(g["zbbbs"], float))
    return out


def _grid_of(fmap):
    """The kernel grid and ψ_N in the kernel's index order.

    ★``psin2d`` is ``[z, r]`` in this module's map dict (the g-file's own
    order, and the shape every caller of ``psin_map`` already holds), while
    the kernel wants ``[r, z]``.  The transpose is
    :func:`fylite.io.geqdsk.kernel_flux_map`'s — this function takes a MAP
    DICT rather than a g-file, which is the one case that entry cannot
    serve, so it applies the same operation and says where the rule lives.
    """
    from . import kernel
    fmap = psin_map(fmap)
    return (kernel.grid_of(fmap["r_grid"], fmap["z_grid"]),
            np.ascontiguousarray(np.asarray(fmap["psin2d"], float).T))


def psin_at(fmap, r, z) -> np.ndarray:
    """ψ_N at ``(R, Z)`` by bilinear interpolation; ``+inf`` outside the grid.

    Out-of-grid points are ``+inf`` rather than clipped or NaN so that a
    caller integrating a plasma quantity treats them as vacuum without a
    special case, and so that ``psin <= 1`` selects the confined region
    directly.

    The kernel's — including the blend order (Z first, then R), which is what
    keeps a beam and a chord sampling the same map in agreement.
    """
    from . import kernel
    grid, p2 = _grid_of(fmap)
    #: ★the kernel face already promotes and broadcasts (z against r); what
    #: this host adds is only the caller's own shape back
    out = kernel.psin_along(grid, p2, r, z)
    return out.reshape(np.shape(r)) if np.shape(r) else out
...
@dataclass(frozen=True)
class Chord:
    """A straight sight line in 3-D: ``P(s) = origin + s · direction``.

    ``direction`` is stored normalized and ``length`` is the segment's extent in
    metres, so ``s`` is a true path length and every integral below is per metre
    of sight line — not per grid cell, and not per parameter step.
    """
    ...
    def __post_init__(self) -> None:
        d = tuple(float(v) for v in self.direction)
        n = math.sqrt(sum(v * v for v in d))
        if n == 0.0:
            raise ChordError(f"chord {self.name!r}: zero direction vector")
        if not self.length > 0.0:
            raise ChordError(f"chord {self.name!r}: length must be > 0")
        object.__setattr__(self, "direction", tuple(v / n for v in d))

    # ---- constructors ----------------------------------------------------- #
    @classmethod
    def poloidal(cls, r1, z1, r2, z2, *, name: str = "") -> "Chord":
        """A sight line in a plane of constant toroidal angle, ``(R1,Z1)→(R2,Z2)``.

        The instrument geometry of an SXR/bolometer fan or a poloidal
        interferometer chord: give the two points where the line meets the
        vessel (or any two points on it)."""
        d = (float(r2) - float(r1), 0.0, float(z2) - float(z1))
        length = math.dist((r1, z1), (r2, z2))
        if length == 0.0:
            raise ChordError(f"chord {name!r}: the two endpoints coincide")
        return cls((float(r1), 0.0, float(z1)), d, length, name)

    @classmethod
    def pinhole(cls, r0: float, z0: float, view_angle: float, *,
                focal_length: float, pitch: float, n_channels: int,
                length: float = 3.0, name: str = "") -> list["Chord"]:
        """A pinhole camera's fan of sight lines, in one poloidal plane.

        The parameterisation an SXR/bolometer camera is actually specified by:
        an aperture at
        ``(r0, z0)``, a central viewing direction ``view_angle`` (radians,
        measured from +R), a detector array of ``n_channels`` at spacing
        ``pitch`` sitting ``focal_length`` behind the aperture.  Channel ``i``
        looks along

            θ_i = atan((n/2 − i + 0.5)·pitch/focal_length) + view_angle + π

        the ``+π`` turning the ray from "detector behind the pinhole" into "sight
        line into the machine".  Returns one :class:`Chord` per channel, ordered
        as the detector array is.

        ``length`` is how far each ray is followed; it only needs to cross the
        plasma, since :func:`line_integral` discards everything outside the LCFS.
        """
        from . import kernel
        n = int(n_channels)
        if n < 1:
            raise ChordError(f"camera {name!r}: n_channels must be >= 1")
        if not focal_length > 0.0 or not pitch > 0.0:
            raise ChordError(
                f"camera {name!r}: focal_length and pitch must be > 0 "
                f"(got {focal_length}, {pitch})")
        #: ★the fan geometry is the kernel's, ``+π`` included.  That term is
        #: the whole content of the formula — it turns "where the detector
        #: sits behind the aperture" into "where the ray goes into the
        #: machine" — and dropping it points the camera out of the vessel,
        #: which reads as an instrument that sees no plasma rather than as a
        #: sign error.
        theta = kernel.pinhole_angles(float(view_angle),
                                      focal_length=float(focal_length),
                                      pitch=float(pitch), n_channels=n)
        return [cls((float(r0), 0.0, float(z0)),
                    (math.cos(th), 0.0, math.sin(th)), float(length),
                    f"{name}{i}" if name else f"ch{i}")
                for i, th in enumerate(theta, start=1)]

    @classmethod
    def tangential(cls, r_start: float, tangency_radius: float,
                   z_height: float = 0.0, *, name: str = "") -> "Chord":
        """A ray at fixed height whose closest approach to the axis is ``R_tan``.

        The beam / tangential-view geometry: the ray enters at ``R = r_start``,
        grazes ``R = |R_tan|`` and exits at ``R = r_start`` again, so the segment
        length is ``2 √(r_start² − R_tan²)``.  Its ``(R, Z)`` trace is curved
        even though the line is straight."""
        r_out, rtan = float(r_start), abs(float(tangency_radius))
        if rtan >= r_out:
            raise ChordError(
                f"chord {name!r}: tangency radius {rtan:.3f} m is not inside the "
                f"entry radius {r_out:.3f} m — the ray never enters")
        sin_a = rtan / r_out
        cos_a = math.sqrt(max(1.0 - sin_a * sin_a, 0.0))
        return cls((r_out, 0.0, float(z_height)), (-cos_a, sin_a, 0.0),
                   2.0 * r_out * cos_a, name)

    # ---- sampling --------------------------------------------------------- #
    def sample(self, n_samples: int = 601) -> dict:
        """Sample the segment uniformly: ``{s, x, y, z, r, ds}`` (SI, metres)."""
        from . import kernel
        n = int(n_samples)
        if n < 2:
            raise ChordError(f"chord {self.name!r}: need >= 2 samples, got {n}")
        out = kernel.chord_samples(self.origin, self.direction,
                                   float(self.length), n)
        s = np.arange(n, dtype=float) * out["ds"]
        ox, oy, oz = self.origin
        ux, uy, uz = self.direction
        return {"s": s, "x": ox + s * ux, "y": oy + s * uy, "z": out["z"],
                "r": out["r"], "ds": out["ds"]}

    def tangency_radius(self) -> float:
        """Closest approach of the line to the machine axis (R = 0)."""
        ox, oy, _ = self.origin
        ux, uy, _ = self.direction
        # |P0 x u| / |u| in the horizontal plane, with u normalized
        return abs(ox * uy - oy * ux) / max(math.hypot(ux, uy), 1e-30)

    def psin(self, fmap, n_samples: int = 601) -> dict:
        """Sample the chord and attach ``psin`` at every point."""
        pts = self.sample(n_samples)
        pts["psin"] = psin_at(fmap, pts["r"], pts["z"])
        return pts
...
# Quadrature
# --------------------------------------------------------------------------- #
def integrate(values, s=None, *, ds=None, rule: str = "simpson") -> float:
    """``∫ f ds`` over uniformly spaced samples.

    ``rule`` is ``"simpson"`` (default, error ``O(h⁴)``), ``"trapezoid"``
    (``O(h²)``) or ``"midpoint"`` (``O(h²)``, the plain ``Σ f·ds`` the older
    chord codes use
    — kept so a comparison against that convention is possible).  Simpson
    falls back to the trapezoid rule for an odd number of intervals rather
    than silently dropping the last one, which would shorten the chord.

    Non-finite samples (a chord leaving the grid, where ``psin_at`` returns
    ``inf``) must be masked by the caller before integrating a plasma
    quantity; this function does not guess what a vacuum sample should
    contribute.  The rules are the kernel's.
    """
    from . import kernel
    ...
    if ds is None:
        if s is None:
            raise ChordError("integrate: give s or ds")
        s = np.asarray(s, float)
        ds = float(s[1] - s[0])
    try:
        return kernel.quadrature(v, float(ds), rule=rule)
    except kernel.KernelError as exc:
        raise ChordError(f"unknown quadrature rule {rule!r}") from exc


def line_integral(fmap, chord: Chord, f_of_psin, *, n_samples: int = 601,
                  rule: str = "simpson", psin_max: float = 1.0,
                  confined="auto") -> dict:
    """``∫ f(ψ_N) ds`` along a chord — the primitive every chord diagnostic needs.

    ``f_of_psin`` maps ψ_N (array) to the local quantity (emissivity, density,
    …).  Samples outside the grid, and beyond ``psin_max``, contribute zero: a
    chord integral of a confined quantity must not pick up vacuum.  Set
    ``psin_max=None`` to integrate everywhere the grid is defined.

    ``confined`` additionally restricts to the **inside of the LCFS polygon**
    (the kernel's containment rule, :func:`fylite.kernel.chord_mask`) —
    necessary because ``psin <= 1`` is not a containment test.  ``"auto"`` (default) applies it whenever the map carries a
    boundary, ``True`` demands one, ``False`` integrates on ``psin`` alone.  The
    result reports ``confined_by_polygon`` so the caller knows which test ran.

    Returns ``{value, path_length, n_inside, psin_min}`` — the integral plus the
    numbers that say whether it can be trusted: how much of the chord was inside
    and how deep it reached.
    """
    from . import kernel
    fmap = psin_map(fmap)
    pts = chord.psin(fmap, n_samples)
    psin = pts["psin"]
    has_poly = "boundary" in fmap
    if confined is True and not has_poly:
        raise ChordError(
            "confined=True but this flux map carries no LCFS polygon — build the "
            "map from a g-file, or pass confined=False and accept that psin<=1 "
            "is not a containment test")
    used_polygon = bool(has_poly and confined is not False)
    bnd = fmap["boundary"] if used_polygon else None
    #: ★which samples count is the KERNEL's rule, and the same one
    #: :func:`fylite.kernel.line_integral` applies to a sampled profile.
    #: This host exists only because ``f_of_psin`` is a callback and a
    #: callback cannot cross the ABI — that is a reason to evaluate ``f``
    #: here, not a reason to decide "inside" here too.  It used to be
    #: decided twice, so a chord integral could disagree with itself about
    #: where the plasma was depending on how the emissivity was spelled.
    inside = kernel.chord_mask(psin, pts["r"], pts["z"], psin_max=psin_max,
                               boundary=bnd)
    vals = np.zeros_like(psin)
    if inside.any():
        vals[inside] = np.asarray(f_of_psin(psin[inside]), float)
    #: ★★and so are the quadrature AND the three trust numbers.  The mask
    #: was already the kernel's; ``path_length``, ``n_inside`` and
    #: ``psin_min`` were still recomputed here from it, which is the same
    #: rule stated twice — one entry says how deep a chord reached.
    out = kernel.chord_reduce(vals, psin, pts["r"], pts["z"], pts["ds"],
                              boundary=bnd, psin_max=psin_max, rule=rule)
    out["confined_by_polygon"] = used_polygon
    return out
