...
#: ★★2026-09-14（用户裁定 F3′）：`discharge_case` 的缺省改 1.5e-1（`pulse_case` 仍 1e-1）——
#: 退火守线圈电流上限之后，1e-1 在 EAST #137985 上磁轴偏 66.4 mm，1.5e-1 偏 45.0 mm。
#: 起始设计走 `discharge_case`（`stage: start`），所以这里跟着改。1.5e-1 上实测
#: `start_cond` 1.0599e4，对 `COND_LAM2/λ²` = 1.2191e4 差 −13.1 %（带 ±15 % 未动）；
#: 显式 λ=1e-1 的几条（κ₂·λ² 与界）不变，说明动的只是缺省，不是响应矩阵。
LAM_DEFAULT = 1.5e-1
