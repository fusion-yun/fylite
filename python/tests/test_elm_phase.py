"""ELM 相位条件平均 —— H 模测量的那一维（H-15 / `FYL-SDD-07` 表 1.2）。

★★**为什么要有。** H 模的测量按时间窗平均，而一个 5 ms 窗里既有崩塌前恢复好的
台基、也有崩塌后被削平的台基：平均出来的剖面**哪一种都不是**，而台基梯度与自举
电流吃的正是这个差。此前本层只有窗口均值（`io/raw.py::reduce_series`，原 `io/est2.py::reduce_est2`），相位这一维
...
def test_a_noise_only_trace_carries_no_elm():
    ...
    assert raw.elm_onsets(t, 0.1 + SIGMA * rng.standard_normal(t.size)) == []
    #: 一条常数迹也没有 ELM（标度为零）
    assert raw.elm_onsets(t, np.full(t.size, 0.3)) == []
...
def test_the_percentile_scale_is_not_what_is_used():
    ...
    assert len(raw.elm_onsets(t, y)) == 9
...
def test_the_refractory_keeps_one_elm_from_becoming_three():
    ...
    assert len(raw.elm_onsets(t, y, refractory_ms=2.0)) == 9
    assert len(raw.elm_onsets(t, y, refractory_ms=0.0)) >= 20
...
def test_the_phase_is_a_fraction_of_the_period_and_undefined_outside():
    ...
    #: 起始时刻本身相位 ≈ 0
    assert float(raw.elm_phase([on[3] + 1e-9], on)[0]) < 1e-5
...
def test_the_band_selects_the_late_inter_elm_stretch():
    ...
    ph = raw.elm_phase(t, on)
    ped = 0.4 + 0.6 * ph
    band = raw.elm_phase_mask(t, on, (0.6, 0.9))
    ...
    crash = float(np.mean(ped[raw.elm_phase_mask(t, on, (0.0, 0.1))]))
...
def test_the_reduction_is_untouched_when_the_option_is_off():
    ...
    a = raw.reduce_series(get, 1, 0.1, btor=1.8)
    b = raw.reduce_series(get, 1, 0.1, btor=1.8, elm=None)
...
def test_the_conditional_reduction_averages_only_the_band():
    ...
    plain = raw.reduce_series(get, 1, 0.1, btor=1.8, window_ms=8.0, drift_window=None)
    cond = raw.reduce_series(get, 1, 0.1, btor=1.8, window_ms=8.0, drift_window=None, elm=elm)
    #: 磁探针那一列是同一个信号，所以直接比它
...
def test_an_empty_intersection_is_an_error_not_a_quiet_fallback():
    ...
    with pytest.raises(RuntimeError) as e:
        raw.reduce_series(get, 1, at, btor=1.8, window_ms=0.05, drift_window=None,
                         elm={"time": t, "dalpha": y, "phase": (0.6, 0.9)})
...
def test_it_refuses_a_series_it_cannot_phase():
    ...
    with pytest.raises(RuntimeError) as e:
        raw.reduce_series(get, 1, 0.1, btor=1.8, elm={"time": t, "dalpha": flat})
    assert "ELM onset" in str(e.value), str(e.value)
    with pytest.raises(RuntimeError) as e:
        raw.reduce_series(get, 1, 0.1, btor=1.8, elm={"time": t})
