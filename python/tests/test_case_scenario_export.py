...
CASE = "FYDOC-CASE-21-cfedr-hmode-20ma"
STATE = "corpus/CFEDR_260114/ONETWO/FILES/statefile_3.000000E+01.nc"


def _case_dir() -> pathlib.Path | None:
    root = os.environ.get("FYDOC_ORACLE")
    if not root:
        return None
    d = pathlib.Path(root) / "cases" / CASE
    return d if (d / "case.yaml").is_file() and (d / STATE).is_file() else None


@pytest.fixture(scope="module")
def scenario(tmp_path_factory) -> tuple[dict, pathlib.Path]:
    case_dir = _case_dir()
    if case_dir is None:
    ...
    out = tmp_path_factory.mktemp("case21") / "scenario.jsonld"
...
def record(scenario) -> dict:
    ...
    doc, path = scenario
    return fydoc.case_json(doc, base=path.parent)
...
def test_the_ladders_q_is_onetwos_on_its_own_rows(record):
    ...
    d = netCDF4.Dataset(str(_case_dir() / STATE))
    ...
    assert q[0] < 0 and q_ref[1] < 0, "B_T < 0: q keeps the document's sign"
    rel = q[1:] / q_ref[1:] - 1.0
    assert abs(float(np.median(rel))) < 0.02, float(np.median(rel))
    assert float(np.max(np.abs(rel))) < 0.35, float(np.max(np.abs(rel)))
...
def test_the_current_is_held_and_the_map_settles(record):
    ...
    for s in record["fylite:steps"]:
        if s["executed_code"]["id"] == "code/steady_current":
            ...
            assert abs(v - 0.02) < 0.012, line
    assert ip is not None and abs(ip / 20037.1 - 1.0) < 0.02
