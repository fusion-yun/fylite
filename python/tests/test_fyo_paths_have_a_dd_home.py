...
BASELINE: dict[str, str] = {
    ...
    #: fylite 借了 `pf_active` 线圈元件的参数化写法。改法有两条且**不等价**：
    #: 加前缀（诚实，保住数据，承认没有 DD 归宿），或转成 outline（有损，
    #: 要先定 90 个矩形怎么变成轮廓）。实测代价：EAST 导出时这 90 条全丢，
    #: `wall.h5` 只剩 8 个叶子。
    ...
    #: 另一个里没有。改法要定：换算成 `b_field_phi_vacuum_r`（要乘 r0），
    #: 还是承认它是本地的。**这是物理判断，不是改名。**
    "DEVICE/b0": "tf/b0",
