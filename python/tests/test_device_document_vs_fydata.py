...
def test_the_browser_copy_carries_the_same_chords():
    """The browser document (``device/east.jsonld`` on the facts path, formerly
    ``fylite_device_east.json``) is a second copy of this machine for the app.
    ★It can be left behind by an edit to the card.  This is the diff that says
    it was not."""
    import json
    from fylite import facts
    hit = facts.find("device", "east")
    if hit is None or hit.document is None:
        pytest.skip("no browser device document device/east.jsonld on the facts path")
    doc = json.loads(hit.document.read_text(encoding="utf-8"))
    if "fylite:point" not in doc:
        pytest.skip(f"{hit.document} carries no fylite:point (the generated browser "
                    "document does not state the POINT chords)")
    browser = doc["fylite:point"]
