...
| :--- | :--- | :--- |
| 内核（`libfylite.so` 的 `fylite_rs_*` 一面 · 两份内核 `.wasm`） | **不开任何连接** none | 只碰调用方点名的路径 only paths the caller names |
| 数据层（同一个 `libfylite.so` 的 `fylite_runtime_*` 一面）· `fylite data` · `fylite case` | 只在计划或调用方**点名端点**时外连（mdsip 只读） outbound only to an endpoint the plan or caller names | 读写计划与命令行点名的路径 |
...
算例报告页（`ui/pages/report.html`：文件选择、拖放，以及 `?src=<地址>` —— 一条
链接就能替读者选定一份远端文档）。
...
(`ui/pages/report.html`: file picker, drag-and-drop, and `?src=<url>` — a link
...
`ui/tests/validate-report.mjs` 用一份带 `<img onerror>` 的计划把它钉住。
...
登记册（`benchmark/`）存的是**指针与 sha256**，不是本体。所以
...
register (`benchmark/`) stores **pointers and sha256**, not
