...
端口绑定）的办法——见[命令行](../../docs/cli.md)〈先看一眼会发生什么〉里的实测输出。
...
这一行只是动理学反演的**入口**。整条链——映射、分离面对齐、快离子扣除、自洽外环、
曲率正则——的需求、实现与逐段实测，见下一章 [KEFIT 动理学反演](../kinetic/kinetic.md)。
...
[稳定性](../../docs/stability-and-control.md)（作 n=0 分析的对象）。
