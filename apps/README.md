...
| 目录 | 是什么 | 谁消费 |
| :--- | :--- | :--- |
| `ui/` | fylite 自己的浏览器前端（静态站点，内核跑在 WebAssembly 里；也整棵内嵌进 `fy app`） | 站点读者 · `fy app` |
| `examples/` | 典型算例：计划文档 + 讲它的一章，经 `fy run` / Python 入口跑 | MyST 书的读者 |
...
| [`east-free-boundary-evolution/`](east-free-boundary-evolution/README.md) | EAST 自由边界平衡演化：给定 PF/IC 电流与 I_p 推进位形，被动结构电流给定或按回路方程感应自洽，报告各电流源处的磁通 |
| [`east-vertical-control/`](east-vertical-control/README.md) | EAST 竖直不稳定性与竖直位置控制：被动壁与 IC 的刚性竖直模（内核 `code/vstab`）→ PD 增益图、回路时延上限、竖直位移扰动的开环 / 闭环响应 + 单文件结果页 |
