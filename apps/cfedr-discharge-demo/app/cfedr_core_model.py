...
def _resume_call(plan: dict, rec, t_stop: float, dt: float, dt_max: float | None, edge_ne: float | None = None) -> dict:
    ...
    #: ★★**两个名字是两件事**（2026-09-20 修）。内核的参数表写得很清楚：
    #: `dt_target` 单位 "1" —— *relative change per step; 0 = fixed dt*（无量纲）；
    #: `dt_max` 单位 "s" —— *controller ceiling*（秒）。此前这里把 `dt_max` 直接塞进了
    #: `dttarget`，等于拿秒当相对变化用：`--dt-max 0.025` 凑巧成了「每步允许变 2.5 %」而看着正常，
    #: 而 `--dt-max 0.5` 就变成「每步允许变 50 %」——控制器放飞，dt 跑到 4.5 s，
    #: 每步 188 ms，整炮平顶的内步少了 179 倍而墙钟反而更差。
    st["dttarget"] = float(DT_TARGET)
    if dt_max:
        st["dt_max"] = float(dt_max)
    #: ★★**步数预算要按控制器真实用的 dt 估，不是按上限估**（2026-09-20 修）。
    #: 此前用 `dt_max`（**上限**）估：`budget = ceil(窗长 / dt_max) * 4`。而自适应控制器
    #: 实际取的 dt 往往远小于上限——平顶段设 `dt_max = 0.5` 而控制器取 ~0.05，预算于是小了
    #: 一个数量级，内核**烧满 `nsteps` 就返回**（实测首次调用 steps 恰好 = 400 = 预算，
    #: 只走完 50 s 里的 21 s），宿主只好反复再调：一个节拍 9–11 次调用，而每次都要把整份
    #: 工况序列化进去、整份记录序列化出来。省下的步数全赔在过门上，整炮反而更慢。
    #: 现在用上一记录报的 `dt_next`（控制器此刻的步长）作估计，没有记录时退回 `dt_max`。
    t_now = float(rec["facts"]["t_end"]["value"]) if rec else float(st.get("t_start", 0.0))
    dt_hint = float((rec or {}).get("facts", {}).get("dt_next", {}).get("value", 0.0) or 0.0) if rec else 0.0
    if not (dt_hint > 0.0):
        dt_hint = float(dt_max or 0.01)
    budget = math.ceil(max(t_stop - t_now, 0.0) / max(min(dt_hint, float(dt_max or dt_hint)), 1e-6)) * 4
...
#:
#: ★★`tol = 1e-7`（2026-09-20）：内核缺省是 `1e-9`，而**残差还在掉，答案早就不动了**。
#: 实测同一次解（65²、8 趟退火）：`tol` 由 1e-9 放到 1e-7，墙钟 **4.92 → 3.64 s（1.35×）**，
#: 而**边界间隙与 q0 四位不变**（0.1449 · 1.336）；到 1e-4 才开始动第四位。
#: 量法与全表见内核仓 `docs/note/equilibrium-speedup-assessment.md` §2。
#: ★这是**宿主侧的口径**：内核缺省不动（别的调用方可能真要 1e-9 的场）。
DISCHARGE_SETTINGS = {"relax": 0.1, "position_control": "c4", "seed": "target", "tol": 1e-7}

#: ★★**热起步的退火趟数**（`--warm-start` 时才用得上）。给了起始安匝，求解器第一趟退火
#: 就落在答案上：`passes` 取 1 / 2 / 4 时 `boundary_gap_rms` · `q0` · `q95` · `axis_r` 逐位相同
#: （0.1710 · 1.329 · 4.463 · 7.5838），后面那几趟全部被拒，只多花 8–12 s。
#: ★★★**但热起步缺省是关的**，理由是整炮复核量出来的（2026-09-20，见
#: `docs/note/cfedr-demo-speedup.md`）：六步链上它更快也更准，整炮上它把爬升段带到**另一个
#: 平衡**，而燃烧段位形是冻结的（八点不动、I_p 不变，`boundary_tol` 门挡住重解），于是那份
#: 度规一路喂进新经典自举公式——**自举电流从 7.3 MA 跑到 36 MA**（等离子体总共才 10.1 MA），
#: 电流剖面极度峰化（li1 0.57 → 0.99），q₀ 从 2.46 单调塌到 0.17。整炮反而更慢（2998 s 对
#: 872 s）：位形省下的 344 s 全赔进输运求解器暴涨的步数里，还倒贴。
WARM_PASSES = 1

#: ★**外迭代轮数上限**：`None` = 不给，由内核自己定（边规则 3000）。
#: 2026-09-20 这里一度缺省 800，那是替内核的一个缺陷打的补丁——边规则下没有任何早停路径
#: （`equilibrium.rs` 的 `settled` 出口写的是 `!edge_fraction`），磨不动的退火趟只能烧满预算，
#: 实测一次解随上限线性涨（800 用 5.70 s、3000 用 28.23 s，而答案一字不变）。
#: **内核已补上停滞早停**（`STALL_ROUNDS` / `STALL_REL`），预算从此几乎不影响墙钟
#: （整炮 761 s 对 796 s，q₀ 与 dfsdev 范围完全相同），所以补丁撤掉：上限交回内核。
#: 仍留 `--eq-max-iter` 给要自己封顶的人。
EQ_MAX_ITER = None

#: ★**自适应时间步的相对变化目标**（无量纲，内核的 `dt_target`）：控制器每个外步后按
#: `dt *= clamp(dt_target/delta, 1/2, 2)` 调整，落在 `[dt_min, dt_max]` 里。
#: 0.025 是此前由 `--dt-max` 的缺省值意外充当的那个数，保留它以免改动缺省行为；
#: 真正的步长上限是 `--dt-max` / `--dt-max-flat`。
DT_TARGET = 0.025
...
def solve_equilibrium(points, ip: float, device: dict, *, settings: dict | None = None,
    ...
    if EQ_MAX_ITER is not None:
        st["max_iter"] = float(EQ_MAX_ITER)
    ...
    try:
        ...
        pass
    #: ★给了起始安匝 = 热起步：只走一趟退火（:data:`WARM_PASSES` 的理由写在那里）。
    #: 放在 `settings` 之前，调用方显式给的 `passes` 仍然压得过它。
    if pf_current:
        st["passes"] = float(WARM_PASSES)
...
def step(state: dict, control: dict, *, dt_max: float | None = 0.025, max_calls: int = 400,
         ...
         deliver_profile: bool = False, warm_start: bool = False,
         lift_exch_cap: bool = False) -> tuple[dict, dict]:
    ...
    ``boundary_mode``
        * ``"isoflux"``（**缺省**）—— 八点**当 LCFS 的 isoflux 控制点**：点动过 ``boundary_tol`` [m]
          或 I_p 变过 0.05 MA 就调 ``code/discharge`` 重解一次自由边界（热起步后约 1.4–1.6 s），把 ψ(R,Z) ·
    ...
    ``warm_start``
        ``True`` 时**热起步**：指令没给 ``pf_current`` 就用上一步解出的安匝作起点，只走一趟退火。
        单步快 6–18 倍，但**缺省关**——整炮上它把爬升段带到另一个平衡，自举电流跑飞、q₀ 塌到
        0.17，而且整炮反而更慢。理由与读数见 :data:`WARM_PASSES`。
    ...
    if boundary_mode == "isoflux":
        ...
        if _points_moved(eq_state.get("points"), pts, boundary_tol) or abs(float(eq_state.get("ip", 0.0)) - float(control["Ip"])) > 0.05e6:
            ...
            if delivered and delivered.get("refused"):
                ...
                delivered = None
            #: ★`--warm-start`（**缺省关**）：把上一步解出的安匝作这一步的起点。波形里的
            #: `pf_current` 是 31 个空表，所以缺省下这一列永远是空的，每步都冷启动。
            #: 单步快 6–18 倍，六步链上 `dfsdev` 也更小——**但整炮上它是错的**，
            #: 自举电流会跑到 36 MA、q₀ 塌到 0.17（读数见 :data:`WARM_PASSES`）。
            #: **指令给了就以指令为准**，回灌只补指令的空缺。
            warm_current = control.get("pf_current")
            if not warm_current and warm_start:
                warm_current = (eq_state.get("facts") or {}).get("_aturns")
            eq, facts, wall = solve_equilibrium(pts, float(control["Ip"]), device,
                                                pf_current=warm_current,
                                                delivered=delivered)
    ...
    if boundary_mode == "check" and device is not None:
        ...
        if not cached or abs(ip_now - float(cached.get("ip", 0.0))) > 0.05 * max(abs(ip_now), 1.0):
            ...
            if pts:
                #: ★同 isoflux 那一路：`--warm-start` 才回灌（这一档一炮只重解约八次）
                warm_current = control.get("pf_current")
                if not warm_current and warm_start:
                    warm_current = (cached.get("facts") or {}).get("_aturns")
    ...
    #: ★★**交换上限只在准稳态段解除**（`lift_exch_cap`，缺省关）。内核那条上限
    #: `0.25 / nu_max` 由**最冷最密的边界格点**定（ν_ei ∝ n/T^1.5），于是一个格点钉死全域步长；
    #: 耦合档的交换项本来是隐式的，它守的是**刚性边界项**。实测（CFEDR 整炮）：
    #: **准稳态的燃烧段解除它，剖面最大相对差 2.8e-7**，输运墙钟 145.6 → 59.7 s；
    #: **瞬态段解除则不行**——爬升 2.8e-2、下降 6.5e-2。所以只在调用方声明准稳态时给 0。
    if lift_exch_cap:
        plan["settings"]["lift_exch_cap"] = 1.0
    ...
    while True:
        ...
        #: ★★**起始步长也要受 `dt_max` 管**（2026-09-20 修）。此前这里只取 `min(left, dt_next)`：
        #: 首次调用 `rec` 为空就直接用整个窗长，续跑则用上一记录的 `dt_next`——两条路都绕过了
        #: `dt_max`。内核的控制器于是从一个巨大的起点按 `dt *= clamp(dt_target/delta, 1/2, 2)`
        #: 一路减半，**头几步就是最贵的那几步**。单个节拍上看不出来（`rec` 为空、窗长本来就小），
        #: 整炮里节拍相连、`dt_next` 一路长大，才把上限彻底架空：实测平顶设 `dt_max = 0.5`
        #: 而实际走到 dt ≈ 4.5 s，内步少了 179 倍、每步却贵 230 倍，整炮反而更慢。
...
def run_discharge(plan, nodes: list, t0: float, t1: float, dt: float = 0.5, *, dt_max: float | None = 0.025,
                  dt_flat: float | None = None, dt_max_flat: float | None = None,
                  flat_span=(150.0, 6150.0), series_path=None,
                  ...
                  warm_start: bool = False, lift_exch_cap_flat: bool = False,
    ...
    其余
        ...
        ``fuel_gain`` · ``fuel_max_factor`` · ``psi_stride`` · ``warm_start`` 原样转给 :func:`step`；
    ...
    t = t0
    while t < t1 - 1e-9:
        in_flat = flat_span[0] <= t < flat_span[1]
        step_dt = dt_flat if (dt_flat and in_flat) else dt
        #: ★★**物理步长上限也分段**（2026-09-20）。平顶是准稳态，那里大步长几乎不要钱：
        #: 燃烧段一个 50 s 节拍实测 dt_max 0.025 → 0.5 提速 4.2 倍，剖面误差仍在 1e-3 以内。
        #: 而**爬升 / 下降段是瞬态**，同样的 dt_max 会把一个 0.5 s 的控制节拍压成一个内步——
        #: 实测整炮 Te 差 989 %、全局量差 11–16 %，整炮从爬升段就跑偏。所以两段各用各的。
        step_dt_max = dt_max_flat if (dt_max_flat and in_flat) else dt_max
        #: 只有平顶（准稳态）那一段才解除交换上限；瞬态段照旧
        step_lift = bool(lift_exch_cap_flat and in_flat)
        ...
        if not density:
            ctl.pop("ne_bar", None)
        try:
            state, out = step(state, ctl, dt_max=step_dt_max, boundary_mode=boundary_mode, device=device,
                              boundary_tol=boundary_tol, edge_ne_ref=edge_ne_ref, psi_stride=psi_stride,
                              deliver_profile=deliver_profile, warm_start=warm_start,
                              lift_exch_cap=step_lift,
...
def main(argv=None) -> int:
    #: ★轮数上限是**模块常量**（`solve_equilibrium` 读它，`guess_state` 那条路也解位形），
    #: 所以 `--eq-max-iter` 在这里一次盖住，而不是顺着两层签名往下传。
    global EQ_MAX_ITER
    ...
    p.add_argument("--dt-max", type=float, default=0.025, help="内核物理步长上限 [s]（瞬态段）")
    #: ★★平顶是准稳态，物理步长上限可以放大得多（实测 0.5 比 0.025 快 4.2 倍而剖面误差 ≤1e-3）；
    #: 爬升 / 下降段**不能**用它——那里一个控制节拍才 0.5 s，压成一个内步会把整炮带偏。
    p.add_argument("--dt-max-flat", type=float, default=None,
                   help="稳态平顶（150–6150 s）的物理步长上限 [s]；不给就沿用 --dt-max")
    #: ★★内核的 `0.25/nu` 交换上限由最冷最密的边界格点定，会钉死全域步长。平顶是准稳态，
    #: 那里解除它实测剖面差 2.8e-7 而输运快 2.4 倍；瞬态段**不能**解（爬升 2.8e-2、下降 6.5e-2）。
    #: 真正的解法是内核的 stage 3（阻尼 / 牛顿耦合趟），这是那之前的权宜，所以缺省关。
    p.add_argument("--lift-exch-cap-flat", action="store_true",
                   help="平顶段解除内核的 0.25/nu 交换步长上限（配合 --dt-max-flat；缺省关）")
    ...
    #: ★★**热起步缺省关**（2026-09-20 整炮复核裁定，见 `docs/note/cfedr-demo-speedup.md`）。
    #: 它单步快 6–18 倍，六步链上形状误差也更小——但整炮上把爬升段带到另一个平衡，
    #: 冻结的燃烧段度规喂进新经典自举后发散（7.3 → 36 MA），q₀ 塌到 0.17，且整炮更慢。
    #: 留着这个开关是为了那次测量可重跑，以及给愿意自己复核的人。
    p.add_argument("--warm-start", action="store_true",
                   help="位形重解用上一步解出的线圈电流作起点：单步快得多，"
                        "但整炮上自举电流会跑飞（缺省关，开之前请按整炮复核）")
    #: ★`--cold-start` 只在 2026-09-20 当天短暂存在过（那时热起步是缺省）。按名拒绝而不是
    #: 静默接受：写了它的脚本本意是「要旧行为」，而旧行为现在就是缺省，静默接受会让人以为
    #: 自己关掉了什么。
    p.add_argument("--cold-start", action="store_true", help=argparse.SUPPRESS)
    p.add_argument("--eq-max-iter", type=float, default=None,
                   help="自由边界解的外迭代轮数上限（不给 = 交给内核，边规则 3000）")
    ...
    if a.cold_start:
        p.error("--cold-start 已退役：冷启动就是缺省。热起步现在是 --warm-start（缺省关）")
    if a.eq_max_iter is not None:
        if not (a.eq_max_iter >= 1.0 and math.isfinite(a.eq_max_iter)):
            p.error(f"--eq-max-iter {a.eq_max_iter} 不是 >= 1 的轮数")
        #: ★在 `guess_state` 之前盖住：那条路也解一次位形
        EQ_MAX_ITER = float(a.eq_max_iter)
    ...
    res = run_discharge(None, nodes, a.t0, a.t1, a.dt, dt_max=a.dt_max, dt_flat=a.dt_flat,
                        dt_max_flat=a.dt_max_flat, lift_exch_cap_flat=a.lift_exch_cap_flat,
                        ...
                        deliver_profile=a.deliver_profile, warm_start=a.warm_start,
