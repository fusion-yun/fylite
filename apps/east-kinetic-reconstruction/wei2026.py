...
def signal_map(doc: dict, override: dict | None) -> dict:
    ...
    out: dict = {}
    for ids in ("thomson_scattering", "reflectometer_profile", "spectrometer_x_ray_crystal", "ece", "nbi",
                "bolometer", "magnetics"):
...
def pull_shot(lib: K.Lib, shot: int, chain: str, server, timeout_s: float, override: dict | None) -> dict:
    ...
    try:
        ...
        #: 总辐射功率（bolometer 绑定，2026-09-20 新立）：fydoc 只有 AXUV 的 `\PRADTOT_AXUV` 一个单节点可取；
        #: 逐弦剖面节点在树模型里有、实测三炮无数，故取不到芯部/边界的划分（见 h98_account 的 `p_rad_note`）
        prad = f.sig((sm.get("bolometer") or {}).get("p_rad"))
    finally:
        f.close()
    return {"shot": int(shot), "chain": chain, "names": names, "signals": sm, "cache": f.cache, "tf": tf_mid["tf"],
            ...
            "p_rad": prad, "missing": f.missing, "seconds": round(time.time() - t0, 1)}
...
#: H98 的口径开关。文献没说它的 H98 从哪来（ASSESSMENT〈六〉），所以两套账都留着、逐片记明算了哪几项。
#:
#: ``legacy``   2026-09-19 之前的账，逐位复现：P_loss = P_LH + P_EC + P_NBI(源) + P_ohm。
#:              不减 dW/dt、不记辐射、NBI 用离子源功率——旧读数要能重算，故原样保留。
#: ``loss``     缺省。P_loss = P_absorbed − dW/dt（− P_rad，见 ``--h98-prad``）。
#:              ★**为什么缺省不减 P_rad**：IPB98(y,2) 的拟合本身用的就是 P_loss = P_heat − dW/dt
#:              （ITER Physics Basis 1999 §3 的 τ_th 数据库口径，辐射不减）；再减一份辐射是把
#:              标度律的自变量换掉。而且本装置只取得到 **AXUV 总**辐射功率一个数（`\PRADTOT_AXUV`），
#:              逐弦剖面节点三炮无数，**分不出芯部与偏滤器/SOL**——减总辐射会把 ITPA 口径里本该留下的
#:              边界辐射也减掉。故：缺省只记不减，``--h98-prad total`` 减一份**总**辐射作敏感性。
H98_RECIPES = ("loss", "legacy")


def dwdt_at(series, t_prev: float | None, t: float, t_next: float | None, half: float = 0.010):
    """相邻 TS 片上的有限差分 dW/dt [W]，与差分格式的名字。

    中间片取**中心差**（t₋₁ → t₊₁）；★**第一片取前向差**（本片 → 下一片）、**末片取后向差**
    （上一片 → 本片）——两端没有对称的邻点，单侧差是一阶的，记在 ``dwdt_scheme`` 里让读数自己说。
    某一端的窗内取不到数（W 序列有洞）就退到另一侧的单侧差；两侧都取不到记 ``unavailable``。"""
    if not series:
        return None, "unavailable (no stored-energy series)"
    w0 = window_mean(*series, t, half)
    wp = window_mean(*series, t_prev, half) if t_prev is not None else None
    wn = window_mean(*series, t_next, half) if t_next is not None else None
    if wp is not None and wn is not None and t_next > t_prev:
        return (wn - wp) / (t_next - t_prev), "central (neighbouring TS slices)"
    if w0 is not None and wn is not None and t_next > t:
        return (wn - w0) / (t_next - t), "forward (first slice / no usable previous slice)"
    if w0 is not None and wp is not None and t > t_prev:
        return (w0 - wp) / (t - t_prev), "backward (last slice / no usable next slice)"
    return None, "unavailable (no usable neighbouring slice)"


def h98_account(heat: dict, p_oh: float, p_rad, dwdt, dwdt_scheme: str, recipe: str, prad_mode: str) -> dict:
    """一片的功率账：P_loss 与**算进去的项**逐条记名——证书须说清数了什么。"""
    p_abs = heat["p_lh"] + heat["p_ec"] + heat["p_nbi_source"] + p_oh
    acc = {"recipe": recipe, "p_absorbed": p_abs, "p_lh": heat["p_lh"], "p_ec": heat["p_ec"],
           "p_nbi": heat["p_nbi_source"], "p_ohm": p_oh,
           "p_nbi_from": "ion-source power (PNBI*SOURCE); neither the kernel nor the device facts "
                         "carry an injected or absorbed beam power for EAST",
           "p_rad_W": p_rad, "p_rad_from": ("bolometer/p_rad = AXUV total radiated power" if p_rad is not None
                                            else None),
           "p_rad_note": "AXUV TOTAL radiated power: the per-chord profile nodes exist in the tree but "
                         "had no data, so no core/edge split is available",
           "dwdt_W": dwdt, "dwdt_scheme": dwdt_scheme}
    if recipe == "legacy":
        acc.update(p_loss=p_abs, terms_included=["p_lh", "p_ec", "p_nbi_source", "p_ohm"],
                   terms_omitted=["dW/dt (recipe legacy)", "p_rad (recipe legacy)"],
                   note="pre-2026-09-19 recipe, reproduced exactly: no dW/dt, no radiation, source beam power")
        return acc
    p_loss, inc, omit = p_abs, ["p_lh", "p_ec", "p_nbi_source", "p_ohm"], []
    if dwdt is None:
        omit.append(f"dW/dt ({dwdt_scheme})")
    else:
        p_loss -= dwdt
        inc.append(f"-dW/dt [{dwdt_scheme}]")
    if prad_mode == "total":
        if p_rad is None:
            omit.append("p_rad (--h98-prad total asked, but no bolometer signal for this shot)")
        else:
            p_loss -= p_rad
            inc.append("-p_rad (AXUV total)")
    else:
        omit.append("p_rad (recorded, not subtracted: IPB98(y,2) is fitted with P_loss = P_heat - dW/dt; "
                    "and only a TOTAL radiated power is available, not the core part)")
    acc.update(p_loss=p_loss, terms_included=inc, terms_omitted=omit)
    return acc
...
def fit_slice(th, rf, xc, it: int, rm: RhoMap, lad: dict, fa: dict, heat: dict, w_dia, v_loop, meas: dict,
              b_tor: float, clean_opts: dict | None = None, ti0=None, point=None,
              p_rad_series=None, h98_opts: dict | None = None) -> dict:
    ...
    # H98：先用 L 模全段样条算一份 W_th（或用抗磁能），P_loss 的口径见 H98_RECIPES
    ...
    opts = h98_opts or {}
    recipe = opts.get("recipe", "loss")
    ts_t = th["times"]
    dwdt, scheme = (None, "not used (recipe legacy)") if recipe == "legacy" else dwdt_at(
        w_dia, ts_t[it - 1] if it > 0 else None, t, ts_t[it + 1] if it + 1 < len(ts_t) else None)
    p_rad = window_mean(*p_rad_series, t, WINDOW_S / 2) if p_rad_series else None
    acct = h98_account(heat, p_oh, p_rad, dwdt, scheme, recipe, opts.get("prad", "none"))
    p_loss = acct["p_loss"]
    lf = lad["facts"]
    ...
    out = {"time_s": t, "status": "ok", "mode": mode, "h98": K._r(h98, 4), "h98_inputs": {
        ...
        "tau98": K._r(tau98, 4),
        #: ★证书要说清数了哪几项：recipe · terms_included · terms_omitted 是这份 H98 的全部口径
        "account": {k: (K._r(v) if isinstance(v, float) else v) for k, v in acct.items()}},
        "rho": K._r(grid, 5),
...
def _run_chunk(args):
    chunk, clean_opts, scan, anderson, nk, h98_opts = args
    try:
        return chunk_job(_WORKER["lib"], _WORKER["shot"], chunk, clean_opts, scan, anderson, nk, h98_opts)
...
def chunk_job(lib: K.Lib, sd: dict, chunk: list, clean_opts: dict | None, scan: dict | None = None,
              anderson: int = K.ANDERSON, nk: int = K.NEWTON_KRYLOV, h98_opts: dict | None = None) -> list:
    """一段相邻的 TS 时刻，顺序做：第一片全扫，其后每片从**上一片**热启动（竖直位置随时间漂，#63948 从 −30 mm
    漂到 +18 mm——只从全炮第一片热启动时，后面的片几乎都退回全扫）。"""
    out, warm = [], None
    for it in chunk:
        r = slice_job(lib, sd, it, warm, clean_opts, scan, anderson, nk, h98_opts)
...
def slice_job(lib: K.Lib, sd: dict, it: int, warm: dict | None, clean_opts: dict | None,
              scan: dict | None = None, anderson: int = K.ANDERSON, nk: int = K.NEWTON_KRYLOV,
              h98_opts: dict | None = None) -> dict:
    th = sd["thomson"]
    t = th["times"][it]
    t0 = time.time()
    try:
        ...
        res = fit_slice(th, sd["reflect"], sd["xcs"], it, rm, lad, eq["fa"], heat, sd["w_dia"], sd["v_loop"],
                        eq["meas"], eq["b_tor"], clean_opts, sd.get("ti0"), pt, sd.get("p_rad"), h98_opts)
...
def cmd_profiles(a) -> int:
    ...
    h98_opts = h98_options(a)
    jobs = [(c, clean_opts, scan, a.anderson, a.nk, h98_opts) for c in chunks]
    if nw > 1:
        ...
        for c in chunks:
            results.extend(chunk_job(lib, sd, c, clean_opts, scan, a.anderson, a.nk, h98_opts))
        K.close_jobs()
    ...
    out = {"@type": "fylite:Wei2026Profiles", "app": APP, "version": K.VERSION, "reference": REF,
           ...
           "method": dict(method_card(clean_opts, h98_opts), scan=scan, anderson=a.anderson, nk=a.nk),
           "data": {"thomson_layout": th["layout"], "thomson_pulses": len(th["times"]),
                    ...
                    "radiated_power": sd.get("p_rad") is not None,
                    "missing": sd["missing"]},
...
def h98_options(a) -> dict:
    return {"recipe": getattr(a, "h98_recipe", "loss"), "prad": getattr(a, "h98_prad", "none")}


def method_card(clean_opts, h98_opts: dict | None = None):
    o = h98_opts or {"recipe": "loss", "prad": "none"}
    h98 = {"recipe": o["recipe"], "p_rad_mode": o["prad"],
           "p_loss": ("P_LH + P_EC + P_NBI(source) + P_ohm (pre-2026-09-19 recipe, reproduced exactly)"
                      if o["recipe"] == "legacy" else
                      "P_absorbed - dW/dt" + (" - P_rad(AXUV total)" if o["prad"] == "total" else "")),
           "dwdt": ("not used" if o["recipe"] == "legacy" else
                    "finite difference of the diamagnetic energy across neighbouring TS slices; "
                    "central inside, forward on the first slice, backward on the last"),
           "p_rad": ("bolometer/p_rad = AXUV total radiated power (fydoc bind/mdsplus/bolometer, 2026-09-20); "
                     "recorded on every slice, subtracted only with --h98-prad total"),
           "p_nbi": "ion-source power: no injected/absorbed beam power in the kernel or the device facts",
           "note": "the paper does not state its H98 recipe; each slice records what was counted "
                   "in h98_inputs.account"}
...
def cmd_transport(a) -> int:
    ...
    prof = fit_slice(th, sd["reflect"], sd["xcs"], it, rm, lad, fa, heat, sd["w_dia"], sd["v_loop"], eq["meas"],
                     eq["b_tor"], None, sd.get("ti0"), pt, sd.get("p_rad"), h98_opts)
    if prof["status"] != "ok":
    ...
    out = {"@type": "fylite:Wei2026Transport", "app": APP, "version": K.VERSION, "reference": REF,
           ...
           "method": method_card(None, h98_opts), "profiles": prof, "heating": report,
...
def main(argv=None) -> int:
    ...
    for name in ("profiles", "transport"):
        ...
        p.add_argument("--h98-recipe", choices=H98_RECIPES, default="loss",
                       help="H98 的 P_loss 口径：loss = P_absorbed − dW/dt（缺省）；legacy = 2026-09-19 之前那份"
                            "（P_LH + P_EC + P_NBI 源 + P_ohm，不减 dW/dt、不记辐射），旧读数照此可重算")
        p.add_argument("--h98-prad", choices=("none", "total"), default="none",
                       help="辐射功率进不进 P_loss：none = 只记不减（缺省；IPB98(y,2) 的拟合口径不减辐射，"
                            "且本装置只取得到 AXUV 总辐射、分不出芯部）；total = 减一份总辐射作敏感性")
