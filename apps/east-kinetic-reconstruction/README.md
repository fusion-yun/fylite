...
# 反演三档 → 结果 JSON（单机一到两分钟，大半花在档 M 的扫描与剔道上）
...
- `tiers.M` / `K` / `P`：`status`（`ok` / `error` / `skipped` 与理由）· `facts`（q₀、q₉₅、l_i(3)、磁轴、χ²、χ²/dof、最劣单道、迭代、
  ...
  用与不用都列）；M 另有 `scan`（哪种扫法与它的参数、65² / 粗网格各解了几次、哪几轮退回全扫；`settings.scan` 同记）、
...
# 阶段 A：一炮的全部 TS 时刻（#63948：35 个脉冲）→ 逐片剖面、清洗记录、NRMSE（12 进程约 4 分钟）
...
两条命令都收 `--scan coarse|full`（缺省 `coarse`，同〈二〉档 M）。它只作用在冷启动的全扫上（每段第一片、热启动解不出
退回的那片）；热启动只在上一片 ±8 mm 的 5 个设定点上解，粗排省不下什么，照全扫（结果里该轮记
`strategy: full`）。所用做法记在结果的 `method.scan` 与每片的 `equilibrium.scan`。
...
| #63948 全炮 35 片的耗时 | 取数 52 s + 各片 192 s（12 进程，`--scan coarse`；`full` 235 s）≈ **4 分钟** | 19.95 s（平衡读 P-EFIT） |
