...
modified:
  ...
  change: 'v0.2 只保留一个可执行文件（用户裁定）：`fylite-data` / `fylite-case` 两个薄壳
    ...
    as-built：`rust/fylite_runtime/src/cli/`（规格驱动解析器 + `data` / `case` 主体）、
...
（`rust/fylite_runtime/src/cli/mod.rs` 的 `include_str!`）并由它建自己的解析器与用法，浏览器的
...
- **Rust**：`rust/fylite_runtime/src/cli/mod.rs`（规格驱动解析器：命令树下降、`--k=v`、短选项、
