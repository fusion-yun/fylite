...
2. **设计书**（信息性）：★2026-09-12 起分**两部**读——**第一部 总体**（`FYL-DESIGN-22` 先读：十二条原则 · 八项共性功能 × 四宿主 · 六种共享文档），其下按四层架构分两组。**内核与中间层**——`FYL-DESIGN-16`
   ...
   功能页各一篇，`-21` 分析页反演部件的流程图重规划，`-19` facts 的发行形态；**第二部 场景线**——`-23` 总目录与章模板（场景表由 `lines.jsonld` 生成），`-24` 动理学平衡重构（样板章：算法流程图 · 命令面 · 效果图 · 判据），`-18` 四页共用的输入生成、图形交互与文档交换。设计书的规范条款经提案入 SRS / SDD；尚未落文本的提案编号集中登记在
   `FYL-SRS-01` 附录，免得撞号。
...
| `X-` | `FYL-DESIGN-24` | X-1..X-3 · X-G-1..X-G-3 | 三面一计划 · 控件皆有词表条目 · 阈值只写量过的；`X-G-` 是本章缺口 |
...
| [`FYL-DESIGN-23`](FYL-DESIGN-23.md) | **场景线总目录与章模板**（第二部封面）——四条 `fy run` 线 ↔ S-L1..S-L4（S-L5 无线，可跑部分住 design 线）；场景表**由 `lines.jsonld` 生成**（20 条：可跑 / 有模板门不认 / 不设模板各带数据里的理由；一处过期自述 G-1：理由串仍说 12 个 code 而声明面 35 个）；章模板 W-1..W-4（一章一场景 · 四件必备 · 可选件 · 不重述共性）；各章整理状态表（-24 样板已落，其余待整理、裁定都在正本） | v0.1 · WD |
