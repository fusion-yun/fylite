...
modified:
  ...
  change: 'v4.5：`FYL-DESIGN-16` 升 v1.3（**中间层也进浏览器**：`fylite_runtime` 编成 wasm 由 JS 调用；
...
| [`FYL-DESIGN-16`](FYL-DESIGN-16.md) | **可替换内核与四层分工**——内核（本地 `.so` / 页内 wasm / 远端进程 / 另一种实现）以 **fyo 文档门为唯一接口**：code 表声明能力、manifest 声明结果，ABI 号降为本地后端内部物；装配搬进内核，宿主退成计划构造器；`fylite_runtime` 定为 **SpData 的 profile**（重叠语义以 SpData 为准并跑其向量）并持有后端表；前端只写计划只读记录。实测底账：Python 125 / 浏览器 146 个扁平调用 vs 文档门 3 个 code。裁定 K-1..K-7 · **N-1（改名：推荐 `fylite_runtime`）** · D-1..D-4 · H-1..H-3，五条既有裁定改口（含「双宿主」→「多宿主」），分期 P0..P3，缺口 G-1..G-6。★v0.2：**多宿主**（CLI / Python 库 / 网页 / AI 面）与**中间层**改口 · ★v0.3 / v0.4：改名**已执行**，落定 `fylite_runtime`（N-2 记下 N-1 那条被证伪的理由与两层的重叠实测）· ★v0.5 / v0.6：**K-8**（装置：A-Box → 中间层 → 整份 fyo 结构体 → 内核；G-3 已关，目标表「不认识装置」改为「不认识数据源头」）· ★★v0.7 **K-8a 实测缺陷**（路径被按 / 切尾丢掉、重复键后写者胜，两文档同名收尾静默并成一条，`rc=0`）· ★v0.8/v0.9/v1.0 **内核收树**：flat tree 定为交互接口、**双向可读可写**（今天两侧各在做文本活，双向树一并删掉四处）；**回调式已撤回**（浏览器侧宿主是 JS、远端每次取值一次往返、溯源说不清输入集）；补数据走 **S-1 先问后跑 / S-2 按名拒绝带树再入 / S-3 编进内核**，惰性取数否决 · ★v1.1 **内核无状态**（实测全局态为零），状态改为 `fylite:state` 声明子树随文档走，单步/多步归计划——补数据再入 / 断点续跑 / 取消 / 从中间复算四事共用一个机制 · ★v1.2 **状态管理拆五件事**：声明产生归内核、携带归中间层、持久化与决定归各宿主（实测早已分散）——机制居中、策略分散 · ★★v1.3 **中间层进 wasm**（三宿主同一张四层图；`geqdsk.js`/`fyo.js`/`session.js` 的职责归中间层；两个 wasm 模块 JS 接线；两处理由作废/换理由，结论不变） | v1.3 · WD |
