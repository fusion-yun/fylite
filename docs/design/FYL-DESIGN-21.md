...
| **1.2 时间窗平均；H 模按 ELM 相位条件平均** | filterscope Dα 触发 | 窗口均值有（`est2`）；★**2026-09-12 相位选择也有了**（H-15）：`io/est2.py` 的 `elm_onsets`（Dα 上升沿 + 死区 + 噪声闸）· `elm_phase`（周期分数，两次起始之外为 `nan`）· `elm_phase_mask`（缺省晚 inter-ELM 窗 0.6–0.9），并在 `reduce_est2` 上开了一个**缺省关**的 `elm=` 开关：打开时平均的是「窗内**且**带内」的采样，空交集**按名报错**而不悄悄退回普通窗口。实测（合成迹）：周期 20 ms、占空比 2 %、σ=0.02 的十次 ELM 数出 9 个起始，只有噪声的迹数出 0 个；同一段台基信号上晚窗均值 0.85 对全窗 0.70。**余下的是那个「触发的输入端口」**：本分发里没有任何 Dα 节点名（EAST 绑定表无 filterscope 通道，与 Thomson / CER / ECE / MSE 同一个缺口），所以序列今天是**交进来**的；要成为计划里的一步，得先有那个端口（槽治理，O 序）| ◐ |
