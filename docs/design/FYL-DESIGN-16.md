...
modified:
  ...
  change: 'v0.8 新增一章「内核怎样收下一棵树」：把 K-8b 那条工作假设正面摆开（它把「今天没有文档
...
〔一句话〕**内核可以收树，代价是一个约三百行、零依赖的「已解析树」阅读器；解析仍然不在
内核里。** 要给的不是 JSON，是一份**已经走完、按定长记录摊平**的树，内核借用着读，不分配、
不解析、不认识文本。
...
## M-2 的形 (The shape of M-2)

**四段缓冲，一次校验，之后全是借用。**

1. **节点表** `[Node32]`，定长记录，先序排列：种类 · 名字（偏移 + 长度）· 第一个孩子 ·
   下一个兄弟 · 载荷偏移与长度 · 形状（偏移 + 维数）。
2. **名字块** UTF-8 字节，按偏移+长度取，不带结尾零。
3. **f64 载荷** —— **8 字节对齐、独立成段**。这是整个选择的关键：数值内核要的是
   `&[f64]`，**零拷贝**；一个面向字节的格式（JSON / CBOR）在这里恰恰最不合适。
4. **整数 / 字符串载荷** 各一段。

内核侧一个模块（`doc.rs`）：`Doc<'a>` 借用这四段，给 `root()` · `child(name)` ·
`at(i)`（结构数组下标）· `f64s()` · `str()` · `need(路径, 说明)`（**取不到按名拒绝**）。
不分配、不解析、不认识文本编码。

**进门先校验一次，O(n)，之后不再信任缓冲**：偏移全在界内 · 先序单调（无环）· f64 段
8 字节对齐 · 名字块是合法 UTF-8。**畸形缓冲当场拒绝**，不带着走。

★**「半结构化」的边界另有一条**：M-2 之后内核**看得见**未声明的键。看得见不等于可以用——
**内核只从已声明的槽取数**，未声明的键至多**记一句注记**（「文档带了我不认识的键」）。
否则「声明或拒绝」这条就散了。
...
| 期 | 做什么 | 判据 |
| :--- | :--- | :--- |
| **T-1** | 定格式；中间层写编码器（它已经有树）；内核写 `doc.rs` 阅读器；**旧门一字不动** | 往返闸：中间层编码 → 内核走一遍 → 把看到的原样报出 → 与源文档逐叶子比 |
| **T-2** | 门加一条收文档的路（新符号或版本位），**先只接一个 code**（`transport` 最小） | 同一算例两条门给**逐位相同**的数 |
| **T-3** | 其余 code 逐个搬过来；K-8a 那类撞名**按构造不可能**（两份文档是两个根） | 每搬一个，两门对拍一次 |
| **T-4** | 删掉路径形与 `case.rs` 里那句按 `/` 切尾 | 门只剩一种入参形状 |

**代价**：内核 +约 300 行（一个模块、零依赖、wasm 不变性质）；中间层 +约 200 行（编码器）；
Python 侧**先不动**（它经中间层）。ABI 号加一。

**不变**：内核仍不读文件、不开网络、不认识数据源头、不分配。收下的是一棵**已经被别人走完的**
树——走树的责任还在中间层，只是走的结果现在整棵交过去，而不是摊成一袋会撞名的路径。
