...
| **复测** | 2026-09-02：成立——35 passed, 0 failed, 0 error, 0 skipped, 0 stale |
...
四行表（解了哪几道方程 / 哪些量是喂进去的 / 单位与径向标签 / COCOS）的完整账在私仓账本（`$FYDOC_ORACLE/tgyro_treg01/README.md`）；下面是登记册随本条记录携带的口径说明，逐条照录：
...
| 门 | 计数 | 首条信息 |
| :--- | :--- | :--- |
| $FYLITE_KERNEL/tests/test_mapping.py | 32 过 / 0 败 / 0 错 / 0 跳 / 0 陈旧 |  |
| $FYLITE_KERNEL/tests/test_flux_chain.py | 3 过 / 0 败 / 0 错 / 0 跳 / 0 陈旧 |  |
