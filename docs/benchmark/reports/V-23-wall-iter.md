...
| **对象** | fylite: code/wall 经树门（ITER 卡片的 pf_passive，324 个 loop 自 fydoc 由 CC BY 原件离散；元件互感 · 电阻 · M dI/dt + R I = 0 的 L/R 本征模，每组另单解；每元 8×8 细丝）＋本记录自带的超导回路屏蔽（Maxwell 共轴圆环互感，Schur 补消去线圈） |
| **数据** | 见 §5 表（3 项） |
...
| **复测** | 复测 2026-09-16（本条写入时把门跑一遍）：成立——7 passed, 0 failed, 0 error, 0 skipped（python/tests/test_benchmark_wall_iter.py，2026-09-16 本机实测） |

> 本页由公开检出的 `tools/benchmark-equilibrium-records.py` 自登记册写出（2026-09-15 起平衡相关记录在本仓直写，不经内核仓的发布器）；判据与读数是登记册的，「复测」是写入当日把门跑一遍的结果。

**适用域**：ITER 卡片的被动集（自 CC BY 原件离散的 324 元，8×8 细丝）；无等离子体、纯 L/R 电路；屏蔽按零电阻回路的 Schur 补，线圈以单丝 × 匝数计（自感取矩形等效半径）；端口按连续环建模（已知过度）；不含 3-D 端口结构、不含等离子体响应
...
| 复测 2026-09-16（本条写入时把门跑一遍） | 7 passed, 0 failed, 0 error, 0 skipped（python/tests/test_benchmark_wall_iter.py，2026-09-16 本机实测） | 成立 | 本条是新立记录，registry 里此前没有可沿用的复测条，故由 `--reruns` 给出本次实测。；同批另跑：EAST 卡片 28 passed / 195 s，ITER 卡片 11 passed / 40 s，合计 39 门全过、零 skip。 |
