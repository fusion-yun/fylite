...
# ICRH 对 METIS：共振层 1.9 %、少数离子四料 ≤ 7.8 %、定常行电子功率 0.955–1.079 与快离子能 0.948–1.014、剖面峰 ≤ 0.05；FWCD 对测量中位 0.979
...
- **类**：对拍　**判决**：**成立**
- **量的是**：ICRH 对 METIS：共振层 1.9 %、少数离子四料 ≤ 7.8 %、定常行电子功率 0.955–1.079 与快离子能 0.948–1.014、剖面峰 ≤ 0.05；FWCD 对测量中位 0.979
...
| 判据 | 容差 | 取法 | 量到 | 判 |
| :--- | ---: | :--- | :--- | :--- |
| 共振层 R_res 对 METIS 的相对差（每行）；x_res 一格 | 0.03 | measured_band | 40 行：R_res 最劣 1.918e-02、中位 1.603e-03；x_res 最劣 0.0670 | **成立** |
| 少数离子四料：n_min · 受热体积份额 · E_crit · tau_s | 0.1 | measured_band | 36 行最劣：n_min 2.145e-02 · 份额 2.462e-02 · E_crit 5.063e-02 · tau_s 7.843e-02 | **成立** |
| 定常行的电子功率与快离子能对 METIS 的比 | 0.15 | measured_band | 定常 16 行：p_el 0.9553 – 1.0791、W_fast 0.9480 – 1.0145；未定常 20 行（排除）：0.0197 – 38.85 倍 | **成立** |
| 剖面：峰位与宽度对 METIS 写下的那条 | 0.05 | measured_band | 36 行：峰位最劣 0.0500（恰一格）、宽度最劣 2.407e-02 | **成立** |
...
- ★★**IC 没有 `code/` 门**：内核有这一族的全部函数，公开仓的树门够不着，1.5D 源里也没有 IC（只有 beam 与 lh 沉入）。这是本域的一处实现空白，不只是登记空白。
