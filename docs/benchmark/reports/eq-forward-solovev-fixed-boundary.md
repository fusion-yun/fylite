...
| q0 对闭式解的相对偏差 | 0.0001 | measured_band | -1.974e-05（相对）· 判据 1e-4 · 余量约 5 倍 | **成立** |
...
**`q0 对闭式解的相对偏差`** — ★★**这条带不画在 fylite 自己的实测上**——那样画出来的带，任何数都能「过」。它画在**同一道题上另一套实现达到的精度**：CHEASE NS=80 的 q0 对闭式解差 3.56e-06，本判据放宽到 1e-4（约 28 倍）仍留足余地。★★**带从立起到今天一格没动**：1.0 版实测 3.27e-03 判 fail，内核改后 1.1 版实测 1.97e-05 判 pass。**一个不会移动的靶子，才是改动值不值的证据。**
...
**★q0 —— **已修**：从超出 33 倍到余量 5 倍**

- ★★**病根是外推，不是精度**。1.0 版实测 −3.26e-03，且**随网格换号**（33² +3.69e-02 → 65² +4.96e-03 → 129² −3.26e-03）——`q_profile` 当时把 q0 取作最内两个磁面到 psi_N = 0 的线性外推，误差随第一个面落在哪里走、不随解收敛。**一个随网格加密而翻号的数，不是在收敛到什么。**
- ★**改法是取解析极限**：近轴那个围道积分有小椭圆的闭式值，`q0 = F(0)·2π/(R_axis·√(det H))`，det H = ψ_RR ψ_ZZ − ψ_RZ²——CHEASE 与各定边界码用的就是它（内核仓 d13376b）。
- ★**现在三档同号且逐档缩小**：33² -0.000483 → 65² -0.000124 → 129² -1.97e-05（约 4× 与 6×）。**「变小」不是证据，「同号且随网格单调缩小」才是**——前者一次巧合就能做到。
- ★同一道题上 CHEASE NS=80 是 3.56e-06，本侧现在 -1.97e-05——仍差约 6 倍，**记在这里，不当作已经追平**。
...
- ★★**这条记录曾经判 fail 而守它的门是绿的——那段经过值得留着**：门守的是**防劣化**（「还是这个数吗」，它的带当时就画在 3.27e-03 上，所以绿）；记录判的是**够不够**（「这个数配得上一个解析基准吗」，对照另一实现的 3.56e-06，不配）。**一道绿的门不等于一条成立的记录**——这正是本册要与测试套件分开存在的理由，也正是这一处缺陷是被册子而不是被测试抓出来的原因。
- ★内核修好之后，内核侧的门也跟着收紧了（带 7e-3 → 5e-5，另加「三档同号且逐档缩小」的断言）——**记录发现问题，门负责它不再回来**，两者分工如此。
...
| 版本 | 日期 | 谁 | 做了什么 |
| :--- | :--- | :--- | :--- |
| 1.0 | 2026-09-16 | Claude Opus 5 (1M context) | 首次入册：新册 eq 组的第一条验证记录，数值取自当日实跑读数；q0 一档判 fail（−3.26e-03 对判据 1e-4），按用户裁定记为保留的缺口 |
| 1.1 | 2026-09-16 | Claude Opus 5 (1M context) | ★改判为成立：内核把 q0 从「最内两个磁面外推」改成「轴上解析极限」（内核仓 d13376b），实测 −3.26e-03 → −1.97e-05，三档网格同号且逐档缩小。**容差一格没动**——带留在原处，改动才有一个不会移动的靶子。本条的 open_defect 随之撤销。 |
...
- `docs/benchmark/readings/solovev_fixed_boundary.json`    `sha256:32c17f7511b7745a7b32604cb8ed1042f7f9c3487c1093c6be48fb35b97e752d`    本次运行的读数（问题定义 + 三档网格的 fylite 侧 + CHEASE 侧 + 逐项比较）。★该件由 `python tools/benchmark-fixed-boundary.py solovev --out <DIR>` 产出，可原地复算。
