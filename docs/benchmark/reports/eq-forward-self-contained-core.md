...
**`抄录点名的八个模块，在本核里是否都有着落`** — ★抄录列的是 **fytok 的 Python 模块名**（numerics / circuits / contour / forces / solver_core / response / inverse_core / reconstruction_core）。本核是 Rust，模块划分不同，**只能按职能对照**——对照表逐条列在 finding 里，★**这是一次翻译，不是逐字核对**，读的人有权知道。
