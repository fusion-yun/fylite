...
*平衡 (Equilibrium) · [静态逆解：形状到线圈电流](../eq/inverse.md)　|　记录正本：`records/eq-inverse-iter-reference-separatrix.jsonld`*
...
| 逆解是否收敛 | 1 | machine_precision | converged = 0（settled = 1，残差 0.000965，78 次迭代 / 16 轮） | **不成立** |
...
**`逆解是否收敛`** — ★判据是 converged == 1。★「settled」不是「converged」——残差不再动与残差足够小是两件事。
...
- ★残差 9.65e-04 已经很小，解也不再动（settled），但门要的是 converged。照实记 fail——把 settled 读成 converged 是这类记录最容易的自欺。
...
- 内核 `libfylite` `sha256:e0e1b16cf0004c128eaaaff81c3d9c36d8a971ce024b2b411c6d7ea5be165ced`　—— 须设 `$FYLITE_KERNEL_LIB` 指向带该门的构建，并设 `$FYLITE_DEVICE_DIR` 指向 ITER 牌所在目录
