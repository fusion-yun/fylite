...
**`IC：剖面对体积分 = 给定功率（总量与离子份额）`** — ★判的是内核函数 `icrh_profile`（门 `code/icrh` 用它铺剖面，门级的两道之和 = 吸收另由 `tr-sources-icrh-metis` 守）；门是内核仓的 Rust 单测。
...
- ★★**账闭合不证物理**：把全部功率放在轴上的模型，账一样闭合。沉积位置与驱动效率由本域其余各条判（EC 对 TORAY、ECCD 与 ICRH 对 METIS、NBI 出生对 NUBEAM、LH 位置对 GENRAY）。
...
| 1.1 | 2026-09-19 | Claude Opus 5 | caveat 与第四格说明照实改：NBI / LH 已有外部参照（NUBEAM 出生、GENRAY 位置），IC 已有门。判据与数值不动。 内核换代（`b27d7145ab3e`：新门 `code/icrh`——ICRH 少数离子加热第一次经门可达，`CASE_CODES` 41 → 42，只加不改）。内核侧：`icrh_door` 2 · `fyo` 7 全过。★没有一处既有缺省数值变动。 |
