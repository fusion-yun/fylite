...
├── figures/                   ← 对照图：等高线 / 剖面叠画 / 余量，★由记录与曲线读数生成
...
| `figures/<ID>-contours.svg` | ★**两侧的磁面等高线画在一起**（有曲线读数的记录才有） | ★生成 |
| `figures/<ID>-qprofile.svg` | ★**两侧的剖面叠画 + 差值面板**（同上） | ★生成 |
...
★★**报告在 toc 上自成一组**（〈逐条报告〉，与三个域组平级），组内按**域树**排序。
它们最初挂在各自的域章下面——那样与书的轴一致，但落在第四层，侧边栏默认收起，读者看不见
（2026-09-16 用户裁定改为平级）。★代价照实记：报告与它所属的域，在 toc 上不再有父子关系；
**这层关系改由正文承担**——各章的〈本域的记录〉表逐条链过去，覆盖表与状态页同样。
两头都有闸：`test_every_record_has_a_report_and_the_book_lists_it` 守「书里列得到」，
`test_the_prose_actually_points_at_the_reports` 守「正文里点得到」——**入了 toc 不等于读者点得到**。
...
按轮次一页，放在 [`summary/`](summary/2026-09-16-equilibrium.md)：

- [2026-09-16 · 平衡组](summary/2026-09-16-equilibrium.md) —— 六域做到全部有据；
  q0 的归属、逆解的零空间、孪生的主场优势，以及四处明写没做的。

★它是**一轮工作的快照，不是正本**：记录才是正本，且记录带着自己的变更史。
下一轮补一页，**不改写上一页**。
