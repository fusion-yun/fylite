...
- 判据：`min(T_e, T_i) > 0`
- 结论：通过——最小值 3000（CORE_PROFILES/te）；读了 CORE_PROFILES/te
...
### `steady-state` — 准稳态窗口：steady_change 在声明的界内

- 判据：`max|steady_change| ≤ 算例声明的 tolerance`
- 结论：未评估——记录里没有 steady_change
- 假设：算例不声明就不判（unevaluated）
- 读不到：SUMMARY/steady_change
