...
| `positive-temperature` | 定律 | 通过 | 300 | 0 | machine_precision |
| `positive-density` | 定律 | 通过 | 5e+18 | 0 | machine_precision |
...
| `boundary-closed` | 定义 | 通过 | 1.154 | 1.5 | measured_band |
...
- 判据：`min(T_e, T_i) > 0`
- 结论：通过——最小值 300（CORE_PROFILES/te）；读了 CORE_PROFILES/te, CORE_PROFILES/ti, SUMMARY/te_axis, SUMMARY/ti_axis
...
- 结论：通过——首末点间距 0.04985 m = 1.15 个采样步（步长中位数 0.04321 m，小半径 0.6 m）；没有限制器外形，只量了闭合性
