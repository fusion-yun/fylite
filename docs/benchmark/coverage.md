...
从线圈电流与剖面正着解 Grad-Shafranov，解得对不对；正反两向共用的那张响应核是不是同一张。　→ [本域章页](eq/forward.md)
...
解出来的磁面上那些积分量（gm1..gm9 · 磁剪切 · Mercier · beta_p · l_i · W_mhd）与形状参数（MXH）算得对不对。　→ [本域章页](eq/surface.md)
...
让平衡随时间走，被动结构里的涡流跟不跟得上——轨迹对不对。　→ [本域章页](eq/evolve.md)
...
反过来问：要这个形状，线圈该通多大电流。　→ [本域章页](eq/inverse.md)
...
从一炮的磁测量、MSE、动理学剖面反推平衡，反得准不准——★判准必须落在**可观测空间**。　→ [本域章页](eq/reconstruct.md)
...
符号、方向、2π 因子、单位——两个码的数能不能放在一起比，先过这一关。　→ [本域章页](eq/convention.md)
...
拉长的位形会不会竖直漂走、增长率多快；线圈受多大力；导出的线性模型对不对。　→ [本域章页](mhd/vertical.md)
...
教科书有闭式答案的那两级：均匀电流柱的 q 带边、气球模 alpha_c(s) 曲线。★它们是本册的**尺**——尺不准，上面的都不算数。　→ [本域章页](mhd/analytic.md)
...
delta-W 变分求解器本身：柱极限对不对、环几何耦合装配对不对、共形映射对不对。　→ [本域章页](mhd/energy.md)
...
三项齐全的 delta-W 对着国际 V5 基准的五码带落不落得进去；阻性壁模窗口在哪。　→ [本域章页](mhd/deltaw.md)
...
多通道 1.5D 方程组本身：离散、时间推进、五类边界条件、电流边界驱动。　→ [本域章页](tr/equations.md)
...
chi / D 从哪来、源项怎么沉积——每个插件对着它移植自的那个上游码，逐位对不对。　→ [本域章页](tr/closure.md)
...
刚性闭包下还收不收敛（Pereverzev-Corrigan）；稳态通量匹配的根找不找得到、找得准不准。　→ [本域章页](tr/paradigm.md)
...
台基高度、锯齿触发与 Kadomtsev 重分布、加料抽气的存量账——守不守得住。　→ [本域章页](tr/pedestal.md)
...
解释性/预测性双模、与平衡的松散耦合、NN 代理对上游权重的逐位一致。　→ [本域章页](tr/coupling.md)
...
守恒回归到 1e-12；对 FUSE.jl / TORAX 逐模块 parity；IMAS/DD 一致；核自包含。★这一域是横切的——它管的是另外五域的**产出合不合格**。　→ [本域章页](tr/conservation.md)
...
| `FR-TR-005` | 求解范式：刚性稳定化与稳态通量匹配 | 刚性稳定化生产路径 |
