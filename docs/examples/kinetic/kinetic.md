...
```bash
export FYDOC_ORACLE=<fydoc>/cases                  # FYDOC-CASE-23：#137985 的线圈电流与 Ip
export FYLITE_DEVICE_DIR=<fylite>/dist/facts/device/east
uv run --no-project --with numpy --with pyyaml python docs/examples/kinetic/kinetic_twin.py > kinetic.json
```

脚本是 `docs/examples/kinetic/kinetic_twin.py`，与校验册同一套孪生（`tools/benchmark-equilibrium.py`
...
- 同一条链在校验册里的记录：[外环](../../benchmark/reports/eq-reconstruct-kinetic-outer.md) ·
