---
title: 物理与数值 · Grad–Shafranov 平衡正解 (Physics & Numerics — The Grad–Shafranov Forward Solve)
subtitle: equilibrium.rs —— 定形边界、箱内定形与自由边界三种正解的方程、离散与迭代
---

(phys02-intro)=
# 引言：一个方程，三个正问题 (Introduction)

〔范围〕本章详述内核模块 `equilibrium.rs`（内核仓 `rust/fylite/src/`，约 2900 行）所实现的
**轴对称理想 MHD 平衡正解**：给定电流源（多项式、表格或解析族的 $p'$、$FF'$）与边界条件，
...
:::{table} `equilibrium.rs` 的三个正解入口及其数学性质。
:name: tbl-p02-three
:align: left
...
〔清洁室声明〕〔源码〕模块头部声明该文件按公开文献独立实现（GS 方程本身、教科书五点差分、
可分离椭圆算子的快速直接法、Solov'ev 解析解），其作者未阅读 EFIT 的 Fortran 源码；对
...
〔源码〕模块头部以此形式注明符号与单位：SI，COCOS 17（Sauter–Medvedev 约定
{cite}`sauter2013cocos`）。

(phys02-eq-gauge)=
## 两种磁通规范 (Two Flux Gauges)

〔源码〕本模块同时出现**两种** $\psi$ 规范，这是读本模块的第一道门：
...
〔源码〕源码注释记录了混用两种规范的实际后果："短一个 $g^2=4\pi^2$"，且该缺陷曾随某一功能
...
〔源码〕`DeltaStarSolver` 在均匀网格 $R_i=R_{\min}+i\,\Delta R$、$Z_j=Z_{\min}+j\,\Delta Z$ 上把
$\Delta^{\ast}$ 离散为
...
〔精确再现空间〕〔源码〕测试注释指出：该守恒格式对 $R^2$、$R^4$ 与 $Z^2$ **精确**（离散算子
...
{cite}`leveque2007fdm`；本仓源码称之为"textbook 5-point finite differences"。

(phys02-discrete-fast)=
## 正弦变换 + Thomas 算法的快速直接解 (Sine-Transform / Thomas Direct Solve)

〔算法〕〔源码〕Dirichlet 问题（边界值取自输入数组的四条边，内部被覆写）按下列四步直接求解：
...
〔复杂度〕〔源码〕显式 DST 为 $O(n_z^2)$ 每条 R 线；源码注明在 $65^2$ 网格上可忽略且免除 FFT
...
源码只写"Hockney's method, any numerical-PDE text"。Thomas 算法（三对角 LU）见
{cite}`press2007nr`。
...
〔源码〕`PolyProfiles`：$p'(\bar\psi)=\sum_k a_k\bar\psi^k$、$FF'(\bar\psi)=\sum_k b_k\bar\psi^k$，
Horner 求值。**注意**：此入口的 $p'$、$FF'$ 是对 $\bar\psi$ 的系数但**未除以 span**，且假定
每弧度规范——这是该入口与 `solve_fixed_box`（显式 `gauge`、显式除以 span）的差别，源码
注释把它记为历史遗留。

(phys02-fixed-iter)=
## 迭代格式 (The Iteration)

〔算法〕〔源码〕`solve_fixed_boundary` 每轮：
...
5. $\text{res}\le$ `tol` 即收敛；达到 `max_iter` 则**报告**（`iterations == max_iter` 且返回残差），
   **不抛错**。
...
〔用途〕〔源码〕`solve_fixed_box` 是为**输运—平衡交替**（{ref}`phys05-intro`）设计的：围绕一团
...
〔源码〕以 $g$ = `gauge`、$\text{span}_{pr}=\text{span}/g$、$x=\mathrm{clamp}(\bar\psi,0,1)$：
...
:::{important}
〔截断是方程的一部分〕〔源码〕$T(x)$ **不是**后处理：测试 `the_boxed_solve_satisfies_the_equation_it_names`
断言解出的 $\psi$ 满足的正是**含 $T$ 的** {eq}`eq-p02-box`。$w=0.05$ 的理由记在源码：输运梯子
默认止于 $\psi_N=0.95$、从不越过 $0.99$，故截断带落在输运从不采样的区间；一个在分离面上仍
"活着"的源会使掩膜逐轮外扩（测试 `a_source_alive_at_the_separatrix_no_longer_runs_away`）。
源码另记录两种更便宜的控制（在 X 点处裁真空室、放宽箱）**试过并否决**。$s^2(3-2s)$ 是三次
Hermite 平滑阶跃的标准形，源码未给出处。
:::

(phys02-box-ip)=
## $I_p$ 等式约束 (The Plasma-Current Constraint)

〔源码〕给定 `ip_target` 时，每轮把 $FF'$ 通道平移一个常数使总环向电流精确等于目标：
...
源码把这一步与"给定 $p'$ 与 $I_p$、解出 $FF'$ 常数"的 CHEASE 型定形边界求解器类比
...
〔源码〕磁轴先取上一轮掩膜的 Chebyshev 膨胀（半径 `dilate`）∩ 真空室内部上 $\mathrm{sign}\cdot\psi$
...
（`FIXED_BOX_TRUST`）的信任域内，收敛后不受限重泛洪一次，若触及箱边或越出信任域则**拒绝**。
真空室内的私有磁通团（与磁轴不连通）因此**不入掩膜**（测试
`the_plasma_is_connectivity_and_a_private_flux_blob_stays_out`）。

〔拒绝码〕〔源码〕`-2` 尺寸不合（`psi0.len() != n`、`nr<3`、`nz<3`）；`-3` 种子不在等离子体内；
...
〔结构〕〔源码〕$\psi=\psi_{ext}+\psi_{pl}$：$\psi_{ext}$ 为线圈（与被动导体）在网格上的真空场，
...
〔源码〕这是**直接 Green 函数求和**，不是 von Hagenow–Lackner 型的边界积分法；源码中不出现
...
〔解析族〕〔源码〕`AnalyticProfile {beta0, emp, enp, r0}`：
...
$j_c$ 每轮重定使总电流精确为 $I_p$；$1-\bar\psi^{e_{mp}}\le0$ 时取零。源码称其为"公开的
EFIT 型解析电流参数化（Lao 等的谱系，按已发表形式）"，即 Lao 等 1985 年论文中的
$(\beta_0,\,e_{mp},\,e_{np})$ 族 {cite}`lao1985efit`。

〔表格族〕〔源码〕`FreeProfile::Table {x, pprime, ffprime}`：形状 $S(R,x)=Rp'(x)+FF'(x)/(\mu_0R)$，
线性插值、端点钳制、分母以 $10^{-300}$ 保护；同样每轮按 $I_p$ 归一，因此表格的**规范**
（每弧度或 Wb）、总体符号与常数因子**全部除掉**——这是把表格与解析族置于同一 Picard 中的
必要条件。

〔表达力边界〕〔源码〕源码记录：EAST #137985 交付剖面的 $j_\phi$ 在 $\psi_N\approx0.82$ 处变号，
...
- **视线检验** `ray_sees_axis`：自候选接触点到磁轴的直线上按 $\lceil d/(0.5\min(\Delta R,\Delta Z))\rceil$
  ...
  $\text{tol}=\max(10^{-9},10^{-3}\abs{s\psi_a-sv})$。源码记录 $10^{-6}\abs{v}$ 的容差会误拒真正的
  限制器接触。
- **限制器接触磁通** `limiter_psi`：沿限制器折线每 $0.5\min(\Delta R,\Delta Z)$ 采样，取能看见磁轴的
  候选中 $\max(s\psi)$；可选**私有区守卫**排除 $\abs{Z}>z_{cut}$ 的壁点。〔源码〕守卫按 FreeGS
  的做法（源码给出其 `equilibrium.py:551`）{cite}`dudson_freegs`，有效 X 点候选在 $Z_x$ 时
  $z_{cut}=0.75\abs{Z_x}$。实测（EAST #137985，$65\times65$）：无守卫接触磁通 3.2762 Wb → 鞍点落在
  $\psi_N=1.213$（判为"限制器"）；有守卫 3.126 Wb → $\psi_N=1.000$（判为"偏滤"）。

〔X 点〕〔源码〕`find_xpoint`：对限制器内每个内部节点（索引 $2..n_r-2$）作中心差分梯度与
...
〔瓶颈规则〕〔源码〕`bottleneck_boundary` 定义每个格点的**最大瓶颈值**
...
$\min(s\psi_{\rm sample},\max_{4\ \text{nodes}}B)$，$\psi_b=\max$ 取遍样本。源码注明：**限制与偏滤
用同一条规则**——最后闭合磁面是"能从磁轴连通到达的最深磁通水平"，限制器 / X 点二分法
由此消失。〔已确立〕最大瓶颈（最宽路径）问题是最短路问题的极小—极大变体，Dijkstra 型算法
{cite}`dijkstra1959note` 把 $+$ 换成 $\min$、$\min$ 换成 $\max$ 即得；源码未给出处。

〔综合判定〕〔源码〕`judge_boundary`：磁轴 = 真空室内 $s\psi$ 最大节点（有等离子体后限制在
...
:::{note}
〔为什么需要反馈〕〔源码〕固定电流的 Picard 映射对刚性垂直模是**不稳定**的——源码引 Jardin
...
下垂因子 0.5 为**本仓选择**，源码未给文献。
:::

〔启动〕〔源码〕真空场**没有内部极值**（$\Delta^{\ast}\psi_{ext}=0$ 服从极大值原理〔已确立〕，
椭圆方程的极值原理见 {cite}`evans2010pde`），故第一轮以真空室质心处半径
$0.3\cdot\min(L_R,L_Z)/2$ 的均匀电流盘（按 $I_p$ 归一）起步；给出热启动场时改用其内部极值位置。

〔闩锁与斜坡〕〔源码〕`zc_anchor` 有限且残差 $<5\times10^{-2}$ 时记 `ramp_at`，
$w_{los}=\min((it-\text{ramp\_at})/150,1)$；守卫闩锁同阈值。源码记录无记忆的 `residual < 5e-2`
门会把过渡本身变成守卫开/关的极限环；无锚定的解保留旧规则，其失败形态被记为"角落乒乒
$\pm3.9\times10^6$ A → 极限环 → 在错误的壁上以 $-787$ kA 反馈收敛"。

〔拒绝码〕〔源码〕`-7` 限制器不包含任何节点；`-6` 掩膜电流为空或相消
...
源码给出两个实测量级作为该分级的依据：抖动地板 $2\times10^{-3}$，周期 2 极限环 $1.3\times10^{-1}$。
...
〔源码〕以下为 `equilibrium.rs` 单元测试所钉住的定值，是本章陈述的证据：
...
Solov'ev 解族的原始出处为 {cite}`soloviev1968theory`（源码只写"Solov'ev closed-form solution"）。

(phys02-asbuilt)=
# 与内核的对应 (Correspondence to the Kernel)

:::{note}
〔口径〕本表按 2026-09-02 的源码快照写成，随实现漂移；C-ABI 入口名以 `c_api.rs` 为准。
`rust/…` 路径在内核仓下成立。
:::

:::{table} 本章内容与内核函数、C-ABI 入口、Python 入口的对应。
:name: tbl-p02-asbuilt
:align: left

| 本章内容 | 内核函数（`equilibrium.rs`） | C-ABI（`fylite_rs_*`） | Python / 页面 |
| :--- | :--- | :--- | :--- |
| 离散 $\Delta^{\ast}$ 与直接解（{ref}`phys02-discrete`） | `DeltaStarSolver::new/solve`, `apply_deltastar` | `deltastar_solve`, `deltastar_apply` | `fylite.kernel` |
| 定形 Picard（{ref}`phys02-fixed`） | `solve_fixed_boundary` | `gs_fixed_solve`（out5 = $\psi_a$, $R_a$, $Z_a$, $I_p$, res） | `fylite.kernel` |
| 箱内解（{ref}`phys02-box`） | `solve_fixed_box`, `BoxSource`, `BoxOptions` | `gs_fixed_box`, `gs_fixed_box_ip`（out8 增 `ff_shift`, `ip_raw`） | 输运精化外环（{ref}`phys05-intro`） |
| 自由边界（{ref}`phys02-free`） | `solve_free_boundary{,_from,_shaped}` | `gs_free_solve`（out12，verdict 0/1/2）, `gs_free_solve_tab`（out14 增 `mask_delta`, `jc`） | `fylite.kernel.gs_free_solve`；`S.model.coupled`；`S.control.evolution` |
| 边界判定（{ref}`phys02-free-boundary`） | `limiter_psi{,_guarded}`, `find_xpoint`, `bottleneck_boundary`, `judge_boundary` | `boundary_flux`（事后瓶颈规则） | — |
| Green 边界 | `BorderGreen` | —（内部） | — |
:::
