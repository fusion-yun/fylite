...
排序截到 **2** 个。`strike_points`（`code/summary` 的 `strike` 行；扁平入口自 2026-09-06 仅神谕构建可见）：`contour(psi_bnd)` 线段与壁折线求交（平行守卫 $\abs{\text{den}}<10^{-30}$）。
...
| :--- | :--- | :--- |
| 描迹磁面与磁面积分 | `fyo:equilibrium` 的 `profiles_1d`（面量） | `code/summary` · `code/outlines`（内核 `trace_surface` 算子，自 2026-09-06 仅神谕构建可见） |
