---
title: 0-D 放电模型 (The 0-D Discharge Layer)
subtitle: 规定剖面档（Tier A）、约束标度律能量平衡档（Tier B）、运行域界限与磁通预算
---

(phys06-intro)=
# 引言：两档不同性质的答案 (Introduction — Two Tiers)

〔范围〕本章详述零维放电层。它有两档，两档给出的是不同性质的答案：
...
- **Tier B（预测，实验性）**：以约束时间标度律闭合能量平衡 $\dd W/\dd t=P_{\rm heat}-W/\tau_E$，求 $W_{th}$
  与中心温度。实现标注 "Status: this tier is an EXPERIMENT（ADR-116 OI-1）"。

〔出处姿态〕〔实现〕模块头部："凡携带物理常数或取自文献的闭式者在此"，并**逐条给出**标度律与
...
$\epsilon$ = `edge_frac`（缺省 0.05），$p$ = 峰化因子（密度缺省 1.0、温度 1.5）。实现未注出处；
$(1-\rho^2)^p$ 族是 0-D 建模的惯用形状（如 {cite}`uckan1990guidelines`〔凭记忆〕），系数是**本仓缺省**。

〔平均与体积〕〔实现〕体积平均 $\expval f=\int f\,2\rho\,\dd\rho/\int2\rho\,\dd\rho$（**圆截面**权 $\dd V\propto\rho$），
体积 $V=2\pi^2R_0a^2\kappa$（**椭圆截面**），线平均 $\bar f=\int_0^1f\,\dd\rho$（中平面弦）。实现注明
几何约定的不一致（FYL-DESIGN-05 O-4）："圆近似配椭圆体积"。表面积 $S=4\pi^2R_0a\sqrt{(1+\kappa^2)/2}$
（椭圆周长的 $2\pi a\sqrt{(1+\kappa^2)/2}$ 近似；"Martin 阈值对等离子体表面回归"）。
〔METIS oracle〕〔实现〕椭球体积对 METIS 认证表（84 片）：圆片精确到 0.2 %，成形等离子体高估 $<17\%$
...
表 VII {cite}`boschhale1992fusion`（实现逐字引）。
...
实现对这三式**均未注出处**，只写"Spitzer 电阻率与通常的 $(1-\sqrt\epsilon)^{-2}$ 捕获粒子修正"，
并自评"好到几十个百分点——够量磁通预算，不够替代电流扩散解"。

〔不含〕〔实现〕0-D 层 无自举份额、无非感应份额、无 $\sigma_{\rm neo}$ 剖面；$Z_{\rm eff}$ **只**经 $\eta$ 进入。
`flux_budget` 与 `loop_voltage_ohmic` 用**同一个** $L_p$（测试钉到 $10^{-18}$）。

(phys06-waveforms)=
# 时间形状：相位与波形 (Phases and Waveforms)

〔实现〕四相 `breakdown / rampup / flattop / rampdown`；梯形波形 `trapezoid(ph, t, flat, start, end)`，
斜坡分母下限 $10^{-9}$ s（"退化相位给阶跃而非无穷"）；三个中心波形只差起止值：$I_p$ (0, 0)、
$n_e$ (2 % flat)、$T_e$ (1 % flat)——"这两个分数就是模型"（避免斜坡相除零）。执行器 `actuator` 在
$[t_{\rm on},t_{\rm off}]$ 内取常数。实现未注出处（工程约定）。

(phys06-tierA)=
# Tier A：规定评估 (Tier A — Prescribed Evaluation)

〔实现〕`evaluate`：逐时刻由 `ne0[k]`、`te0[k]` 与 {eq}`eq-p06-profile` 造剖面，$T_i=T_e\cdot$`ti_over_te`；
$P_{\rm fus}$、$P_\alpha$；$\expval{T_e}$；$V_{\rm loop}$（$\dd I_p/\dd t$ 由 `gradient`）；
...
〔实现〕以 $I$ 取 MA、$P$ 取 MW、$\epsilon=a/R_0$：
...
〔出处〕{eq}`eq-p06-ipb98`：ITER Physics Basis 第 2 章 Eq. (20) {cite}`iterphysicsbasis1999ch2`（实现逐字引）；
{eq}`eq-p06-iter89`：Yushmanov 等 {cite}`yushmanov1990scalings`（实现逐字引）。"系数取自各变体所引出版物；
...
〔出处〕Martin 等 {cite}`martin2008power`（实现逐字引：J. Phys. Conf. Ser. 123 (2008) 012033）。任一输入非正 → 0。
锚：$S(6.2,2.0,1.85)\in(600,900)$ m²；$P_{LH}(0.5\times10^{20},5.3\,\text{T},S)\in(20,90)$ MW（"约 50 MW"）。

(phys06-tierB-march)=
## 能量方程的推进 (Marching the Energy Equation)

〔实现〕`predict`：每步 (1) $n_e(\rho)$；$T_{e0}$ 由 **K-3 归一算子**
...
{cite}`hochbruck2010exponential`〔凭记忆〕）；实现指出隐式 Euler "以 $\ln(1+\Delta t/\tau)/\Delta t$ 而非 $1/\tau$ 松弛"。
$\tau_E$ 与 $P_\alpha$ 在步**起点**求值——耦合显式，"一次松弛扫描，不是 METIS 的定点"。
`balance` 报告每一项可取回且求和闭合（"不是积分器对预算"）。

:::{warning}
〔代码观察〕〔实现〕`predict` 把 **体积平均** 密度传给 `tau_e` 与 `p_lh_threshold` 的 `ne_bar`，而
约束标度律的密度输入自述为"线平均电子密度"、且线平均量存在（仅在 `limits` 由调用方用）。
对 $(1-\rho^2)$ 剖面两者相差 $2/3$ 对 $1/2$。这是本章标出的**待裁定项**。
:::

〔不含〕〔实现〕无隐式代数 $\dd W/\dd t$ 步、无 $\tau_E(W)$ 阻尼定点、无 $\chi_0=K/(W-W_0)$ 形状闭式（形状是固定
$(1-\rho^2)^p$ 族，幅值由 K-3 归一）、无 He 灰、无稀释（$n_i=n_e$，"上界"）、无辐射损失。
`GK-TMT-01` 所述的这些结构在本内核未实现。

(phys06-limits-domain)=
# 运行域界限 (Operating-Domain Limits)

〔实现〕`greenwald_density`：$n_{GW}=\frac{I_p[\text{MA}]}{\pi a^2}\times10^{20}$ m⁻³ {cite}`greenwald1988density`
（实现逐字引；"对线平均密度的经验破裂边界"）；`q_cylindrical`：
...
{cite}`uckan1990guidelines`〔凭记忆〕；实现未注。`limits`：$\expval p=\tfrac23W_{th}/V$、
...
〔实现〕`EJIMA_C = 0.45`——"Ejima 等，NF 22 (1982) 1313：跨装置回归，0.45 是全斜坡通常取值"
{cite}`ejima1982volt`（实现逐字引）。$t_{\rm sustain}=\max((\Phi_{\rm avail}-\Phi_{\rm ramp})/V_{\rm flattop},0)$；
...
8. Greenwald 与 Martin 都要求线平均密度——调用方须给对。
...
:::{table} 0-D 层两档的产出，及其所落的 fyo 数据集。
...
| 运行域界限 | `fyo:summary`：Greenwald 密度、柱位 $q$ 等界限 | `S.design.feasible` |
| 磁通预算 | `fyo:pulse_schedule` / `fyo:summary` 的磁通账 | `S.design.discharge` |
:::

(phys06-sources)=
# 来源与出处 (Sources & Attribution)

〔一手文献（实现逐字引）〕Bosch–Hale {cite}`boschhale1992fusion`；IPB98(y,2) {cite}`iterphysicsbasis1999ch2`；
...
〔一手文献（编者对应，实现未注）〕Spitzer 电阻率 {cite}`spitzer1953transport,huba2013nrl`；捕获修正与
环电感 {cite}`wesson2004tokamaks`；$q_{\rm cyl}$、$(1-\rho^2)^p$ 族 {cite}`uckan1990guidelines`；
指数积分器 {cite}`hochbruck2010exponential`。标 〔凭记忆〕 者为编者补出的对应，条目字段的核验状态见 `references.bib` 的 `note`（{ref}`phys00-evidence`）。
