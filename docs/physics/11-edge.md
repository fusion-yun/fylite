---
title: 边界层、中性粒子与台基 (Edge, Neutrals and Pedestal)
subtitle: 非日冕 Mavrin 拟合、扩展 Lengyel 两点模型（正 / 反解）、一维柱 Monte-Carlo 中性粒子、EPED1-NN 台基代理
---

(phys11-intro)=
# 引言：芯部结果可信度的上界 (Introduction)

〔范围〕本章详述边界层向芯部提供的三项输入：Mavrin 2017 非日冕原子数据评估与 Body 等 2025
**扩展 Lengyel 模型**（闭式、两点态、反解与正解不动点，随附生成的 Mavrin 系数表）；一维柱几何的
解析 Monte-Carlo 中性粒子输运；EPED1-NN 台基代理。它们分别给芯部输运
（{ref}`phys05-intro`）供**边界温度**、**中性源**与**台基顶 Dirichlet 边界**。

〔出处姿态〕〔实现〕边界层是 TORAX（google-deepmind/torax，Apache-2.0）扩展 Lengyel 实现的转录，其常数刻意取
TORAX 的值而非 CODATA-2018（"对齐这些把闭式从 $3\times10^{-7}$ 挪到 $10^{-16}$"）；中性粒子层是**清洁室**
（未读 EIRENE / FRANTIC 实现，方法为标准解析 MC + Woodcock 抽样，速率取公开拟合）；台基层逐行转录
EPEDNN.jl（Apache-2.0）。实现逐字引的文献较完整（Mavrin、Body、Eich、Stangeby、Voronov、Freeman–Jones、Snyder、Meneghini）；
只给姓名的：Brown & Goldston 2021、Verdoolaege 2021、Kallenbach 2024。

〔未实现（明列）〕〔实现 grep〕Eich 2013 $\lambda_q$ 回归 #14/#15、Goldston 漂移 $\lambda_q$、展宽因子 $S$ 与靶板热流卷积、
显式脱靶判据、日冕冷却曲线用于边界、Janev 电离拟合、解析中性穿透长度、加料源形状、SOL 再循环模型、EPED 的
KBM 宽高关系**作为方程**、剥离—气球代理——均不在内核中。理论手册 `GK-TMT-07` 所述的这些结构在本内核只有
下文所列的子集。

(phys11-mavrin)=
# 非日冕原子数据：Mavrin 2017 (Non-Coronal Atomic Data)

〔实现〕八种低 Z 杂质（He, Li, Be, C, N, O, Ne, Ar）的平均电荷态 $\expval Z$ 与冷却率 $L_z$ 作为 $T_e$ **与**滞留参数
$n_e\tau$ 的函数：
...
区间选择为 numpy `searchsorted` 缺省侧（边界**严格低于** $T$；实现记录 `>=` 版本在 896 点中错 119 点、最差 4.8×）。
三条钳制"是模型的一部分"：$T_e$ 钳入物种拟合域、$n_e\tau$ 在 $10^{19}$ m⁻³s 饱和（日冕极限）、不在表内的物种**辐射为零**
（"对边界重杂质的建模陈述"）。$L_{\rm INT}=\int L_z\sqrt T\,\dd T$ 在对数网格上梯形求积（分辨率无缺省；解算器用 100）。
〔出处〕Mavrin 2017 {cite}`mavrin2017noncoronal`（实现逐字引，含 DOI）；$L_{\rm INT}$ 为 Body 等 Eq. 34 {cite}`body2025lengyel`。
...
〔出处〕$\alpha_t$ 与 $\lambda_q$ 的湍流展宽标度 Eich 等 2020 Eq. 9 {cite}`eich2020turbulence`（实现逐字引）；$\kappa_e(Z_{\rm eff})$
{cite}`brown2021conductivity`〔凭记忆：题名待核验〕；`REGULARISER_EPS = 1e-7` 是上游 `CONSTANTS.eps`（"正则子，不是机器 ε"）。
$q_{\rm cyl}$、pitch、$(1-1/e)$ **实现未注出处**。TORAX oracle：36 点最差 $1.5\times10^{-15}$。

(phys11-lengyel-twopoint)=
## 两点态：沿磁力线的温度与靶板 (The Two-Point State)

〔实现〕对流层经验拟合（"Stangeby 2018 PPCF 60 044022 Eq. 33 的形状，系数上游拟合"）$f(T_t)=1-A(1-e^{-T_t/w})^s$，
...
实现在 `required_power_loss` 反解 $f_{\rm pow}$、在 `forward_t_e_target` 正算 $T_t$（**此处且仅此处** $f_{\rm mom}$、$f_{\rm pow,conv}$
...
〔实现〕"给鞘层入口目标温度，需要多少播种杂质——'要吹多少氮'"（Body §5，Eqs. 33, 40, 42）：
...
〔实现〕"上游自注一组输入可容多解……此处不动点只找一个"；$q_\parallel$ **欠松弛 0.4**（$T_t$、$\alpha_t$ 的松弛因上游对象别名
而为空操作——"刻意再现"）；
...
上游可微——"钳制与平滑根在答案上不同：完全脱靶例此处 $2\times10^{-5}$–$4\times10^{-4}$ eV，钳制下恰为零"）。
$q_{cc}^2\le10^{-7}$ 即报 `QccSquaredNegative`——脱靶标志。TORAX oracle：10 例最差 $1.3\times10^{-13}$，4 例脱靶；
...
〔声明的约化〕〔实现〕无限圆柱 $a=r_{\rm edges}[-1]$（成形、偏滤器、极向源定位不在）；只有原子（分子经发射能量 `e0_ev`
...
〔出处〕电子碰撞电离：Voronov 1997 氢行（$A=0.291\times10^{-7}$ cm³/s，$P=0$，$K=0.39$，$X=0.232$）{cite}`voronov1997fit`（实现逐字引）；
电荷交换：Freeman–Jones 拟合 {cite}`freeman1974atomic`（实现逐字引 CLM-R 137）。$T_e\le0.1$ eV 电离率取 0；$E$ 钳到 $[0.1,10^5]$ eV/amu。

〔算法〕〔实现〕解析 MC + **Woodcock δ 跟踪**：$\nu_{\max}=\max_j\nu_j$，飞行长 $s=-\ln u/\nu_{\max}\cdot v$，虚碰撞按 $u\nu_{\max}\ge\nu_j$；
...
守卫：单个历史 $>10^5$ 步 → 中止；错误码 `-2` 于 $n<2$、`n_particles = 0`、非严格增 `r_edges`。

〔EIRENE oracle（JINTRAC 101612）〕〔实现〕3 eV 发射**穿透不足**（10 % 电离深度 $x=0.96$ 对 EIRENE 0.78），434 eV 发射**穿透过头**（0.56）；
...
〔模型〕〔实现〕EPED {cite}`snyder2009development,snyder2009pedestal,snyder2011eped` 由两个约束（ELITE 的非局域剥离—气球模起始、
...
〔算法（逐行自 `EPEDNN.jl::pedestal_array`）〕〔实现〕输入 $[a,\beta_N,B_t,\delta,I_p,\kappa,m,n_{e,\rm ped},R,Z_{\rm eff,ped}]$；$\delta\to\abs{\delta+1}$；
...
〔与已发表值的对照〕〔实现〕DIII-D 132003 / 132017（NF 49 085035 p. 6 的输入、图 5 像素级读数）在 ±20 % 带内；ITER 基线
$p_{\rm ped}$ 79.0 kPa 对 92 kPa（0.859）、$\Delta$ 0.033 对 0.04（0.826）、$\beta_{N,\rm ped}$ 0.500 对 0.6——**六个比值系统性偏低
（0.83–0.97）**，候选原因列三条、无一断言。实现记录一次**错引**（2026-08-25 前把代理自身答案归于 NF 49）已订正。
...
7. **常数刻意非 CODATA-2018**（边界层），与 中性粒子层（CODATA-2018）不一致——两模块间不可混用常数。
...
:::{table} 边界层给出的是芯部的边界条件，而非 SOL 内的解。
...
| 台基（EPED1-NN） | `fyo:core_profiles`：台基顶压强与宽度 | `S.model.evolve(pedestal=True)` |
| 杂质富集 | —（边界条件的标度修正） | `edge.enrichment_kallenbach` |
:::

(phys11-sources)=
# 来源与出处 (Sources & Attribution)

〔一手文献（实现逐字引）〕Mavrin {cite}`mavrin2017noncoronal`；Body 等 {cite}`body2025lengyel`；Eich 等 {cite}`eich2020turbulence`；
...
GELU {cite}`hendrycks2016gelu`；FUSE {cite}`meneghini2024fuse`；TORAX {cite}`citrin2024torax`。标 〔凭记忆〕 者为编者补出的对应，条目字段的核验状态见 `references.bib` 的 `note`（{ref}`phys00-evidence`）。
...
〔实现未注出处〕$q_{\rm cyl}$、pitch、$(1-1/e)$、对流层三组系数（"上游拟合"）、`smooth_sqrt` 形式与 $\varepsilon=10^{-3}$、
