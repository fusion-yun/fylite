---
title: 脉冲设计与击穿 (Pulse Design and Breakdown)
subtitle: 等磁通起点设计、前馈电压（演化的逆）、GSPulse 型全程二次规划、真空零点设计与磁通预算
---

(phys13-intro)=
# 引言：等离子体存在之前与位形之外 (Introduction)

〔范围〕本章详述由目标位形反求线圈电流与电压的三部分内容：**起点**电流设计、丝填充与前馈电压；真空零点设计的
行装配、通道场、带界最小二乘与达成质量报告；以及装配层的 GSPulse 型全程轨迹设计与退火式放电设计
（`fylite.scenario.design`）。磁通预算与欧姆环电压在 {ref}`phys06-current`、
{ref}`phys06-flux`。

〔两个线性问题〕〔实现〕两模块的共同点：**没有 GS 解**。起点设计"是线性的：等离子体是固定的丝云……不是平衡、也不声称是"；
零点设计"纯真空——无等离子体、无 GS、无环向场，场对电流线性，整个设计是一次小最小二乘"。它们回答的是**在哪里开始**
（退火 / 自由边界解需要的起点）与**能否雪崩**（一个可雪崩的极向场零点加足够的磁通预算）。

〔未实现（明列）〕〔实现 grep〕Townsend 雪崩判据 $\alpha(E,p)$ 与 Paschen 常数、连接长度 $L=0.25a_{\rm eff}B_\phi/B_\perp$、
...
〔理由〕〔实现〕"形状设计在两个宿主里都是**退火**：解自由边界平衡、读它找到的边界、用一步岭最小二乘修正通道、重复。
那是局域方法……实测四台装置：有参考炮的一台落在目标 3 cm 内，另三台差 0.36–0.77 m 而状态行写着'反解完成'。"
起点设计把问题线性化。

〔行族〕〔实现〕未知量 $I_c$ [A·turn]，逐通道响应 $\psi_c$、$B_{r,c}$、$B_{z,c}$（`channel_field`），等离子体场由丝云
经同一 `element_response`（"使等磁通行两侧可比"）：
...
〔求解〕〔实现〕响应标度的 Tikhonov：$g_{\rm scale}=\sqrt{\sum a^2/(n_{\rm row}n_{\rm ch})}$、$\lambda_{\rm eff}=\lambda g_{\rm scale}$
...
〔已确立〕等磁通 / 零点行的线性化线圈设计属经典的"等磁通控制"思想 {cite}`hofmann1990isoflux`〔凭记忆〕；实现未注。

〔丝填充〕〔实现〕`fill_filaments`：以边界质心为中心按 $s_k=(k+\tfrac12)/n_{\rm ring}$ 缩放边界（"成形目标一直到轴保持形状"），
权 $w_{k,i}=(1-s_k^2)^{\rm peaking}s_k$（$s$ 为环周长 Jacobian，实现未注），$a=I_pw/\sum w$。"起点模型，仅此——无平衡在内"。

(phys13-feedforward)=
# 前馈电压：演化的精确逆 (Feed-Forward Voltages — the Exact Inverse of Evolution)

〔实现〕给定通道电流轨迹 $x_k$，求电压与感生被动电流：
...
$k=0$：$V_{c,0}=R_cx_{c,0}$（"稠态电压而非变率"），$y_0=0$。"这是 `evolve` 的精确逆——同一隐式 Euler、同一区间末样本——
...
〔实现〕`design_trajectory`："算法形状取自 GSPulse（J. Wai / CFS，MIT）：外层全程二次规划与逐时刻平衡更新交替。两处刻意不同：
...
3 cm 间隙斜坡 50 ms 内在 EAST 上不可行（12 通道 7 个越压，最劣 8.5×），300 ms 可行——"电源电压限制的是形状变化的速率"。
2026-09-02 记录：numpy 的 `bounded_lstsq` 孪生在 `flux_target = 1.0` Wb 上撞 4000 次上限、残差 $1.08\times10^3$ 对内核 $6.7\times10^{-1}$，已删除。

〔退火〕〔实现〕`discharge`：几何调度 $\alpha_i=0.10(0.005/0.10)^{i/7}$（8 遍）、更新 $\gamma=0.4$；每遍自由边界解 → 描迹 → 形状矩 →
...
〔行族与归一〕〔实现〕未知量 $I_c$；采样盘：中心 + $n_{\rm ring}$ 环 × $n_\theta$ 角（缺省 4 × 16 = 65 点，半径 0.3 m——
...
:::{important}
〔归一是全部机制〕〔实现〕"零点行带特斯拉（$\sim10^{-3}$），磁通行带韦伯（$\sim10^{-1}$）。不归一则磁通项压倒零点，'设计'出来的是
均匀场而非零点——且不报错。两族各除以自己的容差（场除 $b_{\rm tol}$、磁通除目标），残差 1 在任一族都意味'达容差'。"
用户指南："`max ≈ rms ≈ 中心` 正是它的征兆"。
:::

〔求解与守卫〕〔实现〕`bounded_lstsq`，`MAX_ITER = 50_000`——"4000 不够且**静默失败**：EAST deck 上箍紧箱到一个通道绑定时需 **32 273**
...
`at_bound`（$\ge i_{\max}(1-10^{-3})$）。未收敛返回错误 **−3**（末迭代仍写出）。

〔可行性裁决（Python）〕〔实现〕`feasible = null_ok ∧ flux_ok ∧ 无越流通道`，理由码 `channel_limit` / `flux_not_met_at_channel_limits` /
...
:::{table} 脉冲设计与击穿各项的产出，以及它们所落的 fyo 数据集。
:name: tbl-p13-asbuilt
:align: left

| 内容 | 结果落在 fyo 的哪里 | Python 入口 |
| :--- | :--- | :--- |
| 起点电流设计与丝填充 | `fyo:pf_active`：起点线圈电流 | `S.design.start_state`、`discharge` |
| 前馈电压（演化的逆） | `fyo:pulse_schedule`：全程电压波形 | `S.design.pulse.design_trajectory` |
| 场零区设计与质量判据 | `fyo:plasma_initiation`：零场区几何与击穿判据 | `S.design.breakdown`、`feasible`、`loop_voltage` |
| 带界最小二乘 | —（设计问题的解法器，{ref}`phys01-intro`） | `fylite.kernel.bounded_lstsq` |
| 通道限值 | `fyo:pf_active` 的电源限值（装置文档给） | `pulse.channel_limits` |
:::

(phys13-sources)=
# 来源与出处 (Sources & Attribution)

〔一手文献（编者对应；实现未注）〕等磁通线圈设计 {cite}`hofmann1990isoflux`；击穿的 Townsend 判据（未实现，供读者外加）
