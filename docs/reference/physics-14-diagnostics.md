---
title: 物理与数值 · 剖面拟合与合成诊断 (Physics & Numerics — Profile Fitting and Synthetic Diagnostics)
subtitle: fitting.rs · diagnostics.rs —— 移位 Legendre 基 + GCV 定阶、PCHIP、解析族提取；弦几何与线积分、Fourier–Bessel 层析、磁矩拟合、逐道自校准
---

(phys14-intro)=
# 引言：观测算子按数学结构分族 (Introduction)

〔范围〕本章详述 `fitting.rs`（约 480 行：加权最小二乘剖面拟合 + 广义交叉验证定阶、单调三次插值 PCHIP、解析剖面族的
Levenberg–Marquardt 提取、g 文件剖面回读）与 `diagnostics.rs`（约 1120 行：三维直线弦、$\psi_N$ 双线性采样、三种求积、
包含判据、线积分与收敛细化、针孔相机角、单丝电流质心的磁矩拟合、Bessel 函数与零点、Fourier–Bessel 层析基与响应行、
反转半径、逐道自校准统计）。磁类观测算子的 Green 响应行在 {ref}`phys12-em-response`、{ref}`phys03-rows-mag`。
...
〔出处姿态〕〔源码〕两个模块只引 Abramowitz–Stegun 9.2.5（Hankel 展开）与 "Fritsch-Carlson"、"SciPy"、"sxht7"（HT-7 软 X 层析
MATLAB 码）、"Levenberg–Marquardt"；其余公式无文献。本章补出处并标核验状态。

(phys14-fit)=
# 剖面拟合：移位 Legendre 基 + GCV (Profile Fitting)

〔基〕〔源码〕$u=1-x^2$、$t=2u-1\in[-1,1]$，Legendre 三项递推 $P_{k+1}=\frac{(2k+1)tP_k-kP_{k-1}}{k+1}$——"**基是 $u=1-x^2$ 中的移位 Legendre，
...
〔加权最小二乘〕〔源码〕$A_{pq}=\sum_iw_i^2b_p(x_i)b_q(x_i)$、$r_p=\sum_iw_i^2b_p(x_i)y_i$、$Ac=r$（Cholesky；非正定 → `None`，
"关于**矩阵**的陈述，不是数值意外"）；$w_i=1/\sigma_i$，$\sigma_i\le0$ 的点**屏蔽**。

〔GCV 定阶〕〔源码〕对每阶 $n$（$p=n+1$ 参数、$N$ 个有效点）：
...
{cite}`craven1979smoothing,golub1979gcv`（源码未注）；对线性投影 $\mathrm{tr}H=p$ 精确，"不带自身的调节常数"。
...
〔PCHIP〕〔源码〕"单调三次（PCHIP）插值——Fritsch–Carlson"：内点斜率为割线的加权调和平均
...
{cite}`fritsch1984method`〔凭记忆〕；SciPy 实现 {cite}`virtanen2020scipy`。理由："拟合器存在是为了平滑的**梯度**，线性插值的导数是阶梯"。

〔解析族提取〕〔源码〕`shape_fit`：$f(x;a,b)=[\max(1-x^a,0)]^b$ 对归一剖面作 Levenberg–Marquardt（$\lambda_0=10^{-3}$，
...
〔源码〕"一个实现回答三个问题：视线去哪、每点看到什么 $\psi_N$、$\int f(\psi)\dd s$ 是多少。"
直线弦 $\vb P(s)=\vb o+\hat d\,s$，$R=\sqrt{x^2+y^2}$（"切向弦与其极向投影不同"）；$\psi_N$ 双线性（Z 先 R 后，格外 $+\infty$——
"NaN 会毒化求和，钳制会在壁上发明等离子体"）。

:::{important}
〔$\psi_N\le1$ **不是**包含判据〕〔源码〕"全网格箱上的 $\psi$ 在等离子体外不单调——极向场线圈在那里造出结构——$\psi_N$ 在远离
约束区处回落到 1 以下。EAST 例上 LCFS 顶在 $z=0.47$ m，而 $z=1.35$ m 的水平弦仍找到 $\psi_N=0.07$ 的样本。"
包含 ⇔ $\psi$ 有限 ∧ $\psi\le\psi_{\max}$ ∧ 在 LCFS 多边形内（少于 3 顶点 = 无多边形，"是**告诫**不是缺省"）。
:::

〔求积〕〔源码〕`Rule`：Simpson（$O(h^4)$；奇数区间时末段补梯形——"丢末样本静默缩短弦，读作物理差"）、梯形、"中点"
...
`line_integral_converged`：样本数加倍直到 $\abs{I_n-I_{n/2}}/\max\le r_{\rm tol}$，**无 Richardson 外推**（"答案**就是**那个计数的定采样积分"），
不收敛**报告**而不隐藏（未导出 C-ABI）。针孔相机 $\theta_i=\arctan\frac{(n/2-i+\tfrac12)\text{pitch}}{f}+\theta_{\rm view}+\pi$
...
无一弦看见等离子体 → `None`；"矩阵随平衡改变须**重建**——它是图的性质不是诊断的"）。
〔出处〕〔已确立〕磁面基 + 极向谐波的 SXR 层析是 JET 等装置的经典做法 {cite}`granetz1988tomography`〔凭记忆〕；
Zernike / Cormack 型正交基 {cite}`cormack1964representation`〔凭记忆〕；源码只注 "sxht7"。

〔Bessel〕〔源码〕$\abs x<12$：80 项升幂级数；$\abs x\ge12$：Hankel 渐近展开（**源码引** Abramowitz–Stegun 9.2.5，$P$/$Q$ 级数
{cite}`abramowitz1964handbook`），项**缩小时取**（渐近展开的标准规则）；对 SciPy 在 $x\in[0.01,60]$、$m\le3$ 内 $3\times10^{-12}$；
源码记录只取首项在 $x=12$ 第三位即错、$J_2$ 零点偏 0.13。零点：均匀扫描 $[10^{-6},4+n\pi+m]$ + 80 次二分。
...
$\sqrt{(\sum c_{2l}J_1)^2+(\sum c_{2l+1}J_1)^2}$ 的极大位置（单模锚 $j'_{1,1}/j_{1,1}=0.4805$；未导出 C-ABI）。
...
〔源码〕一根携带实测 $I_p$ 的环向丝；$(B_R,B_Z)$ 由 `element_response` 的退化单元（{ref}`phys12-em-response`）；LM 同上（$\le100\times30$，
...
{cite}`shafranov1971determination,hutchinson2002principles`；单丝偶极模型源码未注。锚：植入质心复原到 $10^{-6}$ m；1 % 噪声 5 mm 内。

(phys14-selfcal)=
# 逐道自校准统计 (Per-Channel Self-Calibration Statistics)

〔源码〕"这些是**稳健统计**，哪一个放在哪里是方法的全部……逐道因子是跨时片的**中位数**，因为少数坏片不得移动它；判断某道是否
突出的散布是**最大值**，因为那里少数**就是**信号——$\mathrm{MAD}([1,1,2,1])=0$。"$f_i=c_i/m_i$，$\tilde f=\text{median}$，
$\text{keep}_i\iff\abs{f_i/\tilde f-1}\le\text{tol}$（缺省 0.15；"中位数吸收任何全局尺度或单位偏移，只有道间**相对**不一致能拒绝一道"）；
跨片：`factor = median`、`scatter = MAD/|median|`（无 1.4826）；`factor_dispersion = max|f/median − 1|`（"刻意**不是**稳健散布"）。
无 $\chi^2$。宿主（已退役至 git）以 SVD 秩一分量取平衡成分。〔出处〕稳健统计的中位数 / MAD {cite}`huber1981robust`〔凭记忆〕；
源码只注 "sxht7"。前提："`computed` 须来自**未用**可疑道的拟合——否则拟合已吸收其增益误差，因子塌向 1。"
用户指南保真度章：EAST 逐道标定（`RWTMP2`）不可得是全探针拟合发散的根源。

(phys14-point)=
# POINT 偏振—干涉仪的约化与 Faraday 常数（宿主） (POINT Reduction — Host Side)

〔Python / 装置文档〕$\lambda=432.5$ µm、$C_F=2.62\times10^{-13}$（装置文档数据，源码禁止字面量）；Faraday 角 → $\int n_eB_{\rm pol}\dd l$
的目标量 $\texttt{bpolar}=k_{\rm pol}\theta_F[\text{rad}]/(2C_F\lambda^2\cdot10^{19})$——**因子 ½ 的来源未在源码说明**（"GUI line 424"）；
...
:::{table} 拟合 / 诊断内容与内核函数、C-ABI、Python 入口（2026-09-02 快照）。
:name: tbl-p14-asbuilt
:align: left

| 内容 | 内核函数 | C-ABI（`fylite_rs_*`） | Python |
| :--- | :--- | :--- | :--- |
| 剖面拟合 | `fitting::fit`, `fit_order`, `basis`, `evaluate` | `profile_fit`（info3）, `profile_sample` | `S.analysis.profit` |
| PCHIP | `fitting::pchip` | `pchip`（`-5`） | `kernel.pchip` |
| 解析族 | `shape_fit`, `gfile_profile` | `profile_shape_fit`, `gfile_profile` | — |
| 弦 / 采样 / 求积 | `chord_samples`, `psin_along`, `quadrature`, `chord_mask`, `line_integral`, `chord_reduce`, `pinhole_angles` | 同名（`-7` 未知规则） | `device.Chord`, `device.line_integral` |
| 收敛线积分、反转半径 | `line_integral_converged`, `inversion_radius` | **未导出** | — |
| 层析 | `besselj`, `bessel_zeros`, `tomography_basis`, `tomography_rows` | 同名 | `tomography.invert`（已退役至 git） |
| 电流质心 | `current_centroid` | `current_centroid`（out3） | `S.analysis.moments` |
| 自校准 | `factors_single`, `factors_over_slices`, `factor_dispersion` | `selfcal_single`, `selfcal_slices`, `factor_dispersion` | `selfcal.py`（已退役至 git） |
| POINT 约化 | — | — | `io.est2.read_east_mds`（装置文档常数） |
:::

(phys14-sources)=
# 来源与出处 (Sources & Attribution)

〔一手文献（源码引）〕Hankel 展开 {cite}`abramowitz1964handbook`；SciPy {cite}`virtanen2020scipy`。
