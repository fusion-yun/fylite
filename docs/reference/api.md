...
| `io.mds` | EAST MDSplus 取数（Thomson / 逆磁；`efit_east` 树只读其答案作对拍——测量字典读取器按 2026-09-15 用户裁定拒绝）。★传输走中间层的 mdsip 客户端（`kernel.MdsSession`），本包不 import 站点的 `MDSplus` 包 |
| `io.fydoc` | **数据层**的 Python 面（`libfylite.so`，`rust/fylite_runtime/`）：按内容识别文件类型，不同数据源 ↔ fyo 文档的读写与合并；MDSplus 只读、HDF5 / netCDF 的 fyo 与 IMAS 两种布局都在这一层 |
