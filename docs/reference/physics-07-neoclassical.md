---
title: 物理与数值 · 新经典输运 (Physics & Numerics — Neoclassical Transport)
subtitle: neoclassical.rs · dke.rs —— NEO 解析模型族（Sauter 1999 / Redl 2021 / Hirshman–Sigmar / Hinton–Hazeltine / Chang–Hinton / Koh）、σ_neo 与 ⟨j·B⟩↔⟨j_φ⟩ 换算、漂移动理学求解
---

(phys07-intro)=
# 引言：解析族与第一性解 (Introduction)

〔范围〕本章详述 `neoclassical.rs`（约 2300 行）与 `dke.rs`（约 2150 行）：前者是 GACODE **NEO 解析分支**
...
〔出处姿态〕〔源码〕两个模块的头部都声明"**非**清洁室"。源码逐字引的论文：Sauter 1999（含 eqs. (13)/(13a)/(13b)）、
Redl 2021（eqs. 11/14/16–21）、Hirshman–Sigmar 1977（Phys. Fluids 20, 418）；只给姓名的：Lin-Liu & Miller 1995、
Hinton–Hazeltine、Chang–Hinton、Taguchi、Hinton–Rosenbluth、Koh；**DKE 本体在源码中无任何 Belli/Candy 论文引用**。
本章补出处并标核验状态。
...
〔源码〕长度以小半径 $a$，场以 $B_{\rm unit}$，密度与温度归一到**物种 1**（由 `mapping::neo_inputs` 装配时为电子；
...
〔源码〕`surface_averages`：GEO（{ref}`phys04-geo`）在 **2001** 点上求解（`GEO_ntheta_in = 2001`），线性插值到
NEO 的 $n_\theta$ 网格；权 $w_k=g_\theta/B$ 归一；$\expval{B^2}$、$\expval{B_t^2}$、$\expval{a/R}$、$\expval{1/B^2}$、
$I/\psi'=fq/r$。"求积是源码的，不是我们选的更细的——2001 点上平均把 Sauter 电流移 $5\times10^{-6}$，是求积差不是错。"

$$
f_t=1-\frac34\frac{\expval{B^2}}{B_{\max}^2}\int_0^1\frac{\lambda\,\dd\lambda}{\expval{\sqrt{1-\lambda B/B_{\max}}}}
$$ (eq-p07-ft)

〔源码〕`nlambda = 500`，NEO 的开型规则（端权 23/12、次端 7/12）。〔已确立〕{eq}`eq-p07-ft` 是有效捕获份额的
标准定义 {cite}`hirshman1981neoclassical,linliu1995trapped`；源码只给例程名 `compute_fractrap`。
另有 SI 剖面模型用的**解析近似** $f_t^{\rm eff}(\varepsilon)=1-\frac{(1-\varepsilon)^2}{\sqrt{1-\varepsilon^2}(1+1.46\sqrt\varepsilon)}$
（Lin-Liu & Miller 1995 拟合 {cite}`linliu1995trapped`〔凭记忆〕）——源码明言"**不是**同一个量"。
...
〔源码〕两个年份**只差拟合**，装配相同（`sauter_currents`）：
...
〔出处〕{cite}`sauter1999bootstrap`（源码对 $X_{33}$/$F_{33}$/$\sigma_{\rm Sp}$ 引 eqs. (13)/(13a)/(13b)；对 $L_{31}$、$L_{32}$、$L_{34}$、$\alpha$
只引论文）。〔未核验〕本章未逐项核对源码系数与论文原文（Sauter 1999 另有 2002 年勘误
{cite}`sauter2002erratum`〔凭记忆〕，涉及 $F_{32}^{ee}$ 的一项）；证据是 NEO oracle 的位同一。

(phys07-sauter-2021)=
## Redl 2021 重标定 (The 2021 Recalibration)

〔源码〕"同一骨架，完全不同的拟合……$Z_{\rm eff}$ 依赖改变**形状**（$Z^{1.2}-0.71$ 对 1999 的 $Z+1$）。**$L_{34}=L_{31}$，精确**"：
...
$\alpha$、$L_{32}$ 的 2021 形式见源码（`redl_coefficients` / `sauter_redl`）。〔出处〕{cite}`redl2021bootstrap`（源码引 eqs. 11/14/16/17/18/19/20/21）。

:::{important}
〔$L_{34}=L_{31}$ 的裁定〕〔源码〕"★★★2026-08-30 裁定，此前的裁定是错的"：曾有两个 "Redl 2021" 宿主不一致——一个按
...
`c_api.rs::fylite_rs_redl_bootstrap` 注、`test_bootstrap_units.py` 注）。
:::

(phys07-sauter-si)=
## SI 剖面模型（Redl 2021） (The SI Profile Model)

〔源码〕`redl_bootstrap_point`（$\bar\psi$ **每弧度**，不再除 $2\pi$——曾双重换算使 $j_{bs}$ 大 $2\pi$，"实测 5.3–6.0×"）：
...
形式是 Sauter 1999 eqs. (18b)–(18e) {cite}`sauter1999bootstrap`（源码未说；IMAS 数据字典 {cite}`imbeaux2015imas` 亦用之）。
...
〔Hirshman–Sigmar〕〔源码〕`hirshman_sigmar`（`compute_HS`；"Phys. Fluids 20, 418 (1977)" {cite}`hirshman1977multispecies`）：
...
〔已确立：{cite}`helander2002collisional`〕；$L_{ij}$ 系数矩阵与源码一致。`erf` 自写（Maclaurin 级数 + 连分式，
...
〔Hinton–Hazeltine / Chang–Hinton / Taguchi / Hinton–Rosenbluth〕〔源码〕`analytic_fluxes`（`compute_HH`、`compute_CH`）：
...
Hinton–Rosenbluth 势是"**区间切换**而非混合"。**全部常数源码无文献**。〔编者对应〕Hinton–Hazeltine 综述
...
〔未核验〕源码系数与这些论文原文的逐项对应未查证；证据是 libneo 位同一。`chi_neo_ion`（{ref}`phys04-mapping-neo`）
拒绝轴（$(q/\varepsilon)^2$ 发散）。

〔Koh〕〔源码〕`koh`（`compute_Koh`，"Koh et al."，无年份）："Sauter 1999 的骨架，每个 $X$ 分子乘 $1+\delta$"，
...
〔σ_neo〕〔源码〕`sigma_neo_point`：$\sigma_{\rm neo}=\sigma_{\rm Sp}F_{33}$，$F_{33}$ 按年份（0 = 1999，1 = 2021，其他**拒绝**），
...
:::{note}
〔DKE oracle〕〔源码〕以漂移动理学解为 oracle（梯度全零、$E_{\parallel0}=1$ ⇒ $\expval{\vb j\cdot\vb B}=\sigma_{\rm neo}$，同一单位）：
...
〔所解方程〕〔源码 / 推断〕单磁面上、$\rho_\ast$ 一阶线性化的稠态漂移动理学方程：非绝热扰动 $g_s$ 关于 Maxwellian，
$(\theta,\xi,v)$ 局域空间，**全线性化 Fokker–Planck 碰撞算子**（NEO `collision_model = 4`，"FULL LINEARIZED FOKKER-PLANCK"），
**无旋转**（`rotation_model = 1`：全部旋转项恒为零，源码逐条说明），源 = 径向磁漂移 × （密度、温度、势梯度）+ 感应
$\expval{E_\parallel B}$ 驱动；准中性给出一阶势 $\delta\phi(\theta)$。〔出处〕这是 NEO 的表述 {cite}`belli2008neo,belli2012neo`〔后者凭记忆〕；
**源码中无此引用**。

〔基〕〔源码〕俯仰角：Legendre $P_l(\xi)$；能量：广义 Laguerre 族（`laguerre_method` 1 = "half & three-halves"（缺省）、
...
〔碰撞算子〕〔源码〕`compute_fcoll(m_0,\lambda)`（$\lambda=(v_a/v_b)^2$）：三个 $\lambda$ 区间（$<1/10$、中、$>10$，`LAMBDA_LARGE = 10`）
...
多物种、杂质**非**痕量（进入全部 `js` 求和）。〔出处〕线性化 Landau 算子的 Laguerre–Legendre 矩阵元见
{cite}`belli2012neo`〔凭记忆〕、{cite}`helander2002collisional`。

〔组装与源〕〔源码〕未知量序 $((is\,n_e+ie)n_x+ix)n_\theta+it$；约束行（`ix = 0, it = 0, ie ∈ {0,1}`）$\sum_{jt}w_\theta g=0$
...
〔稀疏 LU〕〔源码〕COO **重复项求和**（"UMFPACK 求和重复三元组"）→ CSC → **Reverse Cuthill–McKee** 对称置换（"$(6,17,17)$
...
〔矩〕〔源码〕`transport`（`TRANSP_do`）：$ix=0$ → $\Gamma_s$、$Q_s$、$\delta\phi$；$ix=1$ → $\Pi_s$、$\expval{Bu_\parallel}$；$ix=2$ → $\Gamma_s$、$Q_s$
（权 $2/15$）；$ix=3$ → $\Pi_s$（$2/35$）；"俯仰角谐波决定一行贡献给**哪个**矩"。$\expval{j_\parallel B}=\sum_sZ_s\expval{Bu_\parallel}^{(n)}_s$；
流分解 $K_s$、$v_{\rm pol}$、$v_{\rm tor}$、$\expval{j_{\rm tor}/R}/\expval{1/R}$。`driftxrot3` "名为旋转、内容是几何"，在矩中出现。

〔libneo oracle〕〔源码〕四例 × 两分辨率（(3,5,9)、(6,17,17)），`jpar_dke`、`jtor_dke` 相对误差 $<10^{-8}$（实测 $10^{-14}$–$3\times10^{-10}$，
...
:::{table} 新经典内容与内核函数、C-ABI、Python 入口（2026-09-02 快照）。
:name: tbl-p07-asbuilt
:align: left

| 内容 | 内核函数 | C-ABI（`fylite_rs_*`） | Python |
| :--- | :--- | :--- | :--- |
| NEO 解析族 | `sauter`（0）, `sauter_redl`（1）, `analytic_fluxes`（2）, `koh`（3）, `hirshman_sigmar`（4）, Taguchi/HR（5） | `neo_sauter`（`vintage` 0–5；`-7` 未知） | `scenario.model.neoclassical.bootstrap`；`NeoSource(key=…)` |
| SI Redl 剖面模型 | `redl_coefficients`, `redl_bootstrap_point`, `redl_surface_inputs`, `thermal_pressure`, `psi_bar` | `redl_bootstrap`（8 行）, `redl_coefficients`, `redl_surface_inputs`, `redl_drive`, `trapped_fraction_eps` | `bootstrap_profile`；`RedlSource` |
| σ_neo 与换算 | `sigma_neo_point`, `jparb_jphi`, `f33_*` | `sigma_neo`（5 行）, `jparb_jphi` | `analysis.loop`（自举回灌，{ref}`phys03-rows-fsa`） |
| Chang–Hinton χ | `analytic_fluxes(..).efluxi_ch` | `neo_chi` | `closure.py`；`transport_step` model 2 |
| DKE | `energy_*`, `compute_fcoll`, `collision_matrices`, `equil_from_miller`, `assemble`, `sparse_solve`, `transport`, `solve_surface` | `dke_energy`, `dke_fcoll`, `dke_coll`, `dke_equil`, `dke_assemble`, `dke_transport`, `dke_solve` | `kernel.dke_solve`；`RESOLUTION` 档 |
:::
