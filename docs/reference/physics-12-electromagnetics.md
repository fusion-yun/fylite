---
title: 物理与数值 · 装置电磁、垂直稳定性与控制 (Physics & Numerics — Device Electromagnetics, Vertical Stability and Control)
subtitle: electromagnetics.rs · evolution.rs · stability.rs · control.rs —— 互感与响应、电压驱动电路演化、刚性 n=0 垂直模色散、秩一消元的线性化对象与 PD 闭环
---

(phys12-intro)=
# 引言：M、R 与一个时间推进 (Introduction)

〔范围〕本章详述四个模块：`electromagnetics.rs`（L2，约 800 行：EFIT 型平行四边形单元的互感矩阵、网格 / 探针响应、
通道空间电路矩阵）、`evolution.rs`（L5，约 100 行：电压驱动的隐式 Euler 电路推进）、`stability.rs`（L6，约 330 行：
刚性 n=0 垂直模的丝云、耦合梯度、刚度、理想刚度、色散根）、`control.rs`（L7，约 380 行：无质量等离子体的秩一消元、
开环增长率、PD 闭环仿真）。丝核与椭圆积分在 {ref}`phys01-filament`。

〔出处姿态〕〔源码〕`electromagnetics.rs` 承接 L1 的**位同一性契约**（逐项再现 `circuits.py`）；`stability.rs` 头部记其为
...
〔源码〕单元以六个平行数组 $(r,z,w,h,a,a_2)$ 给出（{ref}`phys01-filament-element`），剖成 $n_u\times n_v$ 丝云；
...
〔源码〕`grid_response`：$G_e(R_i,Z_j)=\frac{1}{n_f}\sum_{f\in e}M(R_i,Z_j;r_f,z_f)$ [Wb/A]（**全磁通**）；
...
（"死行而非无穷"）。源码："角度约定错了不会报错：拟合只是收敛到一个倾斜以匹配的等离子体"。Python 记录：另一实现用
...
〔源码〕"安匝空间的回路方程只带**每匝**几何量 $U/N=M_1\dot x+R_1x$，故 $M_{ch}=WM_1W^T$ 而 $R_{ch}=\sum_jw_j^2R_{1,j}$——
...
〔源码〕`evolution::step`：**纯隐式（向后）Euler**，无 θ 参数；电压取**区间末**样本；等离子体磁通增量作为 EMF 进入
...
向后 Euler 对 $L/R$ 时标谱无条件稳定 {cite}`leveque2007fdm`。等离子体 ↔ 电路的 GS 交替**不在**本模块（由场景层驱动：
...
〔源码〕`plasma_filaments`：由平衡自身剖面 $j_\phi=Rp'(\psi_N)+FF'(\psi_N)/(\mu_0R)$ 造格电流 $I_{\rm cell}=j_\phi\Delta r\Delta z$；
...
〔源码〕$\partial_ZM$ 由 $h=10^{-4}$ m 中心差分；$k$ 的二阶导以外步 $10^{-3}$ m、内步 $10^{-4}$ m 嵌套差分；$k>0$ 失稳。
〔已确立〕刚性位移模型：整个丝集同一 $\delta Z$，恢复力由外场曲率 $\partial_Z^2\psi_{\rm ext}$ 与被动导体涡流耦合给出
{cite}`lazarus1990vertical`〔凭记忆〕、`GK-TMT-11`；源码只注 fyeq 来源。
...
〔源码〕`dispersion_root`：$D(\gamma)$ 在 $\gamma$ 上单调，故**二分**（自 $\gamma_{\max}=10^6$ s⁻¹ 对半下探直到 $D\le0$，
...
裕度 $=k_{\rm ideal}/k-1$（"读自两个**刚度**，不读自增长率——理想不稳区无有限 $\gamma$ 可读"）。
...
〔TokSys 锚〕〔源码〕EAST #137985，全 90 段被动导体：刚度 278916 N/m（1 % 内）、裕度 0.6906（2 % 内）、
...
〔源码〕"等离子体在关心的时标上无质量，其力平衡是代数的，折成导体电感的秩一修正"；开环增长率 = $A$ 的最大实特征值
（`linalg::eigen`，{ref}`phys01-linalg`），不稳模取**实部**并归一到 $C_\xi\cdot\text{mode}=1$；"**必须**与 `dispersion_root` 一致——
...
:::{important}
〔拒绝〕〔源码〕"消元只在**电阻壁区**有效。$M^\ast$ 恰在 $k=k_{\rm ideal}$ 奇异，其上秩一减法不再翻转特征值——$-M^{\ast-1}R$
全部特征值为负、对象读作**稳定**，而色散路线说理想不稳。对最不稳的情形回答'稳定'是本模块最坏的失效形态，故在此**拒绝**
而非返回。"C-ABI `-6`；$k=0$ 亦拒绝；$k<0$ 不拒绝。
:::

(phys12-control-loop)=
## PD 闭环仿真 (Closed-Loop Simulation)

〔源码〕`close_loop`：隐式 Euler 对象 $(M^\ast/\Delta t+\mathrm{diag}R)x_{k+1}=\frac{M^\ast}{\Delta t}x_k+B_{\rm act}u_k$；初态沿不稳模
...
$B_{\rm act}$ 指向 IC 对（缺省方向 $(1,-1)$，反对称）；磁通环行 $C$、$P=I_pG_{\rm loops}$。源码记录 PF 电源单极滞后 $\tau\approx15$ ms
...
:::{table} 电磁 / 稳定性 / 控制内容与内核函数、C-ABI、Python 入口（2026-09-02 快照）。
:name: tbl-p12-asbuilt
:align: left

| 内容 | 内核函数 | C-ABI（`fylite_rs_*`） | Python |
| :--- | :--- | :--- | :--- |
| 互感矩阵、匝数、通道 | `mutual_matrix_self/_cross`, `scale_by_turns`, `channel_weights/_fold`, `channel_matrices`, `resistances` | 同名 | `fylite.device.conductor_set/channel_matrices` |
| 响应 | `grid_response`, `element_flux`, `filament_flux`, `element_response`, `probe_response`, `table_ratio_check` | 同名 | `device.*`, `recon_rs.coil_loop_rows` |
| 电路演化 | `evolution::step`, `evolve` | `step_circuits`（`-6` 非 SPD） | `S.control.evolution.evolve_free_boundary` |
| 垂直稳定性 | `plasma_filaments`, `coupling_gradient`, `vertical_stiffness`, `ideal_stiffness`, `dispersion_root` | `plasma_filaments`, `coupling_gradient`, `vertical_stiffness`, `ideal_stiffness`, `dispersion_root`（1 = 理想不稳） | `S.control.vstab`, `stability.vertical_mode` |
| 对象与闭环 | `control::plant`, `close_loop` | `vertical_plant`（`-6` 拒绝）, `vertical_loop` | `vertical.vertical_system`, `close_vertical_loop` |
:::
