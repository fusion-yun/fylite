...
（★Python 侧**没有命令行**，命令行只有 `fy`。Python 面见本页末节。）
...
★★**`facts` 在 `list` 下，不在 `data` 下**（`FYL-DESIGN-17` E-24：发现面只有
一处）。`fy data facts` 按名拒绝并指向 `fy list facts`——`data` 只搬数据，不回答
「有什么」。
