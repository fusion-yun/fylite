---
title: 物理与数值 · 磁面几何与局域平衡 (Physics & Numerics — Flux-Surface Geometry and Local Equilibrium)
subtitle: surfaces.rs · geometry.rs · mapping.rs · bundle.rs —— 描迹磁面、Miller/MXH 局域度规、GACODE 归一与 gyro-Bohm 单位
---

(phys04-intro)=
# 引言：两个几何层 (Introduction — Two Geometry Tiers)

〔范围〕本章详述四个模块：`surfaces.rs`（约 2900 行，从 $\psi(R,Z)$ 网格**描迹**磁面并求磁面
平均、$q$、体积、形状矩与输运梯子）、`geometry.rs`（约 530 行，GACODE `GEO` 的白箱翻译：
给定 Miller / MXH 参数化的**一个**磁面，构造其度规与磁面平均）、`mapping.rs`（约 960 行，
`tgyro_tglf_map.f90` / `tgyro_neo_map.f90` 的移植：物理剖面 → TGLF / NEO 的归一输入）、
`bundle.rs`（约 460 行，`expro_compute_derived` 的移植：导出剖面与 gyro-Bohm 单位）。

〔两个层不可互换〕〔源码〕`geometry.rs` 头部："`geometry.rs` 对一个 **Miller** 面回答同一批问题；
`surfaces.rs` 对一个**必须先在矩形网格的 ψ 图里找出来**的面回答它们。两者不可互换——一个被
给定形状，另一个必须描迹它。"由此本章分两半：{ref}`phys04-traced`（描迹层）与
{ref}`phys04-geo`（局域层），以 {ref}`phys04-mapping` 的归一层把两者接到输运求解器。

〔出处姿态的分裂〕〔源码〕`surfaces.rs` **没有任何上游来源与文献引用**，是本仓自有代码；
`geometry.rs`、`mapping.rs`、`bundle.rs` 是 GACODE 的**白箱翻译**（Apache-2.0，仓根 `NOTICE`；
GEO / TGYRO 取修订 5efddfdf1），其头部明言"**非**清洁室——刻意与 `equilibrium.rs` 相反，
...
〔COCOS 17〕〔源码〕仓级约定 COCOS 17 {cite}`sauter2013cocos`；`test_cocos_convention.py`
...
〔规范〕〔源码〕`surfaces.rs` 对规范不可知，除以下处：`trace` 要求磁轴为所给场的**最大值**
（全磁通 Wb 约定下成立；梯子传 $-\psi_N$ 以满足之）；`b_field` 与 `li3` 假定**全磁通** Wb
（$B_p$ 含 $2\pi R$）；`metrics_from_polys` 的 `dpsi` 取 **Wb/rad**；`fylite_rs_surface_fsa` 带显式
`psi_scale`（"$B_{\rm pol}=\abs{\nabla\psi}/R$ 只在每弧度规范成立"）。

〔GACODE 方向标志〕〔源码〕`sign_bt = -signb`、`sign_it = -signb·signq`（TGLF），
`btccw = -signb`、`ipccw = -signb·signq`（NEO），`signb = field_sign(torfluxa)`（零映到 $+1$）。
源码只写"上游的拆法"，BTCCW/IPCCW 的含义无进一步出处；其与 COCOS 的映射见 `GK-TMT-04`。

〔两个梯子〕〔源码 / Python〕`TRANSPORT_LADDER = (0.02, 41)`：41 个面落在 $\psi_N\in[0.02,0.95]$，
**不含磁轴**（等值线在轴上退化；输运网格自带轴节点 `with_axis_node`）；`MILLER_LADDER = (0.1, 24)`
（"近轴形状导数是粗网格最先丢失的东西"）。源码记录两种梯子曾因缺省面数不同而给出不同答案，
...
〔算法〕〔源码〕`trace(g, psi, level, axis, limiter, ntheta)`：
...
〔为什么不是 marching squares〕〔源码〕`contour` 存在但**只为绘图**："线段汤穿过鞍点时无法可靠
走通"；任何需要**有序**轮廓的量（形状矩、磁面积分）都走 `trace`。四交点的鞍点歧义单元按边序
配对、不作消歧。

〔颈部剪裁的依据〕〔源码〕"$\psi$ 活在 25 mm 网格上而描迹器步进 6 mm 并二分四十次，一条射线
能穿过而邻居不能的通道是低于场自身分辨率的双线性细节"——一条对准鞍点的射线曾使
$z_{\min}$ 拖到 $-0.948$ m、$\kappa$ 报 1.79 对 EFIT 的 1.389。剪裁是**分辨率论证**，不是物理。

〔边界层级〕〔源码〕头部记：边界形状在 $\bar\psi=0.995$ 而非分离面描迹——"分离面在 X 点有角，
...
`Grid::sample` 双线性插值，**箱外返回 NaN**（"调用方不能把外推误当数据"）；$\abs{\nabla\psi}$
...
〔定义〕〔源码〕`surface_integrals` 对闭合多边形逐边取中点 $(R_m,Z_m)$、边长 $\dd l$、$g=\abs{\nabla\psi}(R_m,Z_m)$，
以**同一权重** $w=R\,\dd l/\abs{\nabla\psi}$ 求全部平均：
...
所依赖的"平均与散度交换"关系的来源（`GK-TMT-04`）。源码注明这些**不是**"轮廓均值"，并给出
...
〔体积〕〔源码〕`surface_volume` ≡ `enclosed_volume`：
...
是 $R$ 的二次式，故三点式是**精确积分**而非求积（源码原话："Pappus by way of Green's theorem"）。
取绝对值使之与走向无关。

(phys04-q)=
## $q$ 的两条通道与端点约定 (Two Routes to q)

〔通道一：环路积分〕〔源码〕`q_profile`：
...
以 $\abs{\nabla\psi_{\rm full}}=2\pi RB_p$ 代入即 {eq}`eq-p04-q`。源码记录了把 $R^2$ 写进被积式的
...
〔通道二：表格〕〔源码〕`metrics_from_polys` **不算** $q$，而是按 $\psi_N$ 用 `interp_uniform`
读 g 文件的 `qpsi` 表。两条通道并存：梯子用表，`fylite_rs_q_profile` 用积分。
`test_traced_geometry_tier.py` 断言描迹 $q$ 与 g 文件自身 $q$ 相对差 $<0.05$。

(phys04-ladder)=
## 输运梯子：$\rho$、$V'$ 与 IMAS 度规 (The Transport Ladder)

〔算法〕〔源码〕`metrics_from_polys`（输入 $-\psi_N$ 图、层级、多边形、`q_table`、`f_table`、
`dpsi` [Wb/rad]、`b0` [T]）：
...
4. $\dd\rho/\dd\psi_N$ 由 `kernels::gradient`（内点二阶，**端点一阶单侧**，{ref}`phys01-interp`）。
...
（$\expval{\abs{\nabla\rho}^2/R^2}$、$\expval{\abs{\nabla\rho}^2}$、$\expval{\abs{\nabla\rho}}$）；源码未给 IDS 版本。

〔轴节点〕〔源码〕`with_axis_node` 前置一个轴节点：`zero` 块（$\rho$、$\psi_N$、$V'$）置 0，
...
〔形状矩〕〔源码〕`shape_metrics` 从描迹轮廓读 Miller 五量：
...
$Z_0$ 是 Miller 的 `zmag`，"**不是**位移磁面上的磁轴高度"。

〔Miller 边界〕〔源码〕`miller_boundary`：$R(\theta)=R_0+a\cos(\theta+\arcsin\delta\sin\theta)$、
$Z(\theta)=Z_0+\kappa a\sin\theta$，三角度**上下各取**；无方形度。这是 Miller 等的局域平衡参数化
{cite}`miller1998noncircular`（源码只用其名，未引论文）。

〔Miller 行与剪切〕〔源码〕`miller_from_polys`（"GACODE `profiles_gen` 约定"）：
$\delta=\mathrm{clamp}(\tfrac12(\delta_u+\delta_l),-0.99,0.99)$，径向导数按描迹小半径 `rmin` 用 `kernels::gradient`：
...
而 `bundle.rs` 交给 GEO 的 `s_delta` 是**裸的** $r\,\dd\delta/\dd r$，由 `geometry::solve` 自己乘
$1/\cos(\arcsin\delta)$（{ref}`phys04-geo-angle`）。两层因此把**不同定义**的 `S_DELTA_LOC` 交给
TGLF；哪一个符合 TGLF 的约定，源码未说。此为本章标出的**待裁定项**。
:::

(phys04-xpoint)=
## X 点、打击点与其他几何原语 (X-Points, Strike Points and Other Primitives)

〔源码〕`x_points`：对 $2..n-2$ 的节点，$\det H=\psi_{RR}\psi_{ZZ}-\tfrac14(\psi_{RZ}+\psi_{ZR})^2<0$、
...
{cite}`uckan1990guidelines`〔凭记忆〕；源码未注出处。
...
〔源码〕`Surface`：长度归一到小半径 $a$、场归一到 $B_{\rm unit}$；$\theta\in(-\pi,\pi]$ 取 $n_\theta$ 点、**端点重复**
...
守卫：$\abs{\text{signb}}<10^{-10}$ → `-1`（"源码在此 STOP"），$\abs\delta>1$ → `-2`，$n_\theta<8$ → `-3`。
...
形式；GEO 的算子求值方法见 Candy {cite}`candy2009unified`。源码只注 `geo.f90`。

(phys04-geo-metric)=
## 度规、场与磁面平均 (Metric, Fields and Averages)

〔源码〕逐 $\theta$：$g_{\theta\theta}=R_\theta^2+Z_\theta^2$、$J_r=R(R_rZ_\theta-R_\theta Z_r)$、
...
〔磁面平均〕〔源码〕**权重 $G_\theta/B$**，$G_\theta=\frac{RB\ell_\theta}{rR_0\abs{\nabla r}}$：
...
周期 $n_\theta-1$。源码记录一次索引移位缺陷（`(k-1).rem_euclid(m)`）曾使 `gradpar_Bmag` 差 $4.3\times10^{-3}$、
自举电流差 $3\times10^{-4}$——NEO 的 `gradpar_Bmag` 在 $\theta=-\pi$ 由对称性恰为零是抓住它的判据。

〔漂移系数与 $E_{1..4}$〕〔源码〕`gsin`、`gcos1`、`gcos2`、`captheta`、`nu`、$f'$：
...
{cite}`candy2009unified`；源码只注 `geo.f90`。

〔本移植的增项〕〔源码〕两个 Fortran 里**没有**的标量：`fsa_r2` $=\expval{R^2}$（"通道曾用
...
〔源码〕`mapping.rs` 全程 **CGS、eV**（"TGYRO 自己的单位"）；常数与 `tgyro_globals` 逐字相同：
...
梯度 ×$10^{-2}$、kg→g ×$10^3$；`pext` Pa→barye ×10）。源码记录 `pext/dpext` 曾是"这个 SI 签名里内核
...
〔源码〕`tglf_local`（`tgyro_tglf_map.f90`）以 $\hat q=\abs q$、$\hat r=r/a$：
...
〔源码〕`neo_inputs`（`tgyro_neo_map.f90`）：$T_{\rm norm}=T_{i,1}$、$n_{\rm norm}=n_e$；
...
Sauter 入口读**另一套** 14 槽序 `NEO_SAUTER_SLOTS`（源码：按错序手搭曾使通量差 200×）。
归一器（`tgyro_flux.f90`）：$\Gamma_{GB}^{\rm NEO}=n_1T_1^{3/2}\rho_\ast^2$、$Q_{GB}^{\rm NEO}=n_1T_1^{5/2}\rho_\ast^2$、
$\Pi_{GB}^{\rm NEO}=n_1T_1^2\rho_\ast^2$（物种 1 参照，**非**电子参照）；电流单位
$\expval{\vb j\cdot\vb B}=\texttt{jpar\_neo}\times J_{eV}n_ev_{\rm norm}B_{\rm unit}$，测试钉
`neo_current_unit(3e19, 1500, 3.0) ≈ 3.864e6` A·T/m²。"NEO 的 `jpar` 对 $\rho_\ast$ 精确**线性**，
...
$$
B_{\rm unit}=\frac{\dd(\Phi_t/2\pi)}{\dd(r^2/2)}\ (\texttt{bundle::derive},\ \text{三点 Lagrange 导数}),\qquad
B_{\rm unit}=\frac{\abs{\dd\Phi/\dd r}}{2\pi r}\ (\texttt{mapping::b\_unit\_from\_rho},\ \texttt{kernels::gradient})
$$ (eq-p04-bunit)

〔已确立〕两式相等（$\dd(r^2/2)=r\,\dd r$）；差别在差分算子：后者端点一阶单侧，测试记录"梯子首
节点读到 1.5 倍真场"。〔出处〕$B_{\rm unit}\equiv\frac{q}{r}\dv{\psi}{r}$ 是 GACODE / GYRO 族的有效场
{cite}`waltz1999shape,gacode_geometry`〔凭记忆〕；源码称之为"归一层产出的**最关键**的数字，错了什么也
...
〔源码〕`bundle::derive`（`expro_compute_derived`）：$a=r_{\min}[n-1]$（最后闭合面）；剪切参数
"上游的**逐量**约定" $\hat s=r\,\dd\ln\abs q/\dd r$、$s_\kappa=(r/\kappa)\dd\kappa/\dd r$、$s_\delta=r\,\dd\delta/\dd r$、
...
源码只注 `expro_util`。测试：$B$ 加倍 $\chi$ 四分之一（$10^{-12}$）；$Q_{GB}(8\,\text{keV})/Q_{GB}(2\,\text{keV})=4^{2.5}$。
...
2. **磁轴不在梯子上**：轴桩假定 $\dd V/\dd\psi_N\to$ 常数；近轴精度实测 $\psi_N=0.23$ 处 5.8 %，
   0.35 处 1.3 %，向外 $<1\%$。
3. **端点差分一阶**（`kernels::gradient`）——$B_{\rm unit}$、剪切在梯子两端最不可靠；`bundle` 层改用
   三点 Lagrange 导数正是为此。
...
9. **GACODE 方向标志的意义未在源码给出**；与 COCOS 的对应须回 `GK-TMT-04` 或 {cite}`sauter2013cocos` 核对。
...
| `bundle::derive` vs TGYRO `treg01` | `out.tgyro.geometry.1/2` | $B_{\rm unit}$、$\hat s$、位移、$V$、$V'$、$\expval{\abs{\nabla r}}$ 相对 $<10^{-5}$；$B_{\rm unit}[0]=2.680430$ T |
...
:::{table} 几何层的主要 C-ABI 入口（`fylite_rs_*`，2026-09-02 快照；完整清单见 `c_api.rs`）。
:name: tbl-p04-asbuilt
:align: left

| 内容 | 内核函数 | C-ABI | Python |
| :--- | :--- | :--- | :--- |
| 描迹 + 磁面积分 | `surfaces::trace`, `surface_integrals` | `trace_surface`（info6）, `surface_fsa`（out6，`psi_scale`） | `fylite.kernel.trace_surface` |
| 输运梯子 + Miller 行 | `equilibrium_ladder`, `metrics_from_polys`, `miller_from_polys` | `equilibrium_ladder`, `equilibrium_ladder_r2` | `fylite.fyo.Ladder` |
| $q$、$F$、体积、$l_i(3)$ | `q_profile`, `f_profile`, `enclosed_volume`, `li3` | `q_profile`, `f_profile`, `enclosed_volume`, `li3` | `fyo` |
| X 点、打击点、间隙、等磁通 | `x_points`, `strike_points`, `wall_clearance`, `shape_observables` | 同名 | `S.design.shape` |
| GEO 局域度规 | `geometry::solve` | `geo_surface`（out14）, `geo_surface_r2`, `geo_surface_gm2` | `fylite.kernel` |
| TGLF / NEO 输入 | `mapping::tglf_local`, `tglf_species`, `neo_inputs` | `tglf_local`, `neo_inputs`, `surface_derived`（out8 SI） | `scenario.model.mapping` |
| $B_{\rm unit}$、稀释、碰撞率 | `b_unit_from_rho`, `ion_dilution`, `collision_rates` | 同名 | `scenario.model.closure` |
| 导出剖面 + gyro-Bohm | `bundle::derive`, `gyrobohm`, `bound_deriv` | `bundle_derive`（21 行 + 6 行 GB）, `gyrobohm`, `bound_deriv` | `scenario.model.mapping` |
:::
...
`profiles_gen`——GACODE 修订 5efddfdf1，Apache-2.0，作者见仓根 `NOTICE`。源码中对这四个模块的
**全部**公式只给上游文件名，未给论文；上表的一手文献是编者的对应。

〔本仓自有（源码未注出处）〕`surfaces.rs` 全部：射线 + 40 次二分的描迹器、`NECK_TOL = 1.25` 与五点
