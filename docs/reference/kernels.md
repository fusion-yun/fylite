...
物理与数值（fydoc `physics/00-overview.md`）十五章里；下表第三列是模块到章节的映射。

| 层 | 模块 | 详述 |
| :--- | :--- | :--- |
| 数值底座 | `kernels.rs` `linalg.rs` | 01 数值内核（fydoc `physics/01-numerics.md`） |
| 平衡正解 / 反解 | `equilibrium.rs` · `inverse.rs` | 02（fydoc `physics/02-equilibrium.md`） · 03（fydoc `physics/03-reconstruction.md`） |
| 磁面、局域几何与归一 | `surfaces.rs` `geometry.rs` `mapping.rs` `bundle.rs` | 04 几何与归一（fydoc `physics/04-geometry.md`） |
| 芯部输运、0-D | `transport.rs` · `zerod.rs` | 05（fydoc `physics/05-transport.md`） · 06（fydoc `physics/06-zerod.md`） |
| 新经典 | `neoclassical.rs` `dke.rs` | 07 新经典（fydoc `physics/07-neoclassical.md`） |
| 湍流与代理 | `gyrofluid.rs` `closure_tables.rs` `flr_tables.rs` `nn.rs` `bgb.rs` | 08 湍流（fydoc `physics/08-turbulence.md`） |
| 加热与电流驱动、源项 | `heating.rs` · `sources.rs` | 09（fydoc `physics/09-heating.md`） · 10（fydoc `physics/10-sources.md`） |
| 边缘、中性粒子与台基 | `edge.rs` `edge_tables.rs` `neutrals.rs` `pedestal.rs` `pedestal_tables.rs` | 11 边缘与台基（fydoc `physics/11-edge.md`） |
| 电磁、演化、稳定性、控制 | `electromagnetics.rs` `evolution.rs` `stability.rs` `control.rs` | 12 电磁与控制（fydoc `physics/12-electromagnetics.md`） |
| 放电设计、击穿 | `pulse.rs` `breakdown.rs` | 13 放电设计与击穿（fydoc `physics/13-pulse-breakdown.md`） |
| 拟合与诊断 | `fitting.rs` `diagnostics.rs` | 14 拟合与诊断（fydoc `physics/14-diagnostics.md`） |
...
| | |
| :--- | :--- |
| 位置 | `tests/data/FYDOC-CASE-03-frozen-libs/corpus/`（`$FY_ORACLE_STORE` 可覆盖），**15 个入口目录、625 条记录、9.7 MB** |
...
内核只算数，**取数与格式归数据层**——`rust/fylite_runtime/`，本仓唯一的 Rust 源码树。
