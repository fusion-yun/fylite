...
#: ★★**版别也要让 Python 那一层读得到**（2026-09-08 用户裁定，`FYL-DESIGN-19` A-18）。
...
#: `fylite.notice` 按 `_notice.json` 的 `default_flavour` 兜底成 internal——
#: 兜的方向是**多说一句**，见那个文件里的 `default_flavour_why`。
printf '%s\n' \
