//! SHA-256 — the checksum an `spo:Concretization` carries.
//!
//! ★Written out rather than pulled in: this crate's dependency table is
//! empty on purpose (`Cargo.toml`), and a digest is eighty lines of
//! arithmetic that FIPS 180-4 fixes to the bit.  Checked against the
//! standard's own vectors below.
