...
| 上游 | 版本 | 许可 | 用途 | 位置 |
| :--- | :--- | :--- | :--- | :--- |
| **h5wasm**（美国国家标准与技术研究院，**NIST**） | 0.10.3 | NIST 公共服务条款（见 `ui/assets/vendor/h5wasm/LICENSE.txt`，原样保留） | 在浏览器里读 HDF5：本仓的 HDF5 面链 `libhdf5` 这个 C 库，而该库在 `wasm32-unknown-unknown` 上编不出来（实测：`libc::FILE` / `off_t` / `ssize_t` 不存在），故浏览器侧由它把 `.h5` 解成一份 fyo 文档，再进源栈 | `ui/assets/vendor/h5wasm/`，按需 `import()`（约 4.2 MB，不进预缓存） |

★**明确承认**：h5wasm 系 **NIST** 开发的软件；其许可要求「明确承认 NIST 为该软件的来源」，
此处即为该承认。本仓**未对其作任何修改**——`tools/vendor-h5wasm.mjs` 只拷贝、不打补丁，
逐文件的 sha256 记在 `ui/assets/vendor/h5wasm/PROVENANCE.md`。h5wasm 内含经 Emscripten
...
| 出处 | 用于 | 位置 |
| :-- | :-- | :-- |
| T. H. Stix, *Plasma Physics* 14 (1972) 367 | 快离子慢化、临界能量、离子 / 电子份额、导心轨道与首轨损失 | `rust/fylite/src/hcd/`（`slowing.rs`、`beam_current.rs`） |
| D. R. Mikkelsen, C. E. Singer, *Nucl. Technol./Fusion* 4 (1983) 237；Y. R. Lin-Liu, F. L. Hinton, *Phys. Plasmas* 4 (1997) 4179 | 束驱电流积分及其俘获电子修正 | `rust/fylite/src/hcd/beam_current.rs` |
| J. Mandrekas, GTFR-102 (1992) | 束弦衰减、矩形足迹与穿透份额 | `rust/fylite/src/hcd/{nbi,footprint}.rs` |
| A. C. Riviere, *Nucl. Fusion* 11 (1971) 363；Hill et al., *Nucl. Fusion* 63 (2023) 125001 | 束电离的分道截面：电子碰撞、质子碰撞电离、电荷交换 | `rust/fylite/src/hcd/stopping.rs` |
| L.-G. Eriksson, T. Hellsten, *Physica Scripta* 52 (1995) 70；Wesson, *Tokamaks* 第 3 版 §5.8 | 少数离子 ICRH 的电子份额与快离子含量 | `rust/fylite/src/hcd/icrh.rs` |
| Y. R. Lin-Liu, V. S. Chan, R. Prater, *Phys. Plasmas* 10 (2003) 4064 | EC 驱流效率 | `rust/fylite/src/hcd/cd.rs` |
...
| R. K. Janev, C. D. Boley, D. E. Post (1989) | 中性束停止 / 电荷交换截面 | `rust/fylite/src/hcd/stopping.rs` |
| S. Weiland et al., *Nucl. Fusion* 58 (2018) 082032（RABBIT） | 快 NBI 模型类（仅作保真度分级，未见其源码） | `python/fylite/scenario/model/nbi.py` |
...
| ITER 装置描述（PF/CS 线圈、壁、110 个磁通环；ITER EDA，2010-04-26） | ITER 组织，经 `fydata` 包 | 依 fydata 所记 | 浏览器预置装置（`ui/facts/device/iter.jsonld`）；演示内置的 ITER 位形即出自此 |
...
- 0-D 集成建模与加热 / 驱流的对标程序（仅作基准，不是本仓任何代码的出处）：J. F. Artaud et al.,
