// app/（本仓）-> fylite_data 的 rust/fylite_data/src/bin/app/assets.rs（内嵌资源表，生成物）
//
...
//: ★★2026-09-02：`app/` 与 `rust/fylite_data/` 都在本仓了——数据层从内核仓搬了
//: 过来，于是这张表的读者与被读的目录同处一棵树。从前这里要跨仓解析内核检出
//: （`$FYLITE_KERNEL`，探测不到就报错），那一整段随之取消：现在是一个相对路径，
//: 猜不错也不需要猜。
const OUT = HERE + '../rust/fylite_data/src/bin/app/assets.rs';
...
const rows = files.map((f) => {
  ...
  //: 这里写的是 `../../../../../app/${f}`——从 `rust/fylite_data/src/bin/app/` 上溯
...
if (check) {
  ...
  console.log(`[embed] -> rust/fylite_data/src/bin/app/assets.rs（${files.length} 个文件）`);
}
