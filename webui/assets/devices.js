...
//
// PRESET devices are fyo/JSON-LD documents under `webui/facts/device/`, listed by
// `webui/facts/device/catalogue.jsonld`.  ★2026-09-04 `webui/facts/device/` -> `webui/facts/device/`:
// `webui/facts` is a symlink to the repository's `facts/` corpus, so the page fetches
...
//
// ★And a preset is a REDISTRIBUTION: `webui/` is published.  Which machines
...
(function (root) {
  ...
  //: sit at two depths (`webui/index.html` and `webui/pages/*.html`) and a
