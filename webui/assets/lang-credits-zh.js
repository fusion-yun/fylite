...
self.FyI18n.register('zh', {
  ...
  'cr.license.h2': '版权与许可',
  'cr.license.p': 'fylite 的开发是中国科学院等离子体物理研究所（ASIPP）<strong>「集成建模讨论组」</strong>工作的一部分，版权归 ASIPP 与 fylite 贡献者所有，按 <strong>Apache License 2.0</strong> 发布：允许自由使用、修改与再分发（含商业用途），条件为保留版权声明与 <code>NOTICE</code>，并对所作修改予以说明。许可全文见仓库的 <code>LICENSE</code>。',
  'cr.license.files': '★本页面（<code>webui/</code> 下的浏览器演示）与它加载的计算内核制品同属该许可：内核由本仓源码构建，制品的 sha256 与出处记录在仓库文档里，可逐位核对。',
  ...
  'cr.nottaken.h2': '刻意未取的东西',
  'cr.nottaken.lead': '一份只列取用的致谢是不完整的。下面几项是<strong>有意不取</strong>的，理由各不相同，但都属于同一份记录：',
  'cr.nottaken.efit': '<strong>EFIT 一脉</strong>：任何形式的 EFIT 族源码、格林表生成器或录得输出都不在仓内；平衡与反演求解器属清净室实现。',
  'cr.nottaken.gray': '<strong>GRAY</strong>：EC 射线追踪的移植已停止；其许可不容直接翻译，fylite 未读过任何 GRAY 物理源码。',
  'cr.nottaken.umfpack': '<strong>UMFPACK</strong>：未移植——本仓的稀疏 LU 从零写起。',

  'cr.thanks.h2': '致谢',
  'cr.thanks.group': '<strong>集成建模讨论组</strong>（ASIPP）：孙有文、任启龙、简祥、夏天阳、于治、刘晓娟、胡也民、肖小涛等。',
  ...
  'cr.thanks.kefit': '<strong>钱金平、李国强、任启龙、L. L. Lao 等</strong>——将 EFIT 移植至 EAST 并建立动理学重构（KEFIT），本仓重构线的上游参照基线。',
  ...
  'cr.thanks.kefitops': '<strong>吴学民、盛回</strong>——KEFIT 运行流程的梳理与指导，并提供 benchmark 算例。',
