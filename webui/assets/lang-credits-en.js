...
self.FyI18n.register('en', {
  ...
  'cr.lead': 'This page states three things: the <strong>copyright and licence</strong> of this project, the <strong>provenance</strong> of each physics module, and the work that is owed acknowledgement. The per-file attribution of record is the repository NOTICE; this page is its readable summary, and every statement made here should be traceable to <code>NOTICE</code> or <code>ACKNOWLEDGEMENTS.en.md</code>.',

  'cr.license.h2': 'Copyright and licence',
  'cr.license.p': 'fylite is developed as part of the work of the <strong>Integrated Modelling Discussion Group</strong> at the Institute of Plasma Physics, Chinese Academy of Sciences (ASIPP). Copyright is held by ASIPP and the fylite contributors, and it is released under the <strong>Apache License 2.0</strong>: free use, modification and redistribution (including commercially) are permitted, provided the copyright notice and <code>NOTICE</code> are retained and modifications are stated. The full licence text is in the repository <code>LICENSE</code>.',
  'cr.license.files': '★This page — the browser demos under <code>webui/</code> — and the compute kernels it loads are under the same licence: the kernels are built from this repository\'s own sources, and each artifact\'s sha256 and provenance are recorded in the repository documentation, so they can be checked byte for byte.',
  ...
  'cr.nottaken.h2': 'What was deliberately not taken',
  'cr.nottaken.lead': 'An acknowledgement that lists only what was taken is incomplete. The items below were <strong>deliberately not taken</strong>, for differing reasons, but they belong to the same record:',
  'cr.nottaken.efit': '<strong>EFIT lineage</strong>: no EFIT-family source, Green&rsquo;s-table generator or recorded output is present in any form; the equilibrium and inverse solvers are clean-room.',
  'cr.nottaken.gray': '<strong>GRAY</strong>: the EC ray-tracing port was stopped; its licence does not permit direct translation, and no GRAY physics source was read for fylite.',
  'cr.nottaken.umfpack': '<strong>UMFPACK</strong>: not ported &mdash; the sparse LU is written from scratch.',

  'cr.thanks.h2': 'Acknowledgements',
  'cr.thanks.group': '<strong>The Integrated Modelling Discussion Group</strong> (ASIPP): Youwen SUN, Qilong REN, Xiang JIAN, Tianyang XIA, Zhi YU, Xiaojuan LIU, Yemin HU, Xiaotao XIAO and others.',
  ...
  'cr.thanks.kefit': '<strong>J. P. Qian, G. Q. Li, Q. L. Ren, L. L. Lao et al.</strong> — for porting EFIT to EAST and building its kinetic reconstruction (KEFIT), the upstream reference baseline of the reconstruction line here.',
  ...
  'cr.thanks.kefitops': '<strong>Xuemin WU, Hui SHENG</strong> — guidance on and clarification of the KEFIT operating procedure, and the benchmark cases they provided.',
