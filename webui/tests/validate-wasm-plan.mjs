...
const ROOT = HERE + '../../';
const ASSETS = ROOT + 'webui/assets/';
...
// --- 五、盘上有没有（有产物时才查） -----------------------------------------
console.log('\n〔五〕这一份检出里实有的');
const v = (/kernel:\s*'([^']*)'/.exec(read('webui/assets/version.js')) || [])[1];
const rv = (/FyRuntimeVersion\s*=\s*'([^']*)'/.exec(read('webui/assets/runtime-version.js')) || [])[1];
for (const row of PLAN) {
