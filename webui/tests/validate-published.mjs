...
//   〔三〕**页面自己不报错**。
//
//   node webui/tests/validate-published.mjs [--site https://fusion-yun.github.io/fylite/]
//                                         [--playwright DIR]
//
...
//: ★★**比对的参照物是「构建出来的那一份站点」，不是检出的 `webui/`**（2026-09-08）。
//: 发布出去的是**公开版**（`tools/build-site.sh --public`），而检出通常是缺省的
//: 内部版（`FYL-SDD-03` A-14），两者按构造就有三份不同：`runtime-version.js`
//: 里的版别标记，以及 `--strip` 从两份语料里删掉的内部提示词条。拿 `webui/` 去比，
...
const REF = (() => {
  ...
  if (!existsSync(dir + 'assets')) {
    ...
    console.log('      发出去的是公开版，而检出是缺省的内部版——拿 webui/ 比会红三条，');
