// 一个能跑的站点，给浏览器门用：磁盘上的 `webui/` + 真正的请求面 + 回放的 mdsip。
//
...
//      2026-09-01 退役 `webui/server/` 之后，`/api/*` 只有这一份实现。
//   3. **静态文件**：这里现起一个只读 HTTP，从**磁盘**上的 `webui/` 伺服，
//      `/api/*` 转给第 2 层。
//
// ★为什么第 3 层不直接用 `fylite` 自己的静态面：它伺服的是**编进二进制**的
// 那一份，改了 `webui/` 要重新 build 才看得见。门要能在改完页面之后立刻跑。两份是否
// 一致由 `validate-embed.mjs` 单独把关，所以这里从磁盘读不会放过「只改了源树、忘了
// 重嵌」这种错——那本来就是另一道门的事。
//
// ★这一层**没有任何协议代码**：它是静态文件加一个转发，mdsip 只被 `fylite`
// 说。这正是退役 `webui/server/` 的意思——一组端点只留一份实现。
 ...
 *
 * ★导出给别的门用：退役 `webui/server/` 之后，「起一个带请求面的宿主」只有这一种
