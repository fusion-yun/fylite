...
//
// Run: node webui/tests/validate-worker-interp-device.mjs [--record]
...
//: over.
//:
//: ★★缺省 λ 从 1e-3 起两次改动之后（2026-09-07 → 3e-1，2026-09-08 → 1e-1，
//: 后者由 `fylite_kernel` 仓 `tests/test_start_against_the_machine.py` 的真机比对
//: 定下），这一段的**数量级变了**，而且是变好的一侧。最早那份夹具（λ=1e-3 +
//: 投影梯度）记的是一个**塌掉的**位形：a_minor 0.0195 m —— 要的是
//: 0.6·a_max ≈ 0.357 m，差了十八倍 —— `chiE` / `chiI` 二十一级**全是 NaN**
//: （那一档的解释性输运根本没算出东西），tau_E 0.51 ms，W_th 1.5 kJ。
//: 现在这一份：a_minor 0.3279 m、chi 有限、gm3 / gm7 O(1)、tau_E 57 ms、
//: W_th 236 kJ。
//:
//: ★★**自由边界解仍然不收敛，这一条不是本次改动能收的。** 最早是 4.9e-8
//: （`converged: false`，跑满 400 步，但那属于那个 2 cm 的塌解：几乎没有东西要
//: 收敛）；λ=3e-1 上是 5.8e-2；现在 λ=1e-1 上是 **5.6e-3**，好了一个量级但仍未
//: 收敛。把预算加到 1200 步得 6.8e-3（`settled: true`，423 步），加到 4000 步
//: 反而回到 6.1e-2 —— **不随预算单调下降**，所以那不是「步数不够」。
