...
转置（`FYL-SDD-03` L-5）、int64 属性变成 BigInt 之后消失、标量与长度 1 数组混淆，这三类
...
b.write("webui/tests/fixtures/equilibrium.h5")
back = fydoc.read("webui/tests/fixtures/equilibrium.h5").to_dict()
open("webui/tests/fixtures/equilibrium.json", "w").write(
